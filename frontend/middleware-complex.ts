...
const ROUTE_PERMISSIONS: Record<string, { modules?: string[], roles?: string[], public?: boolean }> = {
  ...
  '/configuracoes/desempenho/tabela': { modules: ['dashboard_diario'], roles: ['admin'] },
  ...
  try {
    ...
    if (userCookie) {
      try {
        ...
        if (userData && userData.email && userData.id) {
          // Garantir que `ativo` seja true por padrão se não estiver definido
          const user = {
            ...userData,
        ...
        console.log('❌ Erro ao parsear cookie de usuário:', e)
      }
    }
    ...
    if (userDataHeader) {
      try {
        ...
        console.log('❌ Erro ao parsear header de usuário:', e)
      }
    }
...
export function middleware(request: NextRequest) {
  ...
  // Log para verificar se middleware está executando
  ...
  console.log(`🔍 Middleware Debug para ${pathname}:`, {
    hasUser: !!user,
    userCookie: !!request.cookies.get('sgb_user'),
    userHeader: !!request.headers.get('x-user-data'),
    userName: user?.nome || 'N/A',
    userRole: user?.role || 'N/A',
    userModules: user?.modulos_permitidos?.length || 0
  })
...
// Log para verificar se middleware está sendo carregado
console.log('🔧 MIDDLEWARE CARREGADO - arquivo middleware.ts') 