...
export default function PWAStatus({
  ...
  const [install, setInstall] = useState<() => Promise<void> | void>(() =>
    Promise.resolve()
  );
  ...
  const [updateServiceWorker, setUpdateServiceWorker] = useState<
    () => Promise<void> | void
  >(() => Promise.resolve());
  const [clearCache, setClearCache] = useState<() => Promise<void> | void>(() =>
    Promise.resolve()
  );
  ...
  if (compact) {
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        ...
        {isInstalled && <Badge className="badge-success text-xs">PWA</Badge>}
  ...
  return (
    <Card
      className={`bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 ${className}`}
    >
      <CardHeader>
          ...
          <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            {lastUpdateCheck && (
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 text-center">
                Última verificação:{' '}
                {lastUpdateCheck.toLocaleTimeString('pt-BR')}
              </p>
            )}
          </div>
        )}
