...
export default function SignaturePad({
  ...
  // Debug logs
  console.log('🔧 SignaturePad renderizado', {
    isEmpty,
    isUploading,
    onSignatureComplete,
  });
  ...
  const canvasToBlob = useCallback((): Promise<Blob> => {
    return new Promise((resolve, reject) => {
      ...
      canvas.toBlob(
        blob => {
          if (blob) {
            console.log('✅ Canvas convertido para blob', {
              size: blob.size,
              type: blob.type,
            });
            resolve(blob);
      ...
      if (onError) {
        onError(
          error instanceof Error
            ? error.message
            : 'Erro desconhecido ao salvar assinatura'
        );
      }
    } finally {
  ...
  return (
    <div
      className={`border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-800 ${className}`}
    >
      {/* Título */}
      <div className="flex items-center gap-2 mb-3">
        <Pen size={18} className="text-gray-600 dark:text-gray-400" />
      ...
      {/* Debug Info */}
      <div className="text-xs text-gray-500 dark:text-gray-400 mb-2 p-2 bg-gray-100 dark:bg-gray-700 rounded">
        <strong>Debug:</strong> isEmpty: {isEmpty.toString()}, isDrawing:{' '}
        {isDrawing.toString()}, isUploading: {isUploading.toString()}
      </div>
