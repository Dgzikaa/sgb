            ...
            <>
              <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center">
                {selectedOption.avatar ? (
                  <Image src={selectedOption.avatar} alt={selectedOption.label} width={32} height={32} className="w-full h-full rounded-full object-cover" />
                ) : (
              ...
              <div>
                ...
                {selectedOption.email && (
                  <p className="text-xs text-gray-500">{selectedOption.email}</p>
                )}
              </div>
            </>
          ) : (
              ...
              filteredOptions.map((option, index) => (
                ...
                >
                  <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center flex-shrink-0">
                    {option.avatar ? (
                      <Image src={option.avatar} alt={option.label} width={32} height={32} className="w-full h-full rounded-full object-cover" />
                    ) : (
                      <User className="w-4 h-4 text-indigo-600" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {option.role && (
                      <span className={`
                        ...
                        ${option.role === 'admin' ? 'bg-red-100 text-red-800' : 
                          option.role === 'manager' ? 'bg-blue-100 text-blue-800' : 
                          'bg-green-100 text-green-800'}
                      `}>
                        {option.role === 'admin' ? 'Administrador' : 
                         option.role === 'manager' ? 'Gerente' : 'Funcionário'}
                      </span>
                    )}
                  </div>
                </button>
