...
export const ConfirmDialogProvider: React.FC<ConfirmDialogProviderProps> = ({ children }) => {
  ...
  const getDialogStyles = () => {
    ...
    switch (type) {
      case 'danger':
        return {
          container: 'max-w-md bg-gradient-to-br from-white to-red-50 border-0 shadow-2xl',
          header: 'pb-6 border-b border-gray-200 bg-gradient-to-r from-red-50 to-rose-50',
          iconBg: 'bg-gradient-to-r from-red-500 to-rose-500',
          footer: 'pt-6 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-red-50'
        }
      case 'warning':
        return {
          container: 'max-w-md bg-gradient-to-br from-white to-orange-50 border-0 shadow-2xl',
          header: 'pb-6 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-yellow-50',
          iconBg: 'bg-gradient-to-r from-orange-500 to-yellow-500',
          footer: 'pt-6 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-orange-50'
        }
      case 'success':
        return {
          container: 'max-w-md bg-gradient-to-br from-white to-green-50 border-0 shadow-2xl',
          header: 'pb-6 border-b border-gray-200 bg-gradient-to-r from-green-50 to-emerald-50',
          iconBg: 'bg-gradient-to-r from-green-500 to-emerald-500',
          footer: 'pt-6 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-green-50'
        }
      default:
        return {
          container: 'max-w-md bg-gradient-to-br from-white to-blue-50 border-0 shadow-2xl',
          header: 'pb-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50',
          iconBg: 'bg-gradient-to-r from-blue-500 to-indigo-500',
          footer: 'pt-6 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-blue-50'
        }
    }
  }
...
export const useGlobalConfirm = () => {
  ...
  return {
    ...
    confirmDanger: (title: string, message: string, details?: string[], onConfirm?: () => void | Promise<void>) =>
      confirmDialog({
        title,
        message,
        type: 'danger',
...
export const GlobalConfirmListener: React.FC = () => {
  ...
  React.useEffect(() => {
    const handleShowConfirmDialog = (event: CustomEvent<ConfirmDialogOptions & { resolve: (value: boolean) => void }>) => {
      const { resolve, ...options } = event.detail
      confirm(options).then(resolve)
    }

    window.addEventListener('showConfirmDialog', handleShowConfirmDialog as EventListener)
    return () => window.removeEventListener('showConfirmDialog', handleShowConfirmDialog as EventListener)
  }, [confirm])
