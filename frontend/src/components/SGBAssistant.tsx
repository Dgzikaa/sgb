...
async function getBasicSalesData(barInfo: BarInfo | null): Promise<Record<string, unknown>> {
  try {
    ...
    const total = vendas?.reduce((sum: number, venda: unknown) => sum + parseFloat(venda.liquido || '0'), 0) || 0
...
async function analyzeToday(barInfo: BarInfo | null): Promise<{ content: string; metadata?: { command?: string; data?: Record<string, unknown> } }> {
  const today = new Date().toISOString().split('T')[0]
  try {
    const supabase = await getSupabaseClient();
    if (!supabase) return {
...
async function analyzeWeek(barInfo: BarInfo | null): Promise<{ content: string; metadata?: { command?: string; data?: Record<string, unknown> } }> {
  return {
...
async function detectAnomalies(barInfo: BarInfo | null): Promise<{ content: string; metadata?: { command?: string; data?: Record<string, unknown> } }> {
  return {
...
async function generateSuggestions(barInfo: BarInfo | null): Promise<{ content: string; metadata?: { command?: string; data?: Record<string, unknown> } }> {
  return {
...
async function processUserInput(input: string, barInfo: BarInfo | null): Promise<{ content: string; metadata?: { command?: string; data?: Record<string, unknown> } }> {
  ...
  try {
    ...
    return {
      ...
      metadata: { command: 'chatgpt', data: response.metadata as Record<string, unknown> }
    }
  } catch (error) {
