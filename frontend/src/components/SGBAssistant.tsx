    ...
    <div className="fixed right-4 bottom-4 top-4 w-96 bg-white/95 backdrop-blur-sm border border-gray-200/50 rounded-2xl shadow-2xl z-50 flex flex-col">
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-gray-200">
        ...
        <div className="mt-2 flex flex-wrap gap-1">
                     {['💰 Vendas hoje', '📊 Análise semana', '🔍 Anomalias', '💡 Sugestões'].map((cmd) => (
            <button
              key={cmd}
...
async function processUserInput(input: string, barInfo: { nome: string; id?: number } | null): Promise<{ content: string; metadata?: { command?: string; data?: unknown; feedback?: 'positive' | 'negative' } }> {
  const lowercaseInput = input.toLowerCase()

  // Comandos específicos que usam análise local
  ...
  try {
    ...
    // Buscar dados contextuais se necessário
    let contextData = null
    if (lowercaseInput.includes('vendas') || lowercaseInput.includes('faturamento')) {
      // Buscar dados básicos de vendas para contexto
    ...
    const response = await openaiClient.chat({
      ...
      context: {
        ...
        barId: barInfo?.id,
        currentData: contextData ? salesDataToRecord(contextData) : undefined
      }
    })
    ...
    // Fallback para comandos específicos em caso de erro
          if (lowercaseInput.includes('análise semana') || lowercaseInput.includes('📊 análise semana')) {
      ...
      return await detectAnomalies(barInfo)
          ...
          if (lowercaseInput.includes('sugestões') || lowercaseInput.includes('💡 sugestões')) {
...
async function getBasicSalesData(barInfo: { nome: string; id?: number } | null): Promise<SalesData | null> {
  try {
    ...
    const total = vendas?.reduce((sum: number, venda: { liquido: string; meio: string; created_at: string }) => sum + parseFloat(venda.liquido || '0'), 0) || 0
...
function salesDataToRecord(data: SalesData | null): Record<string, string | number | boolean> {
  ...
  return {
    ...
    quantidade: data.vendasHoje?.quantidade ?? 0,
    ticketMedio: data.vendasHoje?.ticketMedio ?? 0,
    // Adicione outros campos relevantes se necessário
  };
}

// Funções de análise (começando simples, vão evoluir)
async function analyzeToday(barInfo: { nome: string; id?: number } | null) {
  const today = new Date().toISOString().split('T')[0]
  ...
  try {
    ...
    return {
      content: '❌ Erro ao buscar dados de vendas de hoje. Verifique a conexão com o banco de dados.'
    }
  }
}

async function analyzeWeek(barInfo: { nome: string; id?: number } | null) {
  return {
...
async function detectAnomalies(barInfo: { nome: string; id?: number } | null) {
  return {
...
async function generateSuggestions(barInfo: { nome: string; id?: number } | null): Promise<{ content: string; metadata?: { command?: string; data?: unknown; feedback?: 'positive' | 'negative' } }> {
  return {
