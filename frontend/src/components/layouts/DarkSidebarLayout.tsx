...
function GlobalPageWrapper({ children }: { children: ReactNode }) {
  useEffect(() => {
    ...
    const applyDarkModeClasses = () => {
      // Cards sem dark mode
      const cards = document.querySelectorAll('[class*="bg-white"]:not([class*="dark:"])')
      cards.forEach(card => {
      ...
      const grayTexts = document.querySelectorAll('[class*="text-gray-900"]:not([class*="dark:"])')
      ...
      const grayTexts700 = document.querySelectorAll('[class*="text-gray-700"]:not([class*="dark:"])')
      ...
      const grayTexts600 = document.querySelectorAll('[class*="text-gray-600"]:not([class*="dark:"])')
      ...
      const borders = document.querySelectorAll('[class*="border-gray-200"]:not([class*="dark:"])')
      ...
      const inputs = document.querySelectorAll('input:not([class*="dark:"]), textarea:not([class*="dark:"]), select:not([class*="dark:"])')
      inputs.forEach(input => {
        input.classList.add('dark:bg-gray-700', 'dark:border-gray-600', 'dark:text-white', 'dark:placeholder-gray-400')
      })
    ...
    // Observar mudan̤as no DOM e aplicar automaticamente
  ...
  return (
    <div className="auto-dark-mode-wrapper">
      {children}
    </div>
  )
}
