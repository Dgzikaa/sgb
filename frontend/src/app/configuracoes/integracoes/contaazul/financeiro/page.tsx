...
export default function ContaAzulFinanceiroPage() {
  ...
  useEffect(() => {
    setPageTitle('Ã°Å¸â€™Â° ContaAzul - Dashboard Financeiro')
    return () => setPageTitle('')
  }, [setPageTitle])
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      if (reciverData && pagarData) {
        const reciverTotal = (reciverData as ContaReceber[]).reduce((sum: number, item: ContaReceber) => sum + (item.total || 0), 0);
        const reciverPago = (reciverData as ContaReceber[]).reduce((sum: number, item: ContaReceber) => sum + (item.pago || 0), 0);
        const reciverAberto = (reciverData as ContaReceber[]).reduce((sum: number, item: ContaReceber) => sum + (item.nao_pago || 0), 0);

        const pagarTotal = (pagarData as ContaPagar[]).reduce((sum: number, item: ContaPagar) => sum + (item.total || 0), 0);
        const pagarPago = (pagarData as ContaPagar[]).reduce((sum: number, item: ContaPagar) => sum + (item.pago || 0), 0);
        const pagarAberto = (pagarData as ContaPagar[]).reduce((sum: number, item: ContaPagar) => sum + (item.nao_pago || 0), 0);
      ...
      // 2. Carregar contas a receber (Ã¡Âºltimas 100)
      ...
      // 3. Carregar contas a pagar (Ã¡Âºltimas 100)
      ...
      // 5. Verificar Ã¡Âºltima sincronizaÃ¡Â§Ã¡Â£o
    ...
    return (
      <div className="p-8">
        <div className="text-center">
          <div className="text-lg">Ã°Å¸â€â€ž Carregando dados financeiros...</div>
        </div>
      </div>
    )
  }
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        <p className="text-gray-600">
          Ã¡Å¡ltima sincronizaÃ¡Â§Ã¡Â£o: {ultimaSync || 'Nunca sincronizado'}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-green-50 p-6 rounded-lg border border-green-200">
          <h3 className="text-lg font-semibold text-green-800 mb-2">Ã°Å¸â€œÂ¥ Receber - Total</h3>
        ...
        <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
          <h3 className="text-lg font-semibold text-blue-800 mb-2">Å“â€¦ Receber - Pago</h3>
          <p className="text-2xl font-bold text-blue-600">{formatCurrency(summary.receber_pago)}</p>
        </div>

        <div className="bg-red-50 p-6 rounded-lg border border-red-200">
          <h3 className="text-lg font-semibold text-red-800 mb-2">Ã°Å¸â€œÂ¤ Pagar - Total</h3>
        ...
        <div className="bg-purple-50 p-6 rounded-lg border border-purple-200">
          <h3 className="text-lg font-semibold text-purple-800 mb-2">Å“â€¦ Pagar - Pago</h3>
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">VisualizaÃ¡Â§Ã¡Â£o:</label>
          <select
            value={tipoVisualizacao}
            onChange={(e) => setTipoVisualizacao(e.target.value)}
            className="border border-gray-300 rounded-md px-3 py-2"
          >
            <option value="RECEBER">Ã°Å¸â€œÂ¥ Contas a Receber</option>
            <option value="PAGAR">Ã°Å¸â€œÂ¤ Contas a Pagar</option>
            <option value="CATEGORIAS">Ã°Å¸ÂÂ·Ã¯Â¸Â Categorias</option>
          </select>
        </div>
      ...
      {/* ConteÃ¡Âºdo Principal */}
      {tipoVisualizacao === 'RECEBER' && (
        <div>
          <h2 className="text-2xl font-bold mb-4">Ã°Å¸â€œÂ¥ Contas a Receber ({contasReceberFiltradas.length})</h2>
              ...
              <table className="min-w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">DescriÃ¡Â§Ã¡Â£o</th>
      ...
      {tipoVisualizacao === 'PAGAR' && (
        <div>
          <h2 className="text-2xl font-bold mb-4">Ã°Å¸â€œÂ¤ Contas a Pagar ({contasPagarFiltradas.length})</h2>
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead className="bg-gray-50">
      ...
      {tipoVisualizacao === 'CATEGORIAS' && (
        <div>
          <h2 className="text-2xl font-bold mb-4">Ã°Å¸ÂÂ·Ã¯Â¸Â Categorias ({categorias.length})</h2>
      ...
      <div className="mt-8 flex gap-4">
        ...
        >
          Ã°Å¸Å¡â‚¬ Nova SincronizaÃ¡Â§Ã¡Â£o
        </a>
      </div>
    </div>
  )
