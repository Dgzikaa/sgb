...
export default function PrivacidePage() {
  ...
  const handleDataDeletion = async () => {
    if (!confirm('ATENÇÃO: Esta ação irá EXCLUIR PERMANENTEMENTE todos os seus dados. Esta ação é IRREVERSÍVEL. Deseja continuar?')) {
      return
    }
      ...
      <div className="container mx-auto px-4 py-6 space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="mt-1 p-3 bg-gray-50 dark:bg-gray-800 rounded border text-sm">
                          • Nome: {userData.name || 'Não informado'}<br />
          ...
          <TabsContent value="audit" className="space-y-6">
            <Card className="card-dark">
              <CardHeader>
                <CardTitle className="card-title-dark">🗄️ Histórico de Consentimentos</CardTitle>
                <p className="card-description-dark">
