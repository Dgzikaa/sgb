...
export default function DashboardFinanceiroPage() {
  ...
  const fixCategoryName = (categoryName: string) => {
    ...
    return categoryName
      ...
      .replace(/Débito/g, 'Débito')
      .replace(/Empréstimos/g, 'Empréstimos')
      .replace(/Sócios/g, 'Sócios')
      .replace(/Utensílios/g, 'Utensílios')
      .replace(/Descartáveis/g, 'Descartáveis')
      .replace(/áágua/g, 'água')
      .replace(/áGUA/g, 'áGUA')
      .replace(/Elétrica/g, 'Elétrica')
      .replace(/ENERGIA ELÉTRICA/g, 'ENERGIA ELÉTRICA')
      .replace(/Manutenção/g, 'Manutenção')
      .replace(/MANUTENÃO/g, 'MANUTENÃO')
      .replace(/AluguelCondomínio/g, 'Aluguel/Condomínio')
      .replace(/CONDOMÍNIO/g, 'CONDOMÍNIO')
      .replace(/SALÁRIOS/g, 'SALÁRIOS')
      .replace(/Décimo/g, 'Décimo')
      .replace(/Gás/g, 'Gás')
      .replace(/Escritório/g, 'Escritório')
      .replace(/AdministrativoOrdinário/g, 'Administrativo Ordinário')
      .replace(/Produção/g, 'Produção')
      .replace(/Operação/g, 'Operação')
      .replace(/Programação/g, 'Programação')
      .replace(/Atrações/g, 'Atrações')
  }
                            ...
                            <>
                              ...
                              <div className={`p-3 rounded-lg ${concentracao > 50 ? 'bg-red-50' : concentracao > 30 ? 'bg-yellow-50' : 'bg-green-50'}`}>
                                <div className={`text-xs font-medium ${concentracao > 50 ? 'text-red-700' : concentracao > 30 ? 'text-yellow-700' : 'text-green-700'}`}>
                                  {concentracao > 50 ? 'Alta dependência de uma fonte' : 
