...
export default function AdminPage() {
  ...
  const importarEventosHistoricos = async () => {
    ...
    try {
      ...
      const response1 = await fetch('/api/admin/eventos/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (result1.requer_confirmacao) {
        ...
        const response2 = await fetch('/api/admin/eventos/import', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            bar_id: barOrdinario.id,
...
function AnalyticsContent({ barId }: { barId: number }) {
  ...
  const carregarAnalytics = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/eventos/analytics?bar_id=${barId}&tipo=${tipoAnalise}`);
      const result = await response.json();
