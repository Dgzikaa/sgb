...
export async function GET(request: NextRequest) {
  try {
    ...
    const criticalEvents =
      events?.filter((e: unknown) => e.level === 'critical').length || 0;
    const warningEvents =
      events?.filter((e: unknown) => e.level === 'warning').length || 0;
    const infoEvents =
      events?.filter((e: unknown) => e.level === 'info').length || 0;
    const authEvents =
      events?.filter((e: unknown) => e.category === 'auth').length || 0;
    const accessEvents =
      events?.filter((e: unknown) => e.category === 'access').length || 0;
    const injectionEvents =
      events?.filter((e: unknown) => e.category === 'injection').length || 0;
    const rateLimitEvents =
      events?.filter((e: unknown) => e.category === 'rate_limit').length || 0;
    const apiAbuseEvents =
      events?.filter((e: unknown) => e.category === 'api_abuse').length || 0;
    const backupEvents =
      events?.filter((e: unknown) => e.category === 'backup').length || 0;
    const systemEvents =
      events?.filter((e: unknown) => e.category === 'system').length || 0;

    const uniqueIps = new Set(
      events?.map((e: unknown) => e.ip_address).filter(Boolean)
    ).size;
    const failedLogins =
      events?.filter((e: unknown) => e.event_type === 'failed_login').length ||
      0;
