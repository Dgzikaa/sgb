...
// Tipos auxiliares para posts, padrÃµes e recomendaÃ§Ãµes
interface Post {
...
interface PadroesTemporais {
  ...
  por_hora: MediaHora[];
  por_dia: MediaDia[];
  por_periodo: MediaPeriodo[];
  melhor_hora: MediaHora;
  melhor_dia: MediaDia;
  melhor_periodo: MediaPeriodo;
}

interface PadraoSazonal {
  mes: number;
  nome: string;
  engajamento_medio: number;
  alcance_medio: number;
...
export async function GET(request: NextRequest) {
  try {
    console.log('ÂÂ° OtimizaÃ¡Â§Ã¡Â£o Temporal - Analisando padrÃ¡Âµes temporais...')
    ...
    console.log('ÂÂ° OtimizaÃ¡Â§Ã¡Â£o Temporal - Analisando para bar:', barId, 'plataforma:', plataforma)
    ...
    // 2. ANALISAR PADRÃ¡â€¢ES TEMPORAIS
    const analisarPadroesTemporais = (dados: Post[], nomePlataforma: string): PadroesTemporais | null => {
      if (!dados || dados.length === 0) return null
      
      // AnÃ¡Â¡lise por hora do dia
      ...
      // AnÃ¡Â¡lise por perÃ¡Â­odo do dia
      const periodos = {
      ...
      dados.forEach(post => {
        const data = new Date(post.data_referencia as string)
        const hora = data.getHours()
        const dia = data.getDay()
        
        // Calcular engajamento baseado na plataforma
        const engajamento = nomePlataforma === 'instagram' ? 
          (post.posts_likes as number || 0) + (post.posts_comments as number || 0) :
          (post.post_likes as number || 0) + (post.post_comments as number || 0)
        
        const alcance = (post.reach as number || 0) + (post.impressions as number || 0) + (post.page_reach as number || 0)
      ...
      const mediasPorHora: MediaHora[] = engajamentoPorHora.map((eng, hora) => ({
        hora,
      ...
      const nomesDias = ['Domingo', 'Segunda', 'TerÃ¡Â§a', 'Quarta', 'Quinta', 'Sexta', 'SÃ¡Â¡bado']
      const mediasPorDia: MediaDia[] = engajamentoPorDia.map((eng, dia) => ({
        dia,
        nome: nomesDias[dia],
      ...
      const mediasPorPeriodo: MediaPeriodo[] = Object.entries(periodos).map(([nome, dados]: [string, any]) => ({
        periodo: nome,
      ...
      return {
        ...
        melhor_hora: mediasPorHora.reduce((prev: MediaHora, curr: MediaHora) => 
          prev.score_combinado > curr.score_combinado ? prev : curr
        ),
        melhor_dia: mediasPorDia.reduce((prev: MediaDia, curr: MediaDia) => 
          prev.score_combinado > curr.score_combinado ? prev : curr
        ),
        melhor_periodo: mediasPorPeriodo.reduce((prev: MediaPeriodo, curr: MediaPeriodo) => 
          prev.score_combinado > curr.score_combinado ? prev : curr
        )
      }
    }

    // 3. GERAR RECOMENDAÃ¡â€¡Ã¡â€¢ES INTELIGENTES
    const gerarRecomendacoes = (padroes: PadroesTemporais): Recomendacao[] => {
      ...
      if (padroes.melhor_hora.posts_count >= 3) {
        recomendacoes.push({
          ...
          impacto: `+${((padroes.melhor_hora.score_combinado / (padroes.por_hora.reduce((sum: number, h: MediaHora) => sum + h.score_combinado, 0) / padroes.por_hora.length) - 1) * 100).toFixed(0)}% performance`,
      ...
      if (padroes.melhor_dia.posts_count >= 3) {
        recomendacoes.push({
          ...
          impacto: `+${((padroes.melhor_dia.score_combinado / (padroes.por_dia.reduce((sum: number, d: MediaDia) => sum + d.score_combinado, 0) / padroes.por_dia.length) - 1) * 100).toFixed(0)}% performance`,
    ...
    // 4. DETECTAR PADRÃ¡â€¢ES SAZONAIS
    const detectarPadroesSazonais = (dados: Post[]): PadraoSazonal[] | null => {
      ...
      dados.forEach(post => {
        ...
        const engajamento = (post.posts_likes as number || 0) + 
                           (post.post_likes as number || 0) + 
                           (post.posts_comments as number || 0) + 
                           (post.post_comments as number || 0)
        dadosMes.engajamento += engajamento
        dadosMes.alcance += (post.reach as number || 0) + (post.impressions as number || 0) + (post.page_reach as number || 0)
        dadosMes.posts += 1
      })
      ...
      return padroesSazonais.sort((a: PadraoSazonal, b: PadraoSazonal) => a.mes - b.mes)
    }

    // 5. SUGERIR CRONOGRAMA OTIMIZADO
    const sugerirCronograma = (padroes: PadroesTemporais): CronogramaDia[] => {
      const cronograma: CronogramaDia[] = []
      const nomesDias = ['Domingo', 'Segunda', 'TerÃ¡Â§a', 'Quarta', 'Quinta', 'Sexta', 'SÃ¡Â¡bado']
      
      // Ordenar dias por performance
      const diasOrdenados = [...padroes.por_dia].sort((a: MediaDia, b: MediaDia) => b.score_combinado - a.score_combinado)
      
      // Sugerir cronograma para os prÃ¡Â³ximos 7 dias
      for (let i = 0; i < 7; i++) {
        const hoje = new Date()
        ...
        if (dadosDia.posts_count >= 2) {
          cronograma.push({
            ...
            prioridade: diasOrdenados.findIndex((d: MediaDia) => d.dia === diaAtual) + 1
          })
        }
      }
      
      return cronograma.sort((a: CronogramaDia, b: CronogramaDia) => a.prioridade - b.prioridade)
    }
    ...
    // Escolher qual padrÃ¡Â£o usar baseado na plataforma solicitada
    ...
    if (!padroesPrincipais) {
      return NextResponse.json({
        success: false,
        error: 'Dados insuficientes para anÃ¡Â¡lise temporal'
      }, { status: 400 })
    }
    ...
    const resultado = {
      ...
      insights: {
        ...
        confiabilidade: padroesPrincipais.total_posts > 50 ? 'Alta' : 
                        padroesPrincipais.total_posts > 20 ? 'MÃ¡Â©dia' : 'Baixa',
        proxima_atualizacao: 'ApÃ¡Â³s 10 novos posts',
        tendencia: 'Crescimento no perÃ¡Â­odo noturno'
      }
    }

    console.log('Å“â€¦ OtimizaÃ¡Â§Ã¡Â£o Temporal processada:', {
      plataforma: plataforma,
    ...
    console.error('ÂÅ’ Erro na OtimizaÃ¡Â§Ã¡Â£o Temporal:', error)
    return NextResponse.json({ 
      success: false, 
