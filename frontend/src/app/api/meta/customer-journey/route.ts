...
export async function GET(request: NextRequest) {
  try {
    console.log('üó∫Ô∏è Customer Journey - Mapeando jornada do cliente...')
    ...
    if (userData) {
      try {
        ...
        console.log(`üë§ Customer Journey - Usando bar_id: ${barId}`)
      } catch (e) {
        console.warn('ö†Ô∏è Erro ao parsear dados do usu·°rio, usando bar_id padr·£o')
      }
    }

    console.log('üó∫Ô∏è Customer Journey - Mapeando jornada para bar:', barId)

    // 1. COLETAR DADOS DE ENGAJAMENTO E CONVERS·ÉO
    const etapasJornada = [
      {
        id: 'discovery',
        nome: 'Descoberta',
      ...
      },
      {
        id: 'intent',
        nome: 'Inten·ß·£o',
        descricao: 'Cliente demonstra inten·ß·£o de compra',
    ...
    // 3. SIMULAR DADOS DE CONVERS·ÉO (normalmente viriam de outras fontes)
    ...
    // 5. GERAR RECOMENDA·á·ïES POR PONTO DE ABANDONO
    const gerarRecomendacoesAbandono = (etapa: string, taxaAbandono: number) => {
      const recomendacoes: any = {
        ...
        awareness: [
          ...
          'Fa·ßa parcerias com influenciadores'
        ],
        consideration: [
        ...
        intent: [
          ...
          'Ofere·ßa promo·ß·µes especiais'
        ],
        purchase: [
          'Melhore o atendimento presencial',
          'Ofere·ßa facilidades de pagamento',
        ...
        advocacy: [
          'Incentive avalia·ß·µes e reviews',
          'Recompense indica·ß·µes',
    ...
    const calcularMetricasChave = (dadosConversao: any) => {
      ...
      return {
        ...
        cac_estimado: discovery.usuarios * 2.5, // R$ 2,50 por usu·°rio alcan·ßado
    ...
    console.log('úÖ Customer Journey Map processado:', {
    ...
    console.error('ùå Erro no Customer Journey Map:', error)
