...
export async function GET(request: NextRequest) {
  try {
    console.log('ðŸŽ¯ Buscando campanhas ESPECáFICAS do Ordiná¡rio...')
    ...
    // CONTA ESPECáFICA DO ORDINáRIO que vimos no Ads Manager
    ...
    console.log(`ðŸŽ¯ Buscando campanhas da conta especá­fica: ${ordinarioAdAccountId}`)

    // 1. BUSCAR CAMPANHAS DA CONTA ORDINáRIO
    ...
    if (!campaignsResponse.ok) {
      return NextResponse.json({
        success: false,
        error: 'Erro ao buscar campanhas do Ordiná¡rio',
    ...
    console.log(`œ… Encontradas ${results.campaigns.length} campanhas do Ordiná¡rio`)

    // 2. BUSCAR INSIGHTS DAS CAMPANHAS (áºltimos 30 dias)
    if (results.campaigns.length > 0) {
      ...
      if (insightsResponse.ok) {
        ...
        results.insights = {
          error: insightsData,
          note: 'Erro ao buscar insights - pode ser problema de permissáµes'
        }
      }
    }
    ...
    console.log('ðŸ“‹ Resumo das campanhas Ordiná¡rio:', results.summary)
    ...
    console.error('Œ Erro ao buscar campanhas do Ordiná¡rio:', error)
    return NextResponse.json({ 
      success: false,
      error: 'Erro ao buscar campanhas do Ordiná¡rio',
