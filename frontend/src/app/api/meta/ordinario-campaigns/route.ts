...
export async function GET(request: NextRequest) {
  try {
    console.log('üéØ Buscando campanhas ESPEC·çFICAS do Ordin·°rio...')
    ...
    // CONTA ESPEC·çFICA DO ORDIN·ÅRIO que vimos no Ads Manager
    ...
    console.log(`üéØ Buscando campanhas da conta espec·≠fica: ${ordinarioAdAccountId}`)

    // 1. BUSCAR CAMPANHAS DA CONTA ORDIN·ÅRIO
    ...
    console.log('üìä URL das campanhas:', campaignsUrl)
    ...
    if (!campaignsResponse.ok) {
      return NextResponse.json({
        success: false,
        error: 'Erro ao buscar campanhas do Ordin·°rio',
    ...
    console.log(`úÖ Encontradas ${results.campaigns.length} campanhas do Ordin·°rio`)

    // 2. BUSCAR INSIGHTS DAS CAMPANHAS (·∫ltimos 30 dias)
    if (results.campaigns.length > 0) {
      console.log('üìà Buscando insights das campanhas...')
      ...
      if (insightsResponse.ok) {
        ...
        console.log(`üìä Insights coletados: ${results.insights.total_records} campanhas com dados`)
      } else {
        results.insights = {
          error: insightsData,
          note: 'Erro ao buscar insights - pode ser problema de permiss·µes'
        }
      }
    }
    ...
    console.log('üìã Resumo das campanhas Ordin·°rio:', results.summary)
