...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('✅ Configuração encontrada:', {
      ...
      instagram_id: configs.instagram_account_id,
      ativo: config.ativo
    })
    ...
    const facebookResponse = await fetch(
      `https://graph.facebook.com/v18.0/${configs.page_id}?fields=id,name,followers_count,fan_count&access_token=${config.access_token}`
    )
    ...
    const facebookInsightsResponse = await fetch(
      `https://graph.facebook.com/v18.0/${configs.page_id}/insights?metric=page_followers,page_impressions&period=day&access_token=${config.access_token}`
    )
    ...
    const instagramResponse = await fetch(
      `https://graph.facebook.com/v18.0/${configs.instagram_account_id}?fields=id,username,followers_count,media_count&access_token=${config.access_token}`
    )
    ...
    const instagramInsightsResponse = await fetch(
      `https://graph.facebook.com/v18.0/${configs.instagram_account_id}/insights?metric=impressions,reach,profile_views&period=day&access_token=${config.access_token}`
    )
    ...
    const campaignsResponse = await fetch(
      `https://graph.facebook.com/v18.0/act_${configs.configuracoes_adicionais?.business_id || configs.business_id || 'unknown'}/campaigns?fields=id,name,status,objective&access_token=${config.access_token}`
    )
