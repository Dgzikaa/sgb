...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Ã°Å¸Â¤â€“ Configurando pgcron META - Bar: ${barId}, AÃ¡Â§Ã¡Â£o: ${action}`)
    ...
    console.error('ÂÅ’ Erro na configuraÃ¡Â§Ã¡Â£o pgcron META:', error)
...
async function configurarCronJobMeta(barId: string) {
  try {
    console.log(`Ã°Å¸â€Â§ Configurando cron job META para bar ${barId}`)
    ...
    // 3. Comando SQL que chama a API de coleta automÃ¡Â¡tica
    const comando = `
    ...
    console.log(`Å“â€¦ Cron job META criado: ${jobName}`)
    ...
    return NextResponse.json({
      ...
      jobName,
      schedule: cronExpression,
      schedule_description: 'Executa 2x por dia: 8h e 20h (manhÃ¡Â£ e noite)',
      api_calls_per_day: 10, // 2 execuÃ¡Â§Ã¡Âµes Ã¡â€” 5 calls cada
      next_runs: [
        'PrÃ¡Â³xima execuÃ¡Â§Ã¡Â£o: 8:00 (manhÃ¡Â£) e 20:00 (noite)',
...
async function verificarStatusMeta(barId: string) {
  try {
    console.log(`Ã°Å¸â€Â Verificando status do cron job META para bar ${barId}`)
    ...
    if (error) {
      return NextResponse.json({
        success: false,
        error: 'Job nÃ¡Â£o encontrado',
        hasActiveJob: false
      })
    }

    // Verificar Ã¡Âºltimas execuÃ¡Â§Ã¡Âµes
    ...
    // Verificar configuraÃ¡Â§Ã¡Â£o Meta
...
async function removerCronJobMeta(barId: string) {
  try {
    console.log(`Ã°Å¸â€”â€˜Ã¯Â¸Â Removendo cron job META para bar ${barId}`)
    ...
    console.log(`Å“â€¦ Cron job META removido: ${jobName}`)
...
async function testarColetaMeta(barId: string) {
  try {
    console.log(`Ã°Å¸Â§Âª Testando coleta META para bar ${barId}`)
...
function getNextExecutionTime() {
  ...
  // PrÃ¡Â³ximas execuÃ¡Â§Ã¡Âµes: 8h e 20h
  ...
  if (now < morning) {
    ...
    // PrÃ¡Â³xima execuÃ¡Â§Ã¡Â£o Ã¡Â© amanhÃ¡Â£ Ã¡Â s 8h
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('ÂÅ’ Erro na verificaÃ¡Â§Ã¡Â£o de status META:', error)
    return NextResponse.json({ 
      error: error instanceof Error ? error.message : 'Erro interno' 
    }, { status: 500 })
  }
