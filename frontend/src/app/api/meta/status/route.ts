  ...
  try {
    ...
    if (!credenciais) {
      return NextResponse.json({
        success: false,
        connected: false,
        error: 'Credenciais Meta nÃ¡Â£o encontradas'
      })
    }
    ...
    // 6. EstatÃ¡Â­sticas recentes (Ã¡Âºltimos 30 dias)
    ...
    // 8. InformaÃ¡Â§Ã¡Âµes das pÃ¡Â¡ginas configuradas
    let pageInfo = {}
    if (credenciais.configuracao_json) {
      try {
        ...
        console.warn('Erro ao parsear configuraÃ¡Â§Ã¡Â£o Meta:', e)
      }
    }
    ...
    console.error('ÂÅ’ Erro ao buscar status Meta:', error)
    return NextResponse.json({
      success: false,
