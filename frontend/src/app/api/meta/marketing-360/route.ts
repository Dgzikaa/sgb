...
export async function GET(request: NextRequest) {
  try {
    console.log('ðŸŽ¯ Marketing 360Â° API - Carregando dados diretamente...')
    ...
    // Calcular datas
    const hoje = new Date()
    const diasAtras = new Date(hoje.getTime() - 7 * 24 * 60 * 60 * 1000) // Ãšltimos 7 dias
    ...
    // Calcular taxa de engajamento REAL (se nÃ£o hÃ¡ impressÃµes, mostrar 0)
    const engagementRate = totalImpressions > 0 ? 
      (totalEngagement / totalImpressions * 100) : 0

    // Calcular variaÃ§Ãµes (comparar com dados de ontem)
    ...
    // ROI baseado apenas em dados reais (se nÃ£o hÃ¡ alcance, mostrar 0)
    ...
    if (campaignsData && campaignsData.length > 0) {
      ...
      campaignMetrics = {
        ...
        conversion_rate: 0 // Calcular se tiver dados de conversÃ£o
      }
    }
    ...
    const responseData = {
      ...
      data: {
        metrics: {
          ...
          weekly_reach: totalReach, // SerÃ¡ 0 se nÃ£o houver dados
          roi_estimate: roiEstimate, // SerÃ¡ 0 se nÃ£o houver dados
          facebook: {
        ...
        variations: {
          ...
          reach_change: 0, // SerÃ¡ calculado quando houver dados histÃ³ricos
          trend_direction: followersChange > 0 ? 'growing' : followersChange < 0 ? 'declining' : 'stable'
        },
        last_updated: new Date().toISOString(),
        data_source: 'real_data_only', // Indicar que sÃ£o apenas dados reais
        data_availability: {
    ...
    console.log('âœ… Marketing 360Â° - Dados processados:', {
    ...
    console.error('âŒ Erro na API Marketing 360Â°:', error)
