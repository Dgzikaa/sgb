...
export async function GET(request: NextRequest) {
  try {
    console.log('üéØ Marketing 360∞ API - Carregando dados diretamente...')
    ...
    // Calcular datas
    const hoje = new Date()
    const diasAtras = new Date(hoje.getTime() - 7 * 24 * 60 * 60 * 1000) // ·öltimos 7 dias
    ...
    // 3. PROCESSAR DADOS E CALCULAR M·âTRICAS
    ...
    // Calcular taxa de engajamento REAL (se n·£o h·° impress·µes, mostrar 0)
    const engagementRate = totalImpressions > 0 ? 
      (totalEngagement / totalImpressions * 100) : 0

    // Calcular varia·ß·µes (comparar com dados de ontem)
    ...
    console.log(`üí∞ Campanhas encontradas: ${campaignsData?.length || 0}`)
    ...
    if (campaignsData && campaignsData.length > 0) {
      ...
      campaignMetrics = {
        ...
        total_spend: campaignsData.reduce((sum: any, c: any) => sum + (parseFloat(c.spend) || 0), 0),
        total_clicks: campaignsData.reduce((sum: any, c: any) => sum + (parseInt(c.clicks) || 0), 0),
    ...
    const responseData = {
      ...
      meta: {
        ...
        raw_data: {
          ...
          campaigns_sample: campaignsData?.slice(0: any, 2) || []
        }
      }
    }

    console.log('úÖ Marketing 360∞ - Dados processados:', {
    ...
    console.error('ùå Erro na API Marketing 360∞:', error)
