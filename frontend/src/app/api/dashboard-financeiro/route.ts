...
async function buscarTodosEventosFinanceiros(supabase: any, barId: number) {
  ...
  while (true) {
    console.log(`üîÑ Buscando p·°gina ${pagina + 1} dos eventos financeiros...`)
    ...
    if (error) {
      console.error(`ùå Erro na p·°gina ${pagina + 1}:`, error)
      throw error
    }

    if (!eventosPagina || eventosPagina.length === 0) {
      console.log(`úÖ Busca finalizada. Total de eventos: ${todosEventos.length}`)
      break
    }

    todosEventos.push(...eventosPagina)
    console.log(`üìÑ P·°gina ${pagina + 1}: ${eventosPagina.length} eventos (Total acumulado: ${todosEventos.length})`)

    // Se a p·°gina retornou menos que o limite, ·© a ·∫ltima p·°gina
    if (eventosPagina.length < LIMITE_POR_PAGINA) {
      console.log(`úÖ ·öltima p·°gina atingida. Total final: ${todosEventos.length} eventos`)
      break
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`üí∞ BUSCANDO DADOS FINANCEIROS PARA BAR ${barId} - 2025 (COM PAGINA·á·ÉO)`)

    // 1. BUSCAR TODOS OS EVENTOS COM PAGINA·á·ÉO
    ...
    // 7. TRANSA·á·ïES RECENTES (primeiras 20)
    ...
    console.log(`üí∞ Dados financeiros carregados (TODOS OS DADOS):`)
    console.log(`   üìä Total de eventos processados: ${todosEventos.length}`)
    console.log(`   üìà Receitas: R$ ${totalReceitas.toFixed(2)}`)
    console.log(`   üìâ Despesas: R$ ${totalDespesas.toFixed(2)}`)
    console.log(`   üí∞ Saldo: R$ ${saldoLiquido.toFixed(2)}`)
