...
// FunÃÂ¡ÃÂ§ÃÂ¡ÃÂ£o utilitÃÂ¡ÃÂ¡ria para pegar a Service Account do env
function getGoogleServiceAccount() {
  const json = process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
  if (!json) throw new Error('GOOGLE_SERVICE_ACCOUNT_JSON nÃÂ¡ÃÂ£o definida');
...
interface InsumoRow {
  [key: string]: unknown;
  'CÃÂ¡ÃÂ³digo'?: string;
  'codigo'?: string;
  'Nome'?: string;
  'nome'?: string;
  'Unidade'?: string;
  'unidade'?: string;
  'Custo UnitÃÂ¡ÃÂ¡rio'?: number;
...
export async function POST(req: NextRequest) {
  try {
    // 1. Ler variÃÂ¡ÃÂ¡veis de ambiente
    ...
    if (!insumosSheet || !receitasSheet) {
      throw new Error('NÃÂ¡ÃÂ£o foi possÃÂ¡ÃÂ­vel localizar as abas "Base - Preparos - CMV" ou "Lista - Preparos" na planilha.');
    }
    ...
    const insumos = (insumosRows as InsumoRow[]).map((row: InsumoRow) => ({
      codigo: row['CÃÂ¡ÃÂ³digo'] || row['codigo'],
      nome: row['Nome'] || row['nome'],
      unidade: row['Unidade'] || row['unidade'],
      custo_unitario: row['Custo UnitÃÂ¡ÃÂ¡rio'] || row['custo_unitario'],
      grupo: row['Grupo'] || row['grupo'],
      // ...adicione outros campos conforme necessÃÂ¡ÃÂ¡rio
    }));
