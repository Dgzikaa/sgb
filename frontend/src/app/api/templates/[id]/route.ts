...
export async function GET(
  ...
  try {
    ...
    const estatisticas = {
      ...
      usos_completados: statsData?.filter((item: any) => item.status === 'completado').length || 0,
      usos_em_andamento: statsData?.filter((item: any) => item.status === 'em_andamento').length || 0,
