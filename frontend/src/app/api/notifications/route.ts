...
export async function POST(request: NextRequest) {
  try {
    ...
    if (modo === 'template') {
      ...
      if (error) {
        console.error('Erro ao criar notifica·ß·£o via template:', error)
        return NextResponse.json({ 
          error: 'Erro ao criar notifica·ß·£o via template',
      ...
      return NextResponse.json({
        success: true,
        message: 'Notifica·ß·£o criada via template',
        notificacao_id: notificacao
      })

    } else {
      // Criar notifica·ß·£o direta
      const data = CriarNotificacaoSchema.parse(body)
      
      // Validar permiss·µes baseadas no m·≥dulo
      const permiteAcesso = validarPermissaoModulo(user.role, data.modulo)
      if (!permiteAcesso) {
        return NextResponse.json({ 
          error: 'Sem permiss·£o para criar notifica·ß·µes neste m·≥dulo' 
        }, { status: 403 })
      }

      // Verificar duplica·ß·£o se especificado
      if (data.chave_duplicacao) {
        ...
        if (existente) {
          return NextResponse.json({
            success: true,
            message: 'Notifica·ß·£o j·° existe (duplica·ß·£o evitada)',
      ...
      if (createError) {
        console.error('Erro ao criar notifica·ß·£o:', createError)
        return NextResponse.json({ 
      ...
      console.log(`üì¢ Notifica·ß·£o criada: ${data.modulo}/${data.categoria} - ${data.titulo}`)
    ...
    console.error('Erro na API de criar notifica·ß·£o:', error)
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (data.usuario_id) {
      ...
      // Mostrar todas as notifica·ß·µes do bar (temporariamente)
    ...
    const notificacoesTransformadas = (notificacoes || []).map((notificacao: any) => {
      ...
      return {
        ...
        titulo: notificacao.titulo || 'Notifica·ß·£o',
    ...
    console.error('Erro na API de listar notifica·ß·µes:', error)
...
async function processarEnvioBrowser(supabase: any, notificacao: any) {
  try {
    ...
    console.log(`üì± Notifica·ß·£o enviada via browser: ${notificacao.id}`)
...
// =====================================================
// FUN·á·ïES ESPEC·çFICAS PARA CHECKLISTS
// =====================================================
...
async function criarNotificacaoChecklist(
  ...
  try {
    ...
    if (error) {
      console.error('Erro ao criar notifica·ß·£o de checklist:', error)
      return null
    }

    console.log(`üìã Notifica·ß·£o de checklist criada: ${categoria}`)
    return notificacaoId

  } catch (error) {
    console.error('Erro na fun·ß·£o criarNotificacaoChecklist:', error)
    return null
  }
}
