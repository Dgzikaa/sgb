...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('ÂÅ’ Erro na API de metas:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!categoria || !nome_meta || !tipo_valor) {
      return NextResponse.json(
        { error: 'Categoria, nome da meta e tipo do valor sÃ¡Â£o obrigatÃ¡Â³rios' },
        { status: 400 }
      );
    }
    ...
    if (error) {
      console.error('ÂÅ’ Erro ao criar meta:', error);
      return NextResponse.json({ error: 'Erro ao criar meta' }, { status: 500 });
    }

    console.log(`Å“â€¦ Meta criada: ${nome_meta}`);
    return NextResponse.json({
      success: true,
      data: novaMeta,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!Array.isArray(metas)) {
      return NextResponse.json(
        { error: 'Formato invÃ¡Â¡lido: esperado array de metas' },
        { status: 400 }
      );
    }
    ...
    for (const meta of metas) {
      ...
      if (error) {
        console.error(`ÂÅ’ Erro ao atualizar meta ${meta.id}:`, error);
        continue;
      }
    ...
    console.log(`Å“â€¦ ${metasAtualizadas.length} metas atualizadas`);
    return NextResponse.json({
      success: true,
    ...
    console.error('ÂÅ’ Erro ao atualizar metas:', error);
    return NextResponse.json({ error: 'Erro interno do servidor' }, { status: 500 });
  }
} 
