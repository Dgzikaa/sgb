...
function normalizarTelefone(telefone: string): string {
  ...
  if (numeros.length >= 10) {
    console.log(
      `📱 Normalizando: "${telefone}" -> "${numeros}" (${numeros.length} dígitos)`
    );
  }
  ...
  if (numeros.length === 13 && numeros.startsWith('55')) {
    ...
    console.log(
      `📱 Removendo código 55 (13 dígitos): ${numeros} -> ${resultado}`
    );
    return resultado;
  }
  ...
  if (numeros.length === 12 && numeros.startsWith('55')) {
    const resultado = numeros.substring(2);
    console.log(
      `📱 Removendo código 55 (12 dígitos): ${numeros} -> ${resultado}`
    );
    return resultado;
  }
  ...
  if (numeros.length === 14 && numeros.startsWith('5511')) {
    ...
    console.log(
      `📱 Removendo código 5511 (14 dígitos): ${numeros} -> ${resultado}`
    );
    return resultado;
  }
  ...
  if (
    numeros.length === 13 &&
    (numeros.startsWith('5561') || numeros.startsWith('5511'))
  ) {
    const resultado = numeros.substring(4);
    console.log(
      `📱 Removendo código 55XX (13 dígitos): ${numeros} -> ${resultado}`
    );
    return resultado;
  }
  ...
  if (numeros.length === 9 && numeros.startsWith('9')) {
    ...
    console.log(
      `📱 Adicionando DDD 61 para celular: ${numeros} -> ${resultado}`
    );
    return resultado;
  }
  ...
  console.log(
    `⚠️ Telefone não normalizado: ${telefone} -> ${numeros} (${numeros.length} dígitos)`
  );
  return numeros;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (errorTelefones || errorGetin) {
      console.error('❌ Erro ao buscar telefones:', {
        errorTelefones,
        errorGetin,
      });
      return NextResponse.json(
        { error: 'Erro ao buscar dados de telefone' },
        { status: 500 }
      );
    }
    ...
    for (const [telefoneNormalizado, dadosTelefone] of telefoneMap.entries()) {
      if (debugCount < 5) {
        console.log(
          `📱 Debug telefone ${debugCount + 1}: ${telefoneNormalizado} -> ${formatarTelefone(telefoneNormalizado)} (${dadosTelefone.nome})`
        );
        debugCount++;
      }
    }
    ...
    for (const [, dadosTelefone] of telefoneMap.entries()) {
      ...
      const { data: clientesPorTelefone } = await supabase
        ...
        .select('id')
        .or(
          `telefone.eq.${dadosTelefone.telefone},telefone.eq.${formatarTelefone(dadosTelefone.telefone)},telefone.ilike.%${dadosTelefone.telefone}%`
        )
        .eq('bar_id', barId);
      ...
      const visitasUnicas = visitasCliente
        .filter((visita: unknown, index: number, array: unknown[]) => {
          ...
          return (
            array.findIndex(
              (v: unknown) => `${v.cliente_id}-${v.data_visita}` === key
            ) === index
          );
        })
        .sort(
          (a: unknown, b: unknown) =>
            new Date(a.data_visita).getTime() -
            new Date(b.data_visita).getTime()
        )
      ...
      const valorTotal = visitasUnicas.reduce(
        (sum: number, v: unknown) => sum + parseFloat(v.valor_gasto || 0),
        0
      );
    ...
    let clientesFiltrados = clientesTelefone.filter(
      cliente => cliente.total_visitas >= minVisitas
    );

    // Aplicar filtros adicionais
    if (statusAtividade && statusAtividade !== 'todos') {
      clientesFiltrados = clientesFiltrados.filter(
        cliente => cliente.status_atividade === statusAtividade
      );
    }
    if (categoriaRecorrencia && categoriaRecorrencia !== 'todos') {
      clientesFiltrados = clientesFiltrados.filter(
        cliente => cliente.categoria_recorrencia === categoriaRecorrencia
      );
    }
    ...
    if (eventoId) {
      ...
      if (!errorEvento && visitasEvento) {
        ...
        clientesEvento = visitasEvento.map((visita: unknown) => ({
          ...
          categoria_recorrencia:
            visita.clientes.total_visitas >= 10
              ? 'VIP'
              : visita.clientes.total_visitas >= 5
                ? 'Frequente'
                : visita.clientes.total_visitas >= 2
                  ? 'Regular'
                  : 'Novo',
        }));
      }
    ...
    if (generoMusical && generoMusical !== 'todos') {
      ...
      if (!errorGenero && eventosGenero?.length > 0) {
        ...
        const { data: visitasGenero, error: errorVisitasGenero } =
          await supabase
            .from('cliente_visitas')
            .select(
              `
        ...
        if (!errorVisitasGenero && visitasGenero) {
          ...
          const telefonesFiltrados = new Set(
            clientesGenero.map((c: unknown) => c.telefone)
          );
    ...
    const clientesVIP = clientesFiltrados.filter(
      (c: unknown) => c.categoria_recorrencia === 'VIP'
    ).length;
    const clientesFrequentes = clientesFiltrados.filter(
      (c: unknown) => c.categoria_recorrencia === 'Frequente'
    ).length;
    const clientesAtivos = clientesFiltrados.filter(
      (c: unknown) => c.status_atividade === 'Ativo'
    ).length;
    const receitaTotal = clientesFiltrados.reduce(
      (sum: number, c: unknown) => sum + parseFloat(c.valor_total_gasto || 0),
      0
    );
    const ticketMedio = totalClientes > 0 ? receitaTotal / totalClientes : 0;

    // Top clientes para diferentes métricas
    const topPorVisitas = clientesFiltrados
      .slice(0, 10)
      .map((cliente: unknown) => ({
        ...cliente,
    ...
    console.log(
      '🏆 TOP POR VISITAS (primeiros 3):',
      topPorVisitas.slice(0, 3).map((c: unknown) => ({
        nome: c.nome,
        telefone: c.telefone,
    ...
    const topPorTicket = [...clientesFiltrados]
      .sort(
        (a: unknown, b: unknown) =>
          parseFloat(b.ticket_medio || 0) - parseFloat(a.ticket_medio || 0)
      )
      .slice(0, 10)
      .map((cliente: unknown) => ({
        ...cliente,
    ...
    console.log(
      '💰 TOP POR TICKET MÉDIO (primeiros 3):',
      topPorTicket.slice(0, 3).map((c: unknown) => ({
        nome: c.nome,
        telefone: c.telefone,
        total_visitas: c.total_visitas,
        ticket_medio: c.ticket_medio,
    ...
    const topPorReceita = [...clientesFiltrados]
      .sort(
        (a: unknown, b: unknown) =>
          parseFloat(b.valor_total_gasto || 0) -
          parseFloat(a.valor_total_gasto || 0)
      )
      .slice(0, 10)
      .map((cliente: unknown) => ({
        ...cliente,
    ...
    console.log(
      '💎 TOP POR FATURAMENTO (primeiros 3):',
      topPorReceita.slice(0, 3).map((c: unknown) => ({
        nome: c.nome,
        telefone: c.telefone,
        total_visitas: c.total_visitas,
        ticket_medio: c.ticket_medio,
    ...
    const clientesCampanha = clientesFiltrados
      .filter(
        (c: unknown) =>
          c.total_visitas >= 4 &&
          (c.telefone_normalizado || c.telefone) &&
          (c.telefone_normalizado || c.telefone).length >= 10
      )
      .map((cliente: unknown) => ({
        ...cliente,
        telefone: formatarTelefone(
