...
// ========================================
// Ã°Å¸â€œÅ  GET /api/ai/dashboard (Dashboard Executivo)
// ========================================
export async function GET(request: NextRequest) {
  try {
    ...
    if (!['funcionario', 'financeiro', 'admin'].includes(permissao)) {
      return NextResponse.json({ error: 'Sem permissÃ¡Â£o para acessar dashboard de IA' }, { status: 403 });
    }
    ...
    // ========================================
    // Ã°Å¸â€Â¢ KPIs PRINCIPAIS
    // ========================================
    const kpisPromises = [
      // Taxa de conclusÃ¡Â£o de checklists
      supabase
        .from('ai_metrics')
      ...
      // Tempo mÃ¡Â©dio de execuÃ¡Â§Ã¡Â£o
      supabase
        .from('ai_metrics')
        .select('valor, meta_valor, variacao_percentual, performance')
        .eq('bar_id', bar_id)
    ...
    // ========================================
    // Ã°Å¸â€œË† TENDÃ¡Å NCIAS DE MÃ¡â€°TRICAS
    // ========================================
    ...
    // ========================================
    // Ã°Å¸â€™Â¡ RECOMENDAÃ¡â€¡Ã¡â€¢ES RESUMO
    // ========================================
    ...
    // ========================================
    // Ã°Å¸â€œÅ  SCORE GERAL DE SAÃ¡Å¡DE
    // ========================================
    const scoresSaude = [];
    ...
    const resumoExecutivo = {
      ...
      status_geral: scoreSaudeGeral >= 90 ? 'excelente' : 
         ...
         ...(anomaliasResumo && anomaliasResumo.criticas_ativas > 0 ? [`${anomaliasResumo.criticas_ativas} anomalias crÃ¡Â­ticas ativas`] : []),
         ...(insightsResumo && insightsResumo.criticos_pendentes > 0 ? [`${insightsResumo.criticos_pendentes} insights crÃ¡Â­ticos pendentes`] : []),
         ...(!agenteStatus.ativo ? ['Agente IA desativado'] : []),
         ...(Object.values(kpis).filter((k) => k.performance === 'critico').map((k) => `KPI crÃ¡Â­tico detectado`))
       ],
       oportunidades: [
         ...(recomendacoesResumo && recomendacoesResumo.alta_prioridade > 0 ? [`${recomendacoesResumo.alta_prioridade} recomendaÃ¡Â§Ã¡Âµes de alta prioridade`] : []),
