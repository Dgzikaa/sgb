...
export async function GET(request: NextRequest) {
  try {
    ...
    if (processedParams.periodo_dias) processedParams.periodo_dias = parseInt(processedParams.periodo_dias as string);
    ...
    if (filters.incluir_insights) {
      ...
      insightsResumo = {
        ...
        por_status: totalInsights.data?.reduce((acc: Record<string, number>, i) => {
    ...
    const agenteStatus: AgenteStatus = {
      ...
      sucesso_rate: logsRecentes.data?.length ? 
        (logsRecentes.data?.filter((l: { status: string }) => l.status === 'concluido').length || 0) / (logsRecentes.data?.length || 1) * 100 : 0,
