...
const QuerySchema = z.object({
  query_type: z.enum([
    // Ã°Å¸â€™Â° Financeiro & Faturamento
    'maior_faturamento',
    'faturamento_periodo',
    'comparativo_mensal',
    'top_clientes',
    ...
    // Ã°Å¸â€œÂ± WhatsApp & ComunicaÃ¡Â§Ã¡Â£o
    'whatsapp_stats',
    'engagement_whatsapp',
    'mensagens_pendentes',
    'campanhas_ativas',
    
    // Ã°Å¸Ââ€¢ ProduÃ¡Â§Ã¡Â£o & Tempo
    'tempo_producao',
    'produtos_demorados',
    'eficiencia_cozinha',
    'picos_movimento',
    
    // Ã°Å¸Â¤â€“ IA & Analytics
    'anomalias_recentes',
    ...
    // Ã°Å¸â€˜Â¥ Equipe & UsuÃ¡Â¡rios
    'atividade_usuarios',
    'usuarios_ativos',
    'perfil_funcionarios',
    
    // Ã°Å¸â€œÅ  Dashboards & VisÃ¡Â£o Geral
    'dashboard_executivo',
    'visao_360',
...
// ========================================
// Ã°Å¸Â¤â€“ POST /api/ai/query (Consultas inteligentes)
// ========================================
export async function POST(request: NextRequest) {
  try {
    ...
    switch (query_type) {
      ...
      default:
        return NextResponse.json({ error: 'Tipo de consulta nÃ¡Â£o suportado' }, { status: 400 });
    }
...
// ========================================
// Ã°Å¸â€™Â° CONSULTAS DE FATURAMENTO
// ========================================
...
/**
 * Faturamento em perÃ¡Â­odo especÃ¡Â­fico
 */
async function getFaturamentoPeriodo(bar_id: number, inicio?: string, fim?: string) {
  ...
  if (!pagamentos || pagamentos.length === 0) {
    return {
      ...
      faturamento_total: 0,
      transacoes: 0,
      mensagem: 'Nenhum faturamento encontrado no perÃ¡Â­odo'
    };
  }
  ...
  return {
    ...
    mensagem: `Faturamento de R$ ${faturamentoTotal.toFixed(2)} em ${pagamentos.length} transaÃ¡Â§Ã¡Âµes no perÃ¡Â­odo`
  };
}
...
async function getComparativoMensal(bar_id: number) {
  ...
  return {
    ...
    comparativo: {
      ...
      status: variacao > 0 ? 'crescimento' : variacao < 0 ? 'queda' : 'estÃ¡Â¡vel'
    },
    mensagem: `${variacao > 0 ? 'Ã°Å¸â€œË†' : variacao < 0 ? 'Ã°Å¸â€œâ€°' : 'Å¾Â¡Ã¯Â¸Â'} ${Math.abs(variacao).toFixed(1)}% em relaÃ¡Â§Ã¡Â£o ao mÃ¡Âªs anterior`
  };
}
 ...
 */
async function getTopClientes(bar_id: number, limite: number, inicio?: string, fim?: string) {
 ...
 */
async function getProdutosMaisVendidos(bar_id: number, limite: number, inicio?: string, fim?: string) {
  ...
  produtos.forEach((produto: any) => {
    ...
    produtosAgrupados[nome].quantidade_total += produto.itm_qtd || 0;
    produtosAgrupados[nome].valor_total += produto.valor_total || 0;
  });
...
async function getPerformancePeriodo(bar_id: number, inicio?: string, fim?: string) {
  ...
  return {
    ...
    mensagem: `Performance do perÃ¡Â­odo: R$ ${faturamentoTotal.toFixed(2)} em ${pagamentos.data?.length || 0} transaÃ¡Â§Ã¡Âµes`
  };
}
...
async function getResumoMes(bar_id: number, mes?: string) {
  ...
  return {
    ...
    faturamento_por_dia: faturamentoPorDia,
    mensagem: `Resumo do mÃ¡Âªs ${mesReferencia}: R$ ${faturamento.toFixed(2)} em ${diasAtivos} dias ativos`
  };
}
