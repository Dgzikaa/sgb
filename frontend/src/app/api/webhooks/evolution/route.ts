...
async function checkChecklistResponse(phoneNumber: string, message: string) {
  try {
    ...
    const codigoMatch = messageClean.match(
      /(?:ok|pronto|feito|concluido|concluído|finalizado)\s+([a-f0-9]{8})/i
    );
    ...
    if (codigoMatch) {
      ...
      for (const agendamento of agendamentos || []) {
        const responsaveis =
          agendamento.checklist_schedules?.responsaveis_whatsapp || [];
        ...
        if (isResponsavel) {
          ...
          console.log(
            `✅ Checklist específico concluído via código ${codigo}: ${agendamento.checklist_schedules?.titulo}`
          );

          // Enviar confirmação
          await sendConfirmationMessage(
            phoneNumber,
            agendamento.checklist_schedules?.titulo || 'Checklist',
            codigo
          );
          return;
        }
    ...
    if (isCompletion) {
      ...
      if (meusAgendamentos.length === 1) {
        ...
        console.log(
          `✅ Checklist único concluído: ${agendamento.checklist_schedules?.titulo}`
        );
        await sendConfirmationMessage(
          phoneNumber,
          agendamento.checklist_schedules?.titulo || 'Checklist'
...
async function sendMultipleChecklistsMessage(
  phoneNumber: string,
  agendamentos: unknown[]
) {
  const checklistsList = agendamentos
    .map(ag => {
