...
export async function POST(
  ...
  try {
    ...
    const estruturaRespostas = gerarEstruturaRespostas(checklistData.estrutura as ChecklistEstrutura)
    ...
    console.log(`✅ Execução iniciada: ${checklistData.nome} por ${funcionarioData.nome}`)
...
export async function GET(
  ...
  try {
    ...
    const estatisticas = calcularEstatisticas(stats as ExecucaoParaEstatisticas[] || [])
...
function gerarEstruturaRespostas(estruturaChecklist: ChecklistEstrutura): Record<string, unknown> {
...
function calcularEstatisticas(execucoes: ExecucaoParaEstatisticas[]): ExecucaoEstatisticas {
  const total = execucoes.length
  const completadas = execucoes.filter(e => e.status === 'completado').length
  const em_andamento = execucoes.filter(e => e.status === 'em_andamento').length
  ...
  const tempo_medio = tempos.length > 0 ? tempos.reduce((a, b) => a + b, 0) / tempos.length : 0
