...
export async function GET(
  ...
  try {
    ...
    const estatisticas = {
      ...
      execucoes_completadas: statsExecucoes?.filter((e: any) => e.status === 'completado').length || 0,
      execucoes_pendentes: statsExecucoes?.filter((e: any) => e.status === 'em_andamento').length || 0,
    ...
    console.error('Erro na API de checklist GET:', error)
    return NextResponse.json({ 
...
export async function PUT(
  ...
  try {
    ...
    const mudancasDetectadas = detectarMudancas(checklistExistente, data)
    ...
    console.error('Erro na API de checklist PUT:', error)
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Erro na API de checklist DELETE:', error)
    return NextResponse.json({ 
      error: 'Erro interno do servidor',
      details: error.message 
    }, { status: 500 })
...
function detectarMudancas(original: any, updates: any) {
  ...
  if (updates.estrutura) {
    const mudancasEstrutura = detectarMudancasEstrutura(original.estrutura, updates.estrutura)
    if (mudancasEstrutura.length > 0) {
      mudancas.push(...mudancasEstrutura)
      tipoMudanca = 'edicao_estrutural'
    }
  }
...
function detectarMudancasEstrutura(original: any, updated: any) {
  const mudancas: string[] = []
  ...
  secoesAtualizadas.forEach((secaoAtual: any, index: number) => {
    ...
    itensAtualizados.forEach((itemAtual: any, itemIndex: number) => {
