...
const AgendamentoSchema = z.object({
  ...
  horario: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Horário deve estar no formato HH:MM'),
  ...
  responsaveis_whatsapp: z.array(z.object({
    nome: z.string(),
    numero: z.string().regex(/^\+?[1-9]\d{1,14}$/, 'Número WhatsApp inválido'),
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log da criação
    console.log(`✅ Agendamento criado: ${novoAgendamento.titulo} para checklist ${checklist.nome}`)
...
function calcularProximaExecucao(agendamento: any): string | null {
  ...
  const [hora, minuto] = (agendamento.horario || '00:00').split(':').map(Number)
  ...
  switch (agendamento.frequencia) {
    ...
    case 'semanal': {
      const diasSemana = Array.isArray(agendamento.dias_semana) ? agendamento.dias_semana : []
    ...
    case 'quinzenal': {
      const diasQuinzenal = Array.isArray(agendamento.dias_semana) ? agendamento.dias_semana : []
      let encontrou = false
      let tentativas = 0
