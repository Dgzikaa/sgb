...
const criarTabelas = async (supabase: any) => {
  ...
  if (error) {
    console.error('Œ Erro ao criar tabelas produtos/receitas:', error)
    throw error
  }
  
  console.log('œ… Tabelas produtos/receitas criadas/verificadas')
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Œ Erro ao buscar produtos:', error)
    ...
    let produtosComReceitas = produtos || []
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!codigo || !nome) {
      return NextResponse.json({
        success: false,
        error: 'Campos obrigatá³rios: codigo, nome'
      }, { status: 400 })
    }
    ...
    // Verificar se cá³digo já¡ existe
    ...
    if (existente) {
      return NextResponse.json({
        success: false,
        error: `Cá³digo ${codigo} já¡ existe`
      }, { status: 400 })
    }
    ...
    if (insumosInvalidos.length > 0) {
      return NextResponse.json({
        success: false,
        error: `Insumos ná£o encontrados: ${insumosInvalidos.join(', ')}`
      }, { status: 400 })
    }
    ...
    if (erroProduto) {
      console.error('Œ Erro ao cadastrar produto:', erroProduto)
      return NextResponse.json({ 
        success: false, 
    ...
    if (erroReceitas) {
      ...
      console.error('Œ Erro ao cadastrar receitas:', erroReceitas)
      return NextResponse.json({ 
        success: false, 
    ...
    console.log(`œ… Produto cadastrado: ${codigo} com ${receita.length} insumos`)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!codigo) {
      return NextResponse.json({
        success: false,
        error: 'Cá³digo á© obrigatá³rio para atualizaá§á£o'
      }, { status: 400 })
    }
    ...
    if (erroProduto) {
      console.error('Œ Erro ao atualizar produto:', erroProduto)
      return NextResponse.json({ 
        success: false, 
    ...
    if (receita.length > 0) {
      ...
      if (erroReceitas) {
        console.error('Œ Erro ao atualizar receitas:', erroReceitas)
        return NextResponse.json({ 
          success: false, 
