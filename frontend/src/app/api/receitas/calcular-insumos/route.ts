  ...
  try {
    ...
    if (!produto_codigo || !peso_limpo_g || !bar_id) {
      return NextResponse.json({
        success: false,
        error: 'produto_codigo, peso_limpo_g e bar_id sá£o obrigatá³rios'
      }, { status: 400 })
    }
    ...
    if (produtoError || !produto) {
      console.error('Œ Erro ao buscar produto:', produtoError)
      return NextResponse.json({
        success: false,
    ...
    if (!produto.receitas || produto.receitas.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'Produto ná£o possui receitas cadastradas'
      }, { status: 404 })
    }
    ...
    console.error('Œ Erro ao calcular insumos:', error)
    return NextResponse.json({
      success: false,
