...
export async function PUT(request: NextRequest) {
  try {
    ...
    const temChefe = insumos.some((i) => i.is_chefe)
    if (!temChefe) {
      return NextResponse.json({ 
        success: false, 
    ...
    const insumoChefe = insumos.find((i) => i.is_chefe)
    console.log('🔄 Insumo chefe encontrado:', insumoChefe)
    console.log('🔄 Dados recebidos:', { receita_codigo, tipo_local, bar_id, insumos })
    
    const receitasData = insumos.map((insumo) => ({
      bar_id: bar_id,
