...
const criarTabelaInsumos = async (supabase: any) => {
  const { error } = await supabase.rpc('exec_sql', {
    sql: `
      ...
      -- Adicionar constraint se ná£o existir
      DO $$ 
      BEGIN
  ...
  if (error) {
    console.error('Œ Erro ao criar tabela insumos:', error)
    throw error
  }
  
  console.log('œ… Tabela insumos criada/verificada')
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Œ Erro ao buscar insumos:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Œ Erro ao cadastrar insumo:', error)
      return NextResponse.json({ 
        success: false, 
    ...
    console.log(`œ… Insumo cadastrado: ${codigo}`)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id) {
      return NextResponse.json({
        success: false,
        error: 'ID á© obrigatá³rio para atualizaá§á£o'
      }, { status: 400 })
    }
    ...
    if (error) {
      console.error('Œ Erro ao atualizar insumo:', error)
      return NextResponse.json({ 
        success: false, 
