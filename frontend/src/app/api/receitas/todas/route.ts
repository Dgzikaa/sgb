...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Ã°Å¸ÂÂ½Ã¯Â¸Â Buscando TODAS as receitas para dashboard - bar_id: ${barId}`)
    ...
    console.log(`Ã°Å¸ÂÂ½Ã¯Â¸Â ${todasReceitas?.length || 0} registros de receitas encontrados (incluindo inativas)`)

    // Agrupar receitas por cÃ¡Â³digo
    ...
    console.log(`Å“â€¦ ${receitasComInsumos.length} receitas processadas: ${receitasAtivas.length} ativas, ${receitasInativas.length} inativas`)
    ...
    console.error('ÂÅ’ Erro interno na API receitas/todas:', error)
    return NextResponse.json({
      success: false,
