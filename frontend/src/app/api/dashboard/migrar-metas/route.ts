  ...
  try {
    ...
    if (!bar_id || !metas_config) {
      return NextResponse.json(
        { success: false, error: 'bar_id e metas_config sÃ¡Â£o obrigatÃ¡Â³rios' },
        { status: 400 }
      )
    }

    // Verificar se jÃ¡Â¡ existe configuraÃ¡Â§Ã¡Â£o para este bar
    ...
    // Preparar dados para inserÃ¡Â§Ã¡Â£o/atualizaÃ¡Â§Ã¡Â£o
    const metasData = {
      ...
      ticket_couvert: metas_config.ticket_couvert || metas_config.ticketCouvert || 50,
      ...
      reservas_diarias: metas_config.reservas_diarias || metas_config.reservasDiarias || 133,
      reservas_semanais: metas_config.reservas_semanais || 800,
      reservas_mensais: metas_config.reservas_mensais || 3200,
      metas_por_dia: metas_config.metas_por_dia || metas_config.metasPorDia || {
    ...
    console.error('Erro na migraÃ¡Â§Ã¡Â£o de metas:', error)
    return NextResponse.json(
...
// GET - Verificar status da migraÃ¡Â§Ã¡Â£o
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Erro ao verificar migraÃ¡Â§Ã¡Â£o:', error)
    return NextResponse.json(
      { success: false, error: 'Erro interno do servidor' },
      { status: 500 }
    )
