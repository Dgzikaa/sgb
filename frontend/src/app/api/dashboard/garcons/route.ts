...
export async function GET(request: NextRequest) {
  try {
    ...
    const garcons = Array.from(garconsMap.values()).map(
      (garcom: unknown, index: number) => {
        ...
        const vendas_por_dia = Array.from(garcom.vendas_por_dia.entries())
          .map((entry: unknown) => ({
            data: entry[0],
            vendas: entry[1],
          }))
