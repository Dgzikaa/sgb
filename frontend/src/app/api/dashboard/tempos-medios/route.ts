...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(`üìä Registros de tempo encontrados: ${temposData?.length || 0}`)
      ...
      const isComida = (produto: string, grupo: string): boolean => {
        ...
        const palavrasComida = [
          ...
          'salada', 'prato', 'entrada', 'petisco', 'lanche', 'comida', 'por·ß·£o',
      ...
      const isBebida = (produto: string, grupo: string): boolean => {
        ...
        const palavrasBebida = [
          ...
          'cacha·ßa', 'rum', 'tequila', 'licor', 'espumante', 'champagne'
        ]
