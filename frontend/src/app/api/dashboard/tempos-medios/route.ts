...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('ÂÂ±Ã¯Â¸Â API Tempos MÃ¡Â©dios - ParÃ¡Â¢metros recebidos:', {
      data_inicio,
      data_fim,
      bar_id
    })
    ...
    try {
      ...
      console.log('Ã°Å¸â€œâ€¦ Buscando tempos no perÃ¡Â­odo:', {
      ...
      if (temposError) {
        console.error('ÂÅ’ Erro ao buscar dados de tempo:', temposError)
        return NextResponse.json(
      ...
      console.log(`Ã°Å¸â€œÅ  Registros de tempo encontrados: ${temposData?.length || 0}`)
      ...
      if (!temposData || temposData.length === 0) {
        return NextResponse.json({
          ...
          meta: {
            ...
            observacao: 'Nenhum dado encontrado para o perÃ¡Â­odo'
          }
        })
      }

      // FunÃ¡Â§Ã¡Â£o para classificar produtos como cozinha ou bar
      const isComida = (produto: string, grupo: string): boolean => {
        ...
        const palavrasComida = [
          ...
          'salada', 'prato', 'entrada', 'petisco', 'lanche', 'comida', 'porÃ¡Â§Ã¡Â£o',
      ...
      const isBebida = (produto: string, grupo: string): boolean => {
        ...
        const palavrasBebida = [
          ...
          'refrigerante', 'suco', 'Ã¡Â¡gua', 'cafÃ¡Â©', 'whisky', 'vodka', 'gin',
          'cachaÃ¡Â§a', 'rum', 'tequila', 'licor', 'espumante', 'champagne'
        ]
        
        const gruposBebida = [
          'bebidas', 'cervejas', 'vinhos', 'drinks', 'coquetÃ¡Â©is', 'refrigerantes',
          'sucos', 'Ã¡Â¡guas', 'cafÃ¡Â©s', 'destilados', 'licores', 'espumantes'
        ]
      ...
      // Processar dados e calcular tempos mÃ¡Â©dios
      ...
      temposData.forEach((item: any) => {
        ...
        if (isComida(produto, grupo)) {
          // Para comidas: usar t1_t2 (inÃ¡Â­cio produÃ¡Â§Ã¡Â£o atÃ¡Â© fim produÃ¡Â§Ã¡Â£o)
          ...
          // Para bebidas: usar t0_t3 (lanÃ¡Â§amento atÃ¡Â© entrega)
          ...
          if (item.t1_t2 && item.t1_t2 > 0 && item.t1_t2 <= 3600) { // max 60 min
            tempo = item.t1_t2
            isValidTempo = true
        ...
        // Adicionar tempo vÃ¡Â¡lido aos arrays correspondentes
      ...
      console.log('ÂÂ±Ã¯Â¸Â Tempos calculados:', tempos)
      ...
      return NextResponse.json({
        ...
        meta: {
          ...
          criterios: {
            comidas: 't1_t2 (inÃ¡Â­cio atÃ¡Â© fim produÃ¡Â§Ã¡Â£o), max 45min',
            bebidas: 't0_t3 (lanÃ¡Â§amento atÃ¡Â© entrega), max 20min',
      ...
      console.error('ÂÅ’ Erro ao buscar tempos do banco:', dbError)
      return NextResponse.json(
    ...
    console.error('ÂÅ’ Erro na API Tempos MÃ¡Â©dios:', error)
    return NextResponse.json(
