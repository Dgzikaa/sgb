...
export async function GET(request: NextRequest) {
  try {
    ...
    const bar_id = parseInt(searchParams.get('bar_id') || '1'); // Default para Bar OrdinÃ¡rio

    // Suporte a consulta por data Ãºnica ou perÃ­odo
    if (!data && (!data_inicio || !data_fim)) {
      return NextResponse.json(
        { success: false, error: 'Parmetros obrigatÃ³rios: data (YYYY-MM-DD) OU data_inicio + data_fim' },
        { status: 400 }
      );
    }

    const consultaPeriodo = data_inicio && data_fim;
    const dataConsulta = consultaPeriodo ? `${data_inicio} atÃ© ${data_fim}` : data;
    ...
    // CORREÃ‡Ã¡O: Buscar TODAS as reservas (nÃ¡o filtrar por status)
    ...
    // Calcular estatÃ­sticas - CORRIGIDO para incluir todos os status
