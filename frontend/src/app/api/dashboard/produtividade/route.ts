...
async function calcularMetricasGerais(supabase: SupabaseClient, barId: string, dataInicio: Date, dataFim: Date): Promise<MetricasGerais> {
  ...
  const pendentes = execucoesTyped.filter((e) => ['em_andamento', 'pausado'].includes(e.status))
  ...
  const funcionariosUnicos = new Set(execucoesTyped.map((e) => e.funcionario_id)).size

  return {
    total_execucoes: execucoesTyped.length,
    execucoes_concluidas: concluidas.length,
    execucoes_pendentes: pendentes.length,
    taxa_conclusao: execucoesTyped.length > 0 ? Math.round((concluidas.length / execucoesTyped.length) * 100) : 0,
...
async function calcularRankingFuncionarios(
  ...
  if (funcionarioIdFiltro) {
    execucoesFiltradas = execucoesTyped.filter((e) => e.funcionario_id === funcionarioIdFiltro)
  }

  if (setorFiltro) {
    execucoesFiltradas = execucoesFiltradas.filter((e) => e.funcionario?.setor === setorFiltro)
  }

  if (cargoFiltro) {
    execucoesFiltradas = execucoesFiltradas.filter((e) => e.funcionario?.cargo === cargoFiltro)
  }

  // Agrupar por funcionário
  const funcionarios = new Map<string, {
    funcionario_id: string;
    funcionario?: {
      nome: string;
      email: string;
...
async function calcularEvolucaoTemporal(supabase: SupabaseClient, barId: string, dataInicio: Date, dataFim: Date): Promise<EvolucaoTemporal[]> {
  const { data: execucoes } = await supabase
    .from('checklist_execucoes')
  ...
  // Agrupar por dia
  const evolucaoPorDia = new Map<string, {
    data: string;
    total_execucoes: number;
    execucoes_concluidas: number;
    score_total: number;
...
async function buscarAlertas(supabase: SupabaseClient, barId: string): Promise<Alerta[]> {
  ...
  if (agendamentosAtrasados && agendamentosAtrasados.length > 0) {
    const agendamentosTyped = agendamentosAtrasados as unknown as ChecklistAgendamento[]
    alertas.push({
...
async function calcularEstatisticasPorSetor(supabase: SupabaseClient, barId: string, dataInicio: Date, dataFim: Date): Promise<EstatisticaSetor[]> {
  const { data: execucoes } = await supabase
    .from('checklist_execucoes')
    .select(`
      status,
...
async function calcularEstatisticasPorCargo(supabase: SupabaseClient, barId: string, dataInicio: Date, dataFim: Date): Promise<EstatisticaCargo[]> {
  const { data: execucoes } = await supabase
    .from('checklist_execucoes')
    .select(`
      status,
...
async function buscarTopChecklists(supabase: SupabaseClient, barId: string, dataInicio: Date, dataFim: Date): Promise<TopChecklist[]> {
  const { data: execucoes } = await supabase
    .from('checklist_execucoes')
    .select(`
      checklist_id,
  ...
  const execucoesTyped = execucoes as unknown as ChecklistExecucao[]
  const checklists = new Map<string, {
    checklist_id: string;
    checklist?: {
      nome: string;
      setor: string;
