...
async function calcularMetricasGerais(
  ...
  const concluidas = execucoes.filter(
    (e: unknown) => e.status === 'completado'
  );
  const pendentes = execucoes.filter((e: unknown) =>
  ...
  const funcionariosUnicos = new Set(
    execucoes.map((e: unknown) => e.funcionario_id)
  ).size;
  ...
  return {
    ...
    taxa_conclusao:
      execucoes.length > 0
        ? Math.round((concluidas.length / execucoes.length) * 100)
        : 0,
    score_medio:
      concluidas.length > 0
...
async function calcularRankingFuncionarios(
  ...
  if (funcionarioIdFiltro) {
    execucoesFiltradas = execucoes.filter(
      (e: unknown) => e.funcionario_id === funcionarioIdFiltro
    );
  }

  if (setorFiltro) {
    execucoesFiltradas = execucoes.filter(
      (e: unknown) => e.funcionario?.setor === setorFiltro
    );
  }

  if (cargoFiltro) {
    execucoesFiltradas = execucoes.filter(
      (e: unknown) => e.funcionario?.cargo === cargoFiltro
    );
  }
