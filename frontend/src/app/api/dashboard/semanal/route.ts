...
async function getDashboardSemanalCorrigido(request: NextRequest) {
  ...
  console.log(`🔥🔥🔥 NOVA FUNÇÃO DOMINGO CORRIGIDA: ${VERSAO_DOMINGO_CORRIGIDA} 🔥🔥🔥`)
    ...
    try {
      ...
      const buscarTodosPagamentos = async () => {
        ...
        while (hasMore) {
          ...
          if (data && data.length > 0) {
            ...
            console.log(`📄 Página ${Math.floor(offset/limit) + 1}: ${data.length} registros (total: ${todosPagamentos.length})`)
      ...
      const buscarTodosYuzerBar = async () => {
        ...
        while (hasMore) {
          ...
          if (error) {
            console.error(`❌ Erro na paginação Yuzer Bar offset ${offset}:`, error)
            break
          }
      ...
      const buscarTodosYuzerIngressos = async () => {
        ...
        while (hasMore) {
          ...
          if (error) {
            console.error(`❌ Erro na paginação Yuzer Ingressos offset ${offset}:`, error)
            break
          }
      ...
      const faturamento_contahub_real = pagamentos.reduce((sum: number, item: unknown) => {
        return sum + (parseFloat(item.liquido) || 0)
      }, 0)

      // 2. Bilheteria Sympla
      const faturamento_bilheteria = sympla.reduce((sum: number, item: unknown) => {
        return sum + (parseFloat(item.total_liquido) || 0)
      }, 0)

      // 3. Yuzer Bar (faturamento bar)
      const faturamento_yuzer_bar = yuzerBar.reduce((sum: number, item: unknown) => {
        return sum + (parseFloat(item.valor_total) || 0)
      }, 0)

      // 4. Yuzer Ingressos (faturamento ingressos)
      const faturamento_yuzer_ingressos = yuzerIngresso.reduce((sum: number, item: unknown) => {
        return sum + (parseFloat(item.valor_total) || 0)
      }, 0)

      // 5. Couvert real da tabela período
      const couvert_real_periodo = periodo.reduce((sum: number, item: unknown) => {
      ...
      const periodo_com_pagamento = periodo.filter((item: unknown) => parseFloat(item.vr_pagamentos || '0') > 0)
      ...
      const pedidos_unicos_yuzer_ingresso = [...new Set(yuzerIngresso.map((y: unknown) => y.pedido_id))]
      const clientes_yuzer = pedidos_unicos_yuzer_ingresso.length

      // Buscar TODAS as visitas Sympla do período COM PAGINAÇÃO
      console.log('🔍 Buscando TODAS as visitas_clientes Sympla do período COM PAGINAÇÃO...')
      ...
      const buscarTodasVisitasSympa = async () => {
        ...
        while (hasMore) {
          ...
          if (error) {
            console.error(`❌ Erro na paginação Visitas Sympla offset ${offset}:`, error)
            break
          }
      ...
      const clientes_visitas_sympla = todasVisitasSymplaData.reduce((sum: number, item: unknown) => {
      ...
      if (clientes_pessoas_diario_total > 0) {
        ...
        console.log(`👥 Usando período com pagamento: ${clientes_contahub_periodo}`)
      }
      ...
      for (const dia of diasSemana) {
        ...
        const pagamentos_dia = pagamentos.filter((p: unknown) => p.dt_gerencial === dia.data)
        const sympla_dia = sympla.filter((s: unknown) => s.data_evento === dia.data)
        const yuzer_bar_dia = yuzerBar.filter((y: unknown) => y.data_pedido === dia.data)
        const yuzer_ingresso_dia = yuzerIngresso.filter((y: unknown) => y.data_pedido === dia.data)
        const periodo_dia = periodo.filter((p: unknown) => p.dt_gerencial === dia.data)
        const visitas_sympla_dia = todasVisitasSymplaData.filter((v: unknown) => v.data_visita === dia.data)

        const faturamento_contahub_dia = pagamentos_dia.reduce((sum: number, item: unknown) => sum + (parseFloat(item.liquido) || 0), 0)
        const faturamento_bilheteria_dia = sympla_dia.reduce((sum: number, item: unknown) => sum + (parseFloat(item.total_liquido) || 0), 0)
        const faturamento_yuzer_bar_dia = yuzer_bar_dia.reduce((sum: number, item: unknown) => sum + (parseFloat(item.valor_total) || 0), 0)
        const faturamento_yuzer_ingressos_dia = yuzer_ingresso_dia.reduce((sum: number, item: unknown) => sum + (parseFloat(item.valor_total) || 0), 0)
        const couvert_periodo_dia = periodo_dia.reduce((sum: number, item: unknown) => sum + (parseFloat(item.vr_couvert) || 0), 0)
        ...
        try {
          ...
          if (pessoasDataDia) {
            ...
            console.log(`👥 ${dia.data} - Pessoas diário corrigido: ${clientes_pessoas_diario_dia}`)
          } else {
        ...
        const periodo_com_pagamento_dia = periodo_dia.filter((item: unknown) => parseFloat(item.vr_pagamentos || '0') > 0)
        ...
        if (clientes_pessoas_diario_dia > 0) {
          ...
          console.log(`👥 ${dia.data} - Usando período com pagamento: ${clientes_contahub_dia}`)
        }

        // Yuzer ingressos do dia
        const pedidos_unicos_yuzer_ingresso_dia = [...new Set(yuzer_ingresso_dia.map((y: unknown) => y.pedido_id))]
        const clientes_yuzer_dia = pedidos_unicos_yuzer_ingresso_dia.length

        // Visitas Sympla do dia (já filtradas)
        const clientes_visitas_sympla_dia = visitas_sympla_dia.reduce((sum: number, item: unknown) => {
          return sum + (parseInt(item.pessoas_na_mesa) || 0)
        }, 0)
