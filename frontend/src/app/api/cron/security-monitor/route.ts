...
export async function GET(request: NextRequest) {
  try {
    ...
    const suspiciousPatterns = await analyzeSuspiciousPatterns(
      recentEvents || []
    );
...
async function generateSystemEvents() {
  ...
  if (hour >= 8 && hour <= 22) {
    // Horário comercial - gerar alguns eventos
...
async function calculateDailyMetrics(supabase: unknown) {
  ...
  const metrics = {
    ...
    critical_events: events.filter((e: unknown) => e.level === 'critical')
      .length,
    ...
    access_events: events.filter((e: unknown) => e.category === 'access')
      .length,
    injection_events: events.filter((e: unknown) => e.category === 'injection')
      .length,
    rate_limit_events: events.filter(
      (e: unknown) => e.category === 'rate_limit'
    ).length,
    api_abuse_events: events.filter((e: unknown) => e.category === 'api_abuse')
      .length,
    backup_events: events.filter((e: unknown) => e.category === 'backup')
      .length,
    system_events: events.filter((e: unknown) => e.category === 'system')
      .length,
    unique_ips: new Set(
      events.map((e: unknown) => e.ip_address).filter(Boolean)
    ).size,
    failed_logins: events.filter(
      (e: unknown) => e.event_type === 'failed_login'
    ).length,
    blocked_ips: events.filter((e: unknown) => e.event_type === 'ip_blocked')
      .length,
  };
