...
// FunÃ¡Â§Ã¡Â£o para criar cliente Supabase com validaÃ¡Â§Ã¡Â£o
...
export async function GET(request: NextRequest) {
  try {
    console.log('Ã°Å¸â€Â Iniciando monitoramento automÃ¡Â¡tico de seguranÃ¡Â§a...')
    ...
    // Criar cliente Supabase apenas quando necessÃ¡Â¡rio
    const supabase = getSupabaseClient()

    // 1. Verificar eventos suspeitos dos Ã¡Âºltimos 5 minutos
    ...
    // 2. AnÃ¡Â¡lise de padrÃ¡Âµes suspeitos
    ...
    // 6. Calcular e salvar mÃ¡Â©tricas diÃ¡Â¡rias
    ...
    console.log('Å“â€¦ Monitoramento de seguranÃ¡Â§a concluÃ¡Â­do:', result)
    return NextResponse.json(result)

  } catch (error) {
    console.error('ÂÅ’ Erro no monitoramento de seguranÃ¡Â§a:', error)
...
async function generateSystemEvents() {
  ...
  // Evento de verificaÃ¡Â§Ã¡Â£o de sistema
  await securityMonitor.logEvent({
    level: 'info',
    category: 'system',
  ...
  // Simular alguns eventos de sistema baseados em horÃ¡Â¡rio
  const hour = new Date().getHours()
  
  if (hour === 2) { // 2h da manhÃ¡Â£ - backup
    await securityMonitor.logEvent({
      level: 'info',
      category: 'backup',
  ...
  if (hour >= 8 && hour <= 22) { // HorÃ¡Â¡rio comercial - gerar alguns eventos
    // SimulaÃ¡Â§Ã¡Â£o de acesso normal
    await securityMonitor.logEvent({
      level: 'info',
      category: 'access',
...
async function calculateDailyMetrics(supabase: unknown) {
  ...
  if (error) {
    console.error('Erro ao calcular mÃ¡Â©tricas diÃ¡Â¡rias:', error)
    return
  }
