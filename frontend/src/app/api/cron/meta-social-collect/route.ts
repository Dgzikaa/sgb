...
// ========================================
// Ã°Å¸â€¢Â POST /api/cron/meta-social-collect
...
export async function POST(request: NextRequest) {
  ...
  try {
    console.log(`Ã°Å¸â€¢Â [${executionId}] Iniciando coleta automÃ¡tica de mÃ©tricas sociais...`)
    ...
    if (cronSecret && authHeader !== `Bearer ${cronSecret}`) {
      console.error(`ÂÅ’ [${executionId}] Acesso nÃ£o autorizado ao cron`)
    ...
    if (configError) {
      console.error(`ÂÅ’ [${executionId}] Erro ao buscar configuraÃ§Ãµes:`, configError)
      return NextResponse.json({ error: 'Database error' }, { status: 500 })
    }

    if (!configuracoes || configuracoes.length === 0) {
      console.log(`â€žÂ¹Ã¯Â¸Â [${executionId}] Nenhuma configuraÃ§Ã£o ativa encontrada`)
    ...
    console.log(`Ã°Å¸â€œÅ  [${executionId}] Encontradas ${configuracoes.length} configuraÃ§Ãµes ativas`)
    ...
    for (const config of configuracoes) {
      ...
      try {
        console.log(`Ã°Å¸â€Â [${executionId}] Processando bar ${barId}...`)

        // Verificar se Ã© hora de coletar
        const proximaColeta = new Date(config.proxima_coleta)
        
        if (proximaColeta > now) {
          console.log(`ÂÂ­Ã¯Â¸Â [${executionId}] Bar ${barId}: PrÃ³xima coleta em ${proximaColeta.toISOString()}`)
        ...
        if (config.ultima_coleta) {
          ...
          if (diffHours < (config.frequencia_coleta_horas - 0.5)) {
            console.log(`ÂÂ­Ã¯Â¸Â [${executionId}] Bar ${barId}: Coleta recente hÃ¡ ${diffHours.toFixed(1)}h`)
            resultDetail.status = 'skipped'
        ...
        if (!metaService) {
          console.error(`ÂÅ’ [${executionId}] Bar ${barId}: NÃ£o foi possÃ­vel criar serviÃ§o Meta`)
        ...
        console.log(`Ã°Å¸Å¡â‚¬ [${executionId}] Bar ${barId}: Iniciando coleta...`)

        // Executar coleta completa
        const collectResult = await metaService.collectAllMetrics()

        if (collectResult) {
          console.log(`Å“â€¦ [${executionId}] Bar ${barId}: Coleta realizada com sucesso`)
          ...
          console.error(`ÂÅ’ [${executionId}] Bar ${barId}: Falha na coleta`)
          resultDetail.status = 'failed'
    ...
    // Log final
    console.log(`Ã°Å¸ÂÂ [${executionId}] Coleta automÃ¡tica finalizada:`, {
    ...
    console.error(`ÂÅ’ [${executionId}] Erro crÃ­tico na coleta automÃ¡tica:`, error)
...
// ========================================
// Ã°Å¸â€œâ€¹ GET /api/cron/meta-social-collect
// Status das prÃ³ximas execuÃ§Ãµes agendadas
// ========================================
export async function GET(request: NextRequest) {
  try {
    console.log('Ã°Å¸â€œâ€¹ Consultando status das coletas agendadas...')
    ...
    if (error) {
      console.error('ÂÅ’ Erro ao buscar configuraÃ§Ãµes:', error)
      return NextResponse.json({ error: 'Database error' }, { status: 500 })
    }
