...
class GooglePlacesAPIBackend {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.error('ÂÅ’ GOOGLE_PLACES_API_KEY nÃ¡Â£o configurada')
    }
  }

  async searchPlace(query: string, location?: { lat: number, lng: number }) {
    if (!this.apiKey) {
      throw new Error('Google Places API key nÃ¡Â£o configurada')
    }
    ...
    try {
      ...
      console.log('Ã°Å¸â€Â Buscando lugar no Google Places:', query)
      ...
      console.log(`Å“â€¦ Encontrados ${data.results.length} resultados`)
      return data.results
  ...
  async getPlaceDetails(placeId: string): Promise<GooglePlaceDetails | null> {
    ...
    try {
      ...
      console.log('Ã°Å¸â€œâ€¹ Buscando detalhes do lugar:', placeId)
      ...
      console.log(`Å“â€¦ Detalhes obtidos - Rating: ${data.result.rating}, Reviews: ${data.result.reviews?.length || 0}`)
      return data.result
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Ã°Å¸Å’Å¸ RequisiÃ¡Â§Ã¡Â£o para buscar reviews:', { businessName, address, placeId })

    const googlePlaces = new GooglePlacesAPIBackend()

    // Verificar se a API key estÃ¡Â¡ configurada
    if (!googlePlaces['apiKey']) {
      return NextResponse.json({ 
        success: false, 
        error: 'Google Places API key nÃ¡Â£o configurada. Configure GOOGLE_PLACES_API_KEY no arquivo .env.local' 
      }, { status: 503 })
    }

    // Se jÃ¡Â¡ temos o place_id, usar diretamente
    if (placeId) {
      console.log('Ã°Å¸â€œÂ Usando Place ID fornecido:', placeId)
      ...
      if (!details) {
        return NextResponse.json({ 
          success: false, 
          error: 'Lugar nÃ¡Â£o encontrado com o Place ID fornecido' 
        }, { status: 404 })
      }

      // Buscar mais fotos (atÃ¡Â© 10) com diferentes tamanhos
      ...
      if (details && details.reviews && Array.isArray(details.reviews)) {
        const reviewsToInsert = details.reviews.map((review) => ({
          bar_id: bar_id || 1, // fallback para 1 se nÃ¡Â£o enviado
    ...
    // Caso contrÃ¡Â¡rio, buscar primeiro
    if (!businessName) {
      return NextResponse.json({ 
        success: false, 
        error: 'Nome do negÃ¡Â³cio Ã¡Â© obrigatÃ¡Â³rio' 
      }, { status: 400 })
    }

    console.log('Ã°Å¸â€Â Buscando lugar por nome:', businessName)
    ...
    console.log('Ã°Å¸Å½Â¯ Lugar selecionado:', place.name)
    ...
    if (!details) {
      return NextResponse.json({ 
        success: false, 
        error: 'NÃ¡Â£o foi possÃ¡Â­vel obter detalhes do lugar' 
      }, { status: 500 })
    }
    ...
    console.log('Å“â€¦ Reviews obtidos com sucesso!')
    ...
    console.error('ÂÅ’ Erro na API de reviews:', error)
