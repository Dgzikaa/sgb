...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: custos, error } = await query.order('data_competencia', {
      ascending: false,
    });
    ...
    return NextResponse.json({
      ...
      total_valor:
        custos?.reduce(
          (sum: number, custo: unknown) => sum + parseFloat(custo.valor || 0),
          0
        ) || 0,
    });
  } catch (error) {
  ...
  try {
    ...
    if (!bar_id || !tipo_custo_id || !data_competencia || !valor) {
      return NextResponse.json(
        {
          error:
            'Campos obrigatórios: bar_id, tipo_custo_id, data_competencia, valor',
        },
        { status: 400 }
      );
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (data_competencia !== undefined)
      updateData.data_competencia = data_competencia;
