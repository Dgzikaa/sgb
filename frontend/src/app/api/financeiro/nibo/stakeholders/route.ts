...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!['fornecedor', 'socio', 'funcionario'].includes(type)) {
      return NextResponse.json(
        { success: false, error: 'Tipo deve ser fornecedor, socio ou funcionario' },
        { status: 400 }
      )
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (query || document) {
      ...
      console.log('Buscando stakeholder:', { original: searchTerm, limpo: searchTermLimpo })
      ...
      if (localData && localData.length > 0) {
        ...
        const stakeholders = localData.map(stakeholder => ({
          ...
          type: stakeholder.tipo === 'Supplier' ? 'fornecedor' : 
                stakeholder.tipo === 'Customer' ? 'cliente' : 
                stakeholder.tipo === 'Partner' ? 'socio' : 'fornecedor',
...
async function createStakeholderInNibo(stakeholder: NiboStakeholderRequest): Promise<NiboStakeholderResponse> {
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.error_description || `Erro ${response.status} ao criar stakeholder`)
  }
  ...
  const { error: dbError } = await supabase
    ...
    .insert({
      ...
      documento_tipo: data.document?.type || (stakeholder.document.length === 11 ? 'CPF' : 'CNPJ'),
  ...
  return {
    ...
    pixKeyType: stakeholder.pixKeyType || data.bankAccountInformation?.pixKeyType
  }
}
...
async function getStakeholdersFromNibo(params: { 
  ...
  try {
    ...
    if (!params.type || params.type === 'fornecedor') {
      const suppliers = await searchStakeholdersByType('suppliers', params, headers)
      allStakeholders.push(...suppliers.map(s => ({ ...s, type: 'fornecedor' })))
    }

    // Buscar sócios
    if (!params.type || params.type === 'socio') {
      const partners = await searchStakeholdersByType('partners', params, headers)
      allStakeholders.push(...partners.map(s => ({ ...s, type: 'socio' })))
    }

    // Buscar funcionários
    if (!params.type || params.type === 'funcionario') {
      const employees = await searchStakeholdersByType('employees', params, headers)
      allStakeholders.push(...employees.map(s => ({ ...s, type: 'funcionario' })))
    }
