...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📡 Resposta do OAuth:', tokenResponse.status, tokenResponse.statusText)
    ...
    try {
      ...
      const webhookResponse = await fetch('https://cdpj.partners.bancointer.com.br/webhooks', {
        method: 'GET',
        headers: {
      ...
      console.log('📡 Resposta da API de webhooks:', webhookResponse.status, webhookResponse.statusText)
      ...
      if (webhookResponse.ok) {
        ...
        return NextResponse.json({
          success: true,
          message: 'OAuth funcionando! API de webhooks pode não estar disponível.',
          data: {
            oauth: {
              access_token: tokenData.access_token?.substring(0, 20) + '...',
              expires_in: tokenData.expires_in,
