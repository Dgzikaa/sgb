  ...
  try {
    ...
    if (!bar_id || !codigo_solicitacao) {
      return NextResponse.json(
        { success: false, error: 'bar_id e codigo_solicitacao são obrigatórios' },
        { status: 400 }
      )
    }

    console.log('🔍 Verificando status do pagamento:', { bar_id, codigo_solicitacao })
    ...
    const tokenResponse = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/inter-auth`, {
      method: 'POST',
      headers: {
    ...
    const statusResponse = await fetch(`https://cdpj.partners.bancointer.com.br/pix/v2/pagamentos/${codigo_solicitacao}`, {
      method: 'GET',
      headers: {
    ...
    console.log('📡 Resposta do status:', statusResponse.status, statusResponse.statusText)
