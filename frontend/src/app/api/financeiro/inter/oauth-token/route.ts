...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!credenciais) {
      return NextResponse.json(
        { success: false, error: 'Credenciais do Inter não encontradas para este bar' },
        { status: 400 }
      )
    }
    ...
    const authResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/inter-auth`, {
      method: 'POST',
      headers: {
    ...
    if (!authResponse.ok) {
      ...
      console.error('❌ Erro na autenticação Inter:', authResponse.status, errorText)
      return NextResponse.json(
        { success: false, error: `Falha na autenticação: ${authResponse.status}` },
        { status: 500 }
      )
    }
