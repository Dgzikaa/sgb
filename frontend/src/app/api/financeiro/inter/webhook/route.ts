...
export async function POST(request: NextRequest) {
  try {
    ...
    const { webhookUrl, tipoWebhook, bar_id, conta_corrente, access_token } = body
    ...
    if (!accessToken) {
      ...
      const tokenResponse = await fetch(`${request.nextUrl.origin}/api/financeiro/inter/oauth-token`, {
        method: 'POST',
          ...
          try {
            ...
            if (res.statusCode >= 200 && res.statusCode < 300) {
              ...
              resolve(NextResponse.json({
                success: true,
                data: data ? JSON.parse(data) : { message: 'Webhook configurado com sucesso' },
...
export async function GET(request: NextRequest) {
  try {
    ...
    const tipoWebhook = searchParams.get('tipoWebhook') as 'pix-pagamento' | 'boleto-pagamento'
    ...
    if (!tipoWebhook || !['pix-pagamento', 'boleto-pagamento'].includes(tipoWebhook)) {
      return NextResponse.json({ 
        success: false, 
    ...
    console.log('📋 Listando webhook configurado no Inter:', { bar_id, tipoWebhook })
    ...
    console.log('📡 Status da resposta do Inter:', response.status, response.statusText)

    // Se não há webhook configurado ou API não suporta GET, retornar vazio
    if (response.status === 404 || response.status === 400) {
      console.log('ℹ️ Nenhum webhook configurado para:', tipoWebhook, '(Status:', response.status, ')')
      return NextResponse.json({ 
        success: true, 
        data: null,
        tipoWebhook,
    ...
    if (!response.ok) {
      console.log('⚠️ Erro inesperado do Inter:', response.status, response.statusText)
      return NextResponse.json({ 
        success: true, 
        data: null,
        tipoWebhook,
    ...
    try {
      ...
      console.log('ℹ️ Resposta vazia do Inter (webhook configurado mas sem dados)')
