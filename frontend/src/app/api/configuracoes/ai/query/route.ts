...
export async function POST(request: NextRequest) {
  try {
    ...
    const { query_type, bar_id, periodo_inicio, periodo_fim, limite } =
    ...
    switch (query_type) {
      ...
      case 'faturamento_periodo':
        resultado = await getFaturamentoPeriodo(
          bar_id,
          periodo_inicio,
          periodo_fim
        );
      ...
      case 'performance_periodo':
        resultado = await getPerformancePeriodo(
          bar_id,
          periodo_inicio,
          periodo_fim
        );
      ...
      case 'status_checklists':
        resultado = await getStatusChecklists(
          bar_id,
          periodo_inicio,
          periodo_fim
        );
      ...
      case 'performance_funcionarios':
        resultado = await getPerformanceFuncionarios(
          bar_id,
          periodo_inicio,
          periodo_fim,
          limite
      ...
      case 'dashboard_executivo':
        resultado = await getDashboardExecutivo(
          bar_id,
          periodo_inicio,
          periodo_fim
        );
...
async function getMaiorFaturamento(bar_id: number) {
  ...
  const faturamentoDia =
    diaTotal?.reduce((total, item) => total + (item.valor_liquido || 0), 0) ||
    0;
 ...
 */
async function getFaturamentoPeriodo(
  bar_id: number,
  inicio?: string,
  fim?: string
) {
  ...
  const faturamentoTotal = pagamentos.reduce(
    (total, item) => total + (item.valor_liquido || 0),
    0
  );
  const faturamentoBruto = pagamentos.reduce(
    (total, item) => total + (item.valor_bruto || 0),
    0
  );
  ...
  const melhorDia = Object.entries(faturamentoPorDia).reduce(
    (max, [dia, valor]) =>
      (valor as number) > max.valor ? { dia, valor: valor as number } : max,
    { dia: '', valor: 0 }
  );
...
async function getComparativoMensal(bar_id: number) {
  ...
  const faturamentoAtual =
    fatAtual?.reduce((total, item) => total + (item.valor_liquido || 0), 0) ||
    0;
...
async function getTopClientes(bar_id: number, limite: number) {
  ...
  const faturamentoTotalClientes = clientes.reduce(
...
async function getProdutosMaisVendidos(bar_id: number, limite: number) {
  ...
  produtos.forEach(produto => {
    ...
    (produtosAgrupados[nome] as Record<string, unknown>).quantidade_total =
      (((produtosAgrupados[nome] as Record<string, unknown>)
        .quantidade_total as number) || 0) + (produto.itm_qtd || 0);
    (produtosAgrupados[nome] as Record<string, unknown>).valor_total =
      (((produtosAgrupados[nome] as Record<string, unknown>)
        .valor_total as number) || 0) + (produto.valor_total || 0);
  });
  ...
  const produtosOrdenados = Object.values(produtosAgrupados)
    ...
    .map((produto: any, index) => ({
      ...
      ...produto,
      preco_medio:
        (produto.quantidade_total as number) > 0
          ? (produto.valor_total as number) /
            (produto.quantidade_total as number)
          : 0,
    }));
  ...
  return {
    ...
    resumo: {
      ...
      quantidade_total: produtosOrdenados.reduce(
        (total, p) => total + (p as any).quantidade_total,
        0
      ),
      valor_total: produtosOrdenados.reduce(
        (total, p) => total + (p as any).valor_total,
        0
      ),
    },
 ...
 */
async function getPerformancePeriodo(
  bar_id: number,
  inicio?: string,
  fim?: string
) {
  ...
  const faturamentoTotal =
    pagamentos.data?.reduce(
      (total, item) => total + (item.valor_liquido || 0),
      0
    ) || 0;
  const totalItens =
    analitico.data?.reduce((total, item) => total + (item.itm_qtd || 0), 0) ||
    0;
  const tempoMedioProducao =
    tempos.data && tempos.data.length > 0
      ? tempos.data.reduce(
          (total: number, item: Record<string, unknown>) =>
            total + ((item.tempo_t0_t3 as number) || 0),
          0
        ) / (tempos.data.length || 1)
      : 0;
  ...
  return {
    ...
    performance: {
      ...
      ticket_medio:
        (pagamentos.data?.length || 0) > 0
          ? faturamentoTotal / (pagamentos.data?.length || 1)
          : 0,
    },
...
async function getResumoDia(bar_id: number, data: string) {
  ...
  // Horário de pico
  const horarioPico = fatPorHora.data?.reduce(
    (max, current) => ((current.valor || 0) > (max.valor || 0) ? current : max),
    { hora: '', valor: 0 }
  );
  ...
  return {
    ...
    resumo: {
      ...
      ticket_medio:
        (pagamentos.data?.length || 0) > 0
          ? faturamento / (pagamentos.data?.length || 1)
          : 0,
      horario_pico: horarioPico
        ? `${horarioPico.hora}h com R$ ${horarioPico.valor?.toFixed(2)}`
        : 'Não identificado',
    },
...
async function getResumoMes(bar_id: number, mes?: string) {
  ...
  const faturamento =
    pagamentos?.reduce((total, item) => total + (item.valor_liquido || 0), 0) ||
    0;
    ...
    resumo: {
      ...
      ticket_medio:
        (pagamentos?.length || 0) > 0
          ? faturamento / (pagamentos?.length || 1)
          : 0,
    },
    faturamento_por_dia: faturamentoPorDia,
...
async function getAnomaliasRecentes(bar_id: number, limite: number) {
  ...
  return {
    ...
    resumo: {
      ...
      anomalias_criticas:
        anomalias?.filter(a => a.severidade === 'critica').length || 0,
    },
...
async function getInsightsImportantes(bar_id: number, limite: number) {
  ...
  return {
    ...
    resumo: {
      ...
      insights_criticos:
        insights?.filter(i => i.impacto === 'critico').length || 0,
      insights_urgentes:
        insights?.filter(i => i.urgencia === 'alta').length || 0,
    },
