...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'get') {
      ...
      const results: Array<{webhook: string, success: boolean, error?: string}> = []
      ...
      for (const webhook of webhooks) {
        if (webhook.webhook_url && webhook.enabled) {
          ...
          if (upsertError) {
            console.error(`❌ Erro ao salvar webhook ${webhook.id}:`, upsertError)
            results.push({ 
              webhook: webhook.id, 
              success: false, 
          ...
          if (updateError) {
            console.error(`❌ Erro ao desabilitar webhook ${webhook.id}:`, updateError)
            results.push({ 
              webhook: webhook.id, 
              success: false, 
