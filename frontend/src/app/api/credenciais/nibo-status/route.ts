...
async function getNiboStats(barId: string) {
  try {
    ...
    const [
      ...
      supabase.from('nibo_stakeholders').select('*', { count: 'exact', head: true }).eq('bar_id', barId),
      supabase.from('nibo_agendamentos').select('*', { count: 'exact', head: true }).eq('bar_id', barId),
      supabase.from('nibo_categorias').select('*', { count: 'exact', head: true }).eq('bar_id', barId),
      supabase.from('nibo_usuarios').select('*', { count: 'exact', head: true }).eq('bar_id', barId),
      supabase.from('nibo_contas_bancarias').select('*', { count: 'exact', head: true }).eq('bar_id', barId)
    ])
