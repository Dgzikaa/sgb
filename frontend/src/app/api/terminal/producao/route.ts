  ...
  try {
    ...
    for (const receita of receitas) {
      ...
      if (
        ...
        (quantidade === maiorQuantidade && score > 0)
      ) {
    ...
    const desvio =
      rendimentoEsperado > 0 ? rendimentoProduzido / rendimentoEsperado : 0;
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (data && data.length > 0) {
      const tempos = data
        .map((p: unknown) => p.tempo_producao_segundos)
        .filter((t: number) => t > 0);
      const performances = data
        .map((p: unknown) => p.performance_percentual)
        .filter((p: number) => p > 0);
      ...
      estatisticas.performance_media =
        performances.length > 0
          ? Math.round(
              (performances.reduce((a: number, b: number) => a + b, 0) /
                performances.length) *
                10
            ) / 10
          : 0;
