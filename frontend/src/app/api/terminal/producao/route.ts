...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('üìä Dados recebidos para produ·ß·£o:', body)
    ...
    // 1. Buscar produto para obter informa·ß·µes b·°sicas
    ...
    if (produtoError || !produto) {
      return NextResponse.json({
        success: false,
        error: `Produto ${body.produto_codigo} n·£o encontrado: ${produtoError?.message}`
      }, { status: 404 })
    }
    ...
    let receitaId = receitas[0].id // Usar o ID da primeira receita como refer·™ncia
    ...
    if (producaoError) {
      console.error('ùå Erro ao inserir produ·ß·£o:', producaoError)
      return NextResponse.json({
        success: false,
        error: `Erro ao salvar produ·ß·£o: ${producaoError.message}`
      }, { status: 500 })
    }

    console.log('úÖ Produ·ß·£o salva com sucesso:', producao.id)
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`üè≠ Terminal: Buscando hist·≥rico de produ·ß·£o`, {
      bar_id: barId,
      funcionario,
