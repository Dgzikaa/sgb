...
export async function POST(request: NextRequest) {
  try {
    ...
    const conflitos = await verificarConflitosAtribuicao(supabase as SupabaseClient, data as Record<string, unknown>, user.bar_id.toString())
    ...
    await criarAgendamentosAutomaticos(supabase as SupabaseClient, atribuicao as Record<string, unknown>)
  ...
  try {
    ...
    const atribuicoesEnriquecidas = await Promise.all(
      (atribuicoes || []).map(async (atribuicao: any) => {
        const stats = await calcularEstatisticasAtribuicao(supabase as SupabaseClient, atribuicao.id as string)
        return {
          ...atribuicao,
          estatisticas: stats
        }
...
function validarDadosAtribuicao(data: Record<string, unknown>) {
  ...
  // Validar datas
  if (data.data_fim && new Date(data.data_fim as string) <= new Date(data.data_inicio as string)) {
...
async function verificarConflitosAtribuicao(supabase: SupabaseClient, data: Record<string, unknown>, barId: string) {
...
async function criarAgendamentosAutomaticos(supabase: SupabaseClient, atribuicao: Record<string, unknown>) {
  try {
...
function criarAgendamento(atribuicao: Record<string, unknown>, dataAgendamento: Date) {
  return {
    ...
    configuracao: {
      tolerancia_minutos: (atribuicao.configuracao_frequencia as any).tolerancia_minutos,
      lembrete_antecipado_minutos: (atribuicao.configuracao_frequencia as any).lembrete_antecipado_minutos,
      auto_cancelar_apos_horas: (atribuicao.configuracao_frequencia as any).auto_cancelar_apos_horas
    },
    criado_em: new Date().toISOString()
  }
}

async function calcularEstatisticasAtribuicao(supabase: SupabaseClient, atribuicaoId: string) {
  ...
  const concluidos = agendamentos.filter((a: Record<string, unknown>) => a.status === 'concluido').length
  const pendentes = agendamentos.filter((a: Record<string, unknown>) => a.status === 'agendado' && new Date(a.data_agendada as string) > agora).length
  const atrasados = agendamentos.filter((a: Record<string, unknown>) => a.status === 'agendado' && new Date(a.data_agendada as string) <= agora).length
...
function calcularEstatisticasGerais(atribuicoes: Array<Record<string, unknown>>) {
  const total = atribuicoes.length
  const ativas = atribuicoes.filter((a: Record<string, unknown>) => a.ativo as boolean).length
  const inativas = total - ativas

  const totalAgendados = atribuicoes.reduce((acc: number, a: Record<string, unknown>) => acc + ((a.estatisticas as Record<string, unknown>)?.total_agendados as number || 0), 0)
  const totalConcluidos = atribuicoes.reduce((acc: number, a: Record<string, unknown>) => acc + ((a.estatisticas as Record<string, unknown>)?.concluidos as number || 0), 0)
  const totalAtrasados = atribuicoes.reduce((acc: number, a: Record<string, unknown>) => acc + ((a.estatisticas as Record<string, unknown>)?.atrasados as number || 0), 0)
