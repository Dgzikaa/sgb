...
export async function POST(request: NextRequest) {
  try {
    ...
    // Apenas admin e financeiro podem criar atribui·ß·µes
    if (!['admin', 'financeiro'].includes(user.role)) {
      return NextResponse.json({ 
        error: 'Sem permiss·£o para criar atribui·ß·µes' 
      }, { status: 403 })
    }
    ...
    if (checklistError || !checklist) {
      return NextResponse.json({ 
        error: 'Checklist n·£o encontrado ou inativo' 
      }, { status: 404 })
    }

    // Validar dados espec·≠ficos por tipo de atribui·ß·£o
    const validacao = validarDadosAtribuicao(data)
    if (!validacao.valido) {
      return NextResponse.json({ 
        error: 'Dados de atribui·ß·£o inv·°lidos',
        detalhes: validacao.erros 
      }, { status: 400 })
    }

    // Verificar conflitos de atribui·ß·£o
    const conflitos = await verificarConflitosAtribuicao(supabase: any, data, user.bar_id.toString())
    if (conflitos.length > 0) {
      return NextResponse.json({ 
        error: 'Conflito com atribui·ß·µes existentes',
        conflitos 
      }, { status: 409 })
    }
    ...
    if (createError) {
      console.error('Erro ao criar atribui·ß·£o:', createError)
      return NextResponse.json({ 
        error: 'Erro ao criar atribui·ß·£o' 
      }, { status: 500 })
    }

    // Criar agendamentos autom·°ticos para esta atribui·ß·£o
    await criarAgendamentosAutomaticos(supabase: any, atribuicao)

    console.log(`úÖ Atribui·ß·£o criada: ${checklist.nome} -> ${data.tipo_atribuicao}`)
    ...
    console.error('Erro na API de criar atribui·ß·£o:', error)
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Buscar atribui·ß·µes com pagina·ß·£o
    ...
    // Enriquecer atribui·ß·µes com estat·≠sticas
    ...
    console.error('Erro na API de listar atribui·ß·µes:', error)
    return NextResponse.json({ 
      error: 'Erro interno do servidor',
      details: error.message 
    }, { status: 500 })
  ...
  switch (data.tipo_atribuicao) {
    case 'funcionario_especifico':
      if (!data.funcionario_id) {
        erros.push('ID do funcion·°rio ·© obrigat·≥rio para atribui·ß·£o espec·≠fica')
      }
      break

    case 'cargo':
      if (!data.cargo) {
        erros.push('Cargo ·© obrigat·≥rio para atribui·ß·£o por cargo')
      }
      break

    case 'setor':
      if (!data.setor) {
        erros.push('Setor ·© obrigat·≥rio para atribui·ß·£o por setor')
      }
      break
  }
  ...
  switch (data.frequencia) {
    case 'diaria':
      if (!config.horarios || config.horarios.length === 0) {
        erros.push('Hor·°rios s·£o obrigat·≥rios para frequ·™ncia di·°ria')
      }
      break

    case 'semanal':
      if (!config.dias_semana || config.dias_semana.length === 0) {
        erros.push('Dias da semana s·£o obrigat·≥rios para frequ·™ncia semanal')
      }
      if (!config.horarios || config.horarios.length === 0) {
        erros.push('Hor·°rios s·£o obrigat·≥rios para frequ·™ncia semanal')
      }
      break

    case 'personalizada':
      if (!config.recorrencia_personalizada) {
        erros.push('Express·£o de recorr·™ncia ·© obrigat·≥ria para frequ·™ncia personalizada')
      }
      break
  }
...
async function verificarConflitosAtribuicao(supabase: any, data: any, barId: string) {
  ...
  // Buscar atribui·ß·µes existentes que possam conflitar
      ...
      switch (data.tipo_atribuicao) {
        case 'funcionario_especifico':
          if (existente.funcionario_id === data.funcionario_id) {
            temConflito = true
            motivo = 'Funcion·°rio j·° possui atribui·ß·£o para este checklist'
          }
          break
        ...
        case 'cargo':
          if (existente.cargo === data.cargo) {
            temConflito = true
            motivo = 'Cargo j·° possui atribui·ß·£o para este checklist'
          }
          break
        ...
        case 'setor':
          if (existente.setor === data.setor) {
            temConflito = true
            motivo = 'Setor j·° possui atribui·ß·£o para este checklist'
          }
          break

        case 'todos':
          temConflito = true
          motivo = 'J·° existe atribui·ß·£o geral para este checklist'
          break
      }
    }
...
async function criarAgendamentosAutomaticos(supabase: any, atribuicao: any) {
  try {
    const agendamentos = gerarAgendamentos(atribuicao: any, 30) // Pr·≥ximos 30 dias
    ...
    if (agendamentos.length > 0) {
      ...
      if (error) {
        console.error('Erro ao criar agendamentos autom·°ticos:', error)
      } else {
        console.log(`üìÖ ${agendamentos.length} agendamentos criados para atribui·ß·£o ${atribuicao.id}`)
      }
    }
  } catch (error) {
    console.error('Erro na cria·ß·£o de agendamentos autom·°ticos:', error)
  }
}
...
function gerarAgendamentos(atribuicao: any, dias: number) {
  ...
  switch (atribuicao.frequencia) {
    case 'diaria':
      for (let data = new Date(dataInicio); data <= dataFim; data.setDate(data.getDate() + 1)) {
        if (config.dias_semana && !config.dias_semana.includes(data.getDay())) {
          continue // Pular dias n·£o configurados
        }
        ...
        config.horarios?.forEach((horario: string) => {
          ...
          if (dataAgendamento > new Date()) { // S·≥ agendar para o futuro
    ...
    case 'mensal':
      // Implementa·ß·£o mensal (primeiro dia ·∫til do m·™s, etc.)
...
async function calcularEstatisticasAtribuicao(supabase: any, atribuicaoId: string) {
  // Buscar agendamentos desta atribui·ß·£o
