...
export async function GET(
  ...
  try {
    ...
    const estatisticas = await calcularEstatisticasDetalhadas(supabase, atribuicaoId)
...
export async function PUT(
  ...
  try {
    ...
    if (mudancasSignificativas) {
      await recriarAgendamentos(supabase, atribuicaoAtualizada)
    }
  ...
  try {
    ...
    if (agendamentosPendentes && agendamentosPendentes.length > 0 && !forceDelete) {
      return NextResponse.json({ 
        ...
        sugestao: 'Use force=true para forçar a exclusão ou desative a atribuição'
      }, { status: 409 })
    }
...
async function calcularEstatisticasDetalhadas(supabase: any, atribuicaoId: string) {
  ...
  if (!agendamentos) {
   ...
   const pendentes = agendamentos.filter((a: any) => a.status === 'agendado' && new Date(a.data_agendada) > agora)
   const atrasados = agendamentos.filter((a: any) => a.status === 'agendado' && new Date(a.data_agendada) <= agora)
   const cancelados = agendamentos.filter((a: any) => a.status === 'cancelado')

   // Calcular scores e tempos
   const execucoesCompletas = concluidos.filter((a: any) => a.execucao?.score_final != null)
   const scoreMedio = execucoesCompletas.length > 0 ? 
     execucoesCompletas.reduce((acc: number, a: any) => acc + a.execucao.score_final, 0) / execucoesCompletas.length : 0

   const execucoesComTempo = concluidos.filter((a: any) => a.execucao?.tempo_total_minutos != null)
   const tempoMedio = execucoesComTempo.length > 0 ?
     execucoesComTempo.reduce((acc: number, a: any) => acc + a.execucao.tempo_total_minutos, 0) / execucoesComTempo.length : 0
...
function calcularEvolucaoMensal(agendamentos: any[]) {
  const agora = new Date()
  const evolucao = []
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const concluidos = agendamentosMes.filter((a: any) => a.status === 'concluido')
...
function verificarMudancasSignificativas(atribuicaoAtual: any, novosDados: any): boolean {
...
async function recriarAgendamentos(supabase: any, atribuicao: any) {
  try {
    ...
    if (novosAgendamentos.length > 0) {
      ...
      console.log(`📅 ${novosAgendamentos.length} novos agendamentos criados para atribuição ${atribuicao.id}`)
    }
