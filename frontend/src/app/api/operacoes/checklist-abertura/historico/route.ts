...
export async function GET(request: NextRequest) {
  try {
    ...
    // Buscar histÃ¡Â³rico de checklists de abertura
    ...
    if (error) {
      console.error('Erro ao buscar histÃ¡Â³rico:', error)
      return NextResponse.json({ error: 'Erro ao buscar histÃ¡Â³rico' }, { status: 500 })
    }

    // Formatar dados para retorno
    const historicoFormatado = (historico || []).map((item: ChecklistAbertura) => ({
      id: item.id,
      data: item.data,
      status: item.status,
      ...
      tempo_total: item.hora_inicio && item.hora_conclusao ? 
        calcularTempoTotal(item.hora_inicio, item.hora_conclusao) : null,
    ...
    console.error('Erro na API de histÃ¡Â³rico:', error)
...
function calcularTempoTotal(inicio: string, fim: string): string {
  try {
    ...
    // Se a diferenÃ¡Â§a for negativa, significa que passou para o dia seguinte
