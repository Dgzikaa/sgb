...
export async function GET(request: Request) {
  try {
    ...
    console.log(
      `📊 Recebendo requisição - Página ${page}, Limite ${limit}, Offset ${offset}`
    );
    ...
    console.log(
      `📊 Buscando dados de recorrência (APENAS COM TELEFONE) - Página ${page}, Limite ${limit}...`
    );
    ...
    try {
      console.log(
        `🔍 Buscando ContaHub PERIODO (valores reais) - offset: ${offset}, limit: ${limit}`
      );
      ...
      if (contahubResult.error) {
        ...
        console.log(
          `📊 ContaHub PERIODO: ${totalContahub} registros, total: ${contahub_count}, hasMore: ${hasMoreContahub}`
        );
        ...
        contahub_data?.forEach((periodo: unknown) => {
          ...
          if (telefone) {
            ...
            if (telefoneNormalizado && telefoneNormalizado.length >= 10) {
              ...
              clienteData.valor_total_gasto += parseFloat(
                periodo.vr_pagamentos || 0
              );
            }
          }
        });
    ...
    try {
      console.log(
        `🔍 Buscando Yuzer (apenas com telefone) - offset: ${offset}, limit: ${limit}`
      );
      ...
      if (yuzerResult.error) {
        ...
        console.log(
          `📊 Yuzer: ${totalYuzer} registros, total: ${yuzer_count}, hasMore: ${hasMoreYuzer}`
        );
    ...
    const clientes: ClienteRanking[] = Array.from(clientesMap.values()).map(
      cliente => ({
        ...cliente,
        ticket_medio:
          cliente.total_visitas > 0
            ? cliente.valor_total_gasto / cliente.total_visitas
            : 0,
      })
    );
    ...
    console.log(
      `📊 Dados processados (APENAS COM TELEFONE) - Página ${page}:`,
      {
