...
// Funá§á£o para formatar telefone
...
export async function GET(request: Request) {
  try {
    ...
    console.log(`ðŸ“Š Recebendo requisiá§á£o - Pá¡gina ${page}, Limite ${limit}, Offset ${offset}`);
    ...
    console.log(`ðŸ“Š Buscando dados de recorráªncia (APENAS COM TELEFONE) - Pá¡gina ${page}, Limite ${limit}...`);
    ...
    try {
      ...
      if (contahubResult.error) {
        ...
        contahub_data?.forEach((periodo: any) => {
          ...
          if (telefone) {
            ...
            // Sá³ processar se o telefone normalizado for vá¡lido
      ...
      console.error('Œ Erro ao buscar ContaHub PERIODO:', error);
    }
        ...
        yuzer_data?.forEach((reserva: any) => {
          ...
          if (telefone) {
            ...
            if (telefoneNormalizado && telefoneNormalizado.length >= 10) {
              ...
              // Yuzer: Ná£o temos dados de valor real, apenas contamos as visitas
              // O valor fica 0 para ná£o distorcer os cá¡lculos
            }
          }
        });
      }
    } catch (error) {
      console.error('Œ Erro ao buscar Yuzer:', error);
    }

    // 3. Pular dados do Sympla pois ná£o táªm telefone
    ...
    console.log(`ðŸ“Š Dados processados (APENAS COM TELEFONE) - Pá¡gina ${page}:`, {
      ...
      telefones_processados: clientes.length, // Todos táªm telefone
      contahub: totalContahub,
      yuzer: totalYuzer,
      sympla: 0 // Ná£o incluá­mos Sympla
    });
    ...
    return NextResponse.json({
      ...
      totais: {
        ...
        sympla_total: 0 // Ná£o incluá­mos Sympla
      },
      has_more: {
        contahub: hasMoreContahub,
        yuzer: hasMoreYuzer,
        sympla: false // Ná£o incluá­mos Sympla
      }
    });

  } catch (error) {
    console.error('Œ Erro no endpoint de recorráªncia:', error);
    return NextResponse.json({ 
