...
// Interface para email
interface EmailPeriodo {
  cli_email: string;
}

function isEmailPeriodo(obj: unknown): obj is EmailPeriodo {
  return (
    typeof obj === 'object' &&
    obj !== null &&
    'cli_email' in obj &&
    typeof (obj as any).cli_email === 'string'
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!periodError1 && periodData1) {
      emailsData1 = (periodData1 as unknown[]).filter(isEmailPeriodo);
    }
    if (!periodError2 && periodData2) {
      emailsData2 = (periodData2 as unknown[]).filter(isEmailPeriodo);
    }

    console.log(
      `📧 Emails encontrados - Data1: ${emailsData1.length}, Data2: ${emailsData2.length}`
    );
    ...
    if (emailsData1.length === 0 && emailsData2.length === 0) {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          percentualNovos: 100,
          crescimento:
            totalData1 > 0 ? ((totalData2 - totalData1) / totalData1) * 100 : 0,
          insights: {
    ...
    const emailsUnicosData1 = new Set(
      emailsData1?.map(item => item.cli_email.toLowerCase().trim()) || []
    );
    const emailsUnicosData2 = new Set(
      emailsData2?.map(item => item.cli_email.toLowerCase().trim()) || []
    );

    // Calculate recurrence
    const emailsRecorrentes = new Set(
      [...emailsUnicosData1].filter(email => emailsUnicosData2.has(email))
    );
    const emailsNovos = new Set(
      [...emailsUnicosData2].filter(email => !emailsUnicosData1.has(email))
    );
    ...
    const percentualRecorrencia =
      totalData2 > 0 ? (recorrentes / totalData2) * 100 : 0;
    ...
    console.log(
      `   Recorrentes: ${recorrentes} (${percentualRecorrencia.toFixed(1)}%)`
    );
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        insights: {
          fidelizacao:
            percentualRecorrencia >= 40
              ? 'Alta'
              : percentualRecorrencia >= 25
                ? 'Média'
                : 'Baixa',
          captacao:
            percentualNovos >= 60
