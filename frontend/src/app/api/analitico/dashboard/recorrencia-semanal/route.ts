...
// Interface para dados vindos do contahub
interface ReservaContahub {
  vd_dtgerencial: string;
  tel_cli: string;
  nm_cli: string;
}

function isReservaContahub(obj: unknown): obj is ReservaContahub {
  return (
    typeof obj === 'object' &&
    obj !== null &&
...
// Interface para reservas do dashboard
...
function isReservaDashboard(obj: unknown): obj is ReservaDashboard {
  return (
    typeof obj === 'object' &&
    obj !== null &&
    'phone' in obj &&
  ...
  try {
    ...
    try {
      ...
      if (errorGetin || !reservasGetin || reservasGetin.length === 0) {
        console.log(
          '⚠️ Tabela getin_reservas não encontrada ou sem dados, tentando outras fontes...'
        );
        ...
        if (!contahubError && contahubData && contahubData.length > 0) {
          ...
          dadosReservas = (contahubData as unknown[])
            .filter(isReservaContahub)
            .map(item => ({
              phone: item.tel_cli,
              name: item.nm_cli,
      ...
      console.log(
        `📊 Total de registros com telefone: ${dadosReservas.length}`
      );
      ...
      const reservas: ReservaDashboard[] = (dadosReservas as unknown[]).filter(isReservaDashboard);
      ...
      const clientesRecorrentes = clientesArray.filter(
        cliente => cliente.total_visitas > 1
      ).length;
      const taxaRecorrencia =
        clientesUnicos > 0 ? (clientesRecorrentes / clientesUnicos) * 100 : 0;
      ...
      const clientesRecorrentesDetalhados = clientesArray
        ...
        .map(cliente => ({
          nome: cliente.nome,
          telefone: cliente.telefone.replace(
            /(\d{2})(\d{4,5})(\d{4})/,
            '($1) $2-$3'
          ), // formatar telefone
      ...
      return NextResponse.json({
        ...
        meta: {
          ...
          fonte_dados:
            reservasGetin && reservasGetin.length > 0
              ? 'getin_reservas'
              : 'analitico',
        },
      });
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Erro ao buscar dados: ' + (dbError as Error).message,
        },
        { status: 500 }
      );
    }
