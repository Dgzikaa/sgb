...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (faceError) {
      console.error('âŒ Erro ao verificar registro facial:', faceError)
      return NextResponse.json(
        { success: false, error: 'Erro interno do servidor' },
        { status: 500 }
      )
    ...
    console.log(`âœ… Status verificado para ${usuario.nome}: ${faceRegistered ? 'Registrado' : 'NÃ£o registrado'}`)
