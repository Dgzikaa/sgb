...
function findBestMatch(inputDescriptor: number[], storedDescriptors: any[]) {
  ...
  for (const stored of storedDescriptors) {
    try {
      ...
      console.log(
        `📏 Distância para ${stored.user_nome}: ${distance.toFixed(4)} (threshold: ${stored.confidence_threshold})`
      );
      ...
      console.error(
        `❌ Erro ao calcular distância para usuário ${stored.user_nome}:`,
        error
      );
    }
  }
  ...
  try {
    ...
    if (!faceDescriptors || faceDescriptors.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: 'Nenhuma face registrada encontrada para este bar',
        },
        { status: 404 }
      );
    }
    ...
    console.log(
      `🔍 Comparando com ${faceDescriptors.length} faces registradas`
    );
