...
function findBestMatch(inputDescriptor: number[], storedDescriptors[]) {
  ...
  for (const stored of storedDescriptors) {
    try {
      ...
      console.log(`üìè Dist·¢ncia para ${stored.user_nome}: ${distance.toFixed(4)} (threshold: ${stored.confidence_threshold})`)
      ...
      console.error(`ùå Erro ao calcular dist·¢ncia para usu·°rio ${stored.user_nome}:`, error)
    }
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`üîç Comparando com ${faceDescriptors.length} faces registradas`)
    ...
    if (!bestMatch) {
      ...
      return NextResponse.json(
        { success: false, error: 'Face n·£o reconhecida. Tente novamente ou use login tradicional.' },
        { status: 401 }
      )
    }

    console.log(`úÖ Face reconhecida: ${bestMatch.user_nome} (similaridade: ${bestMatch.similarity.toFixed(1)}%)`)
    ...
    // Log de auditoria
    console.log(`üéâ LOGIN FACIAL CONCLU·çDO: ${user.nome} (${user.email}) - Bar ${barId}`)
