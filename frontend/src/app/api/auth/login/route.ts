...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!usuarios || usuarios.length === 0) {
      ...
      console.log('🔍 Todos os usuários (incluindo inativos):', todosUsuarios?.length || 0);
      ...
      console.log('🔍 Usuários encontrados por email:', usuariosPorEmail?.length || 0);
    ...
    if (!usuarioPrincipal.senha_redefinida) {
      console.log('🔑 Primeiro acesso detectado - redirecionando para redefinição de senha')
    ...
    const barIds = [...new Set(baresAcesso.map((b: { bar_id: string }) => b.bar_id))];
    ...
    const baresComNome = baresAcesso.map((bar: { bar_id: string; role: string; modulos_permitidos?: string[] }) => {
      const barData = barsData?.find((b: { id: string; nome: string }) => b.id === bar.bar_id);
      return {
        ...bar,
    ...
    const credenciaisPromises = baresComNome.map(async (bar: { bar_id: string }) => {
