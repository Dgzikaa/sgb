...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      body.insumos &&
      Array.isArray(body.insumos) &&
      body.insumos.length > 0
    ) {
      console.log(
        '📊 Calculando aderência à receita com',
        body.insumos.length,
        'insumos'
      );
      ...
      body.insumos.forEach((insumo: InsumoProducao, index: number) => {
        ...
        const calculado =
          parseFloat(String(insumo.quantidade_calculada)) || planejado;
        ...
        if (calculado > 0 && real > 0) {
          ...
          console.log(
            `📋 Insumo ${index + 1} (${insumo.nome}): Calculado=${calculado}${insumo.unidade_medida}, Real=${real}${insumo.unidade_medida}, Aderência=${aderencia.toFixed(1)}%`
          );
        }
      });
      ...
      if (insumos_validos > 0) {
        ...
        console.log(
          `🎯 Desvio médio: ${desvio_medio.toFixed(1)}% | Aderência final: ${percentual_aderencia_receita.toFixed(1)}%`
        );
      }
    }
    ...
    const dadosProducao = {
      ...
      inicio_producao: body.inicio_producao
        ? new Date(body.inicio_producao).toISOString()
        : new Date().toISOString(),
      fim_producao: body.fim_producao
        ? new Date(body.fim_producao).toISOString()
        : new Date().toISOString(),
      ...
      percentual_aderencia_receita:
        Math.round(percentual_aderencia_receita * 100) / 100, // 2 decimais
    ...
    if (
      ...
      const insumosParaSalvar = body.insumos.map((insumo: InsumoProducao) => ({
        ...
        quantidade_planejada:
          parseFloat(String(insumo.quantidade_necessaria)) || 0,
        quantidade_calculada:
          parseFloat(String(insumo.quantidade_calculada)) ||
          parseFloat(String(insumo.quantidade_necessaria)) ||
          0,
        quantidade_utilizada_real:
          parseFloat(String(insumo.quantidade_real)) || 0,
      ...
      if (insumosError) {
        console.error(
          '⚠️ Erro ao salvar insumos (produção salva):',
          insumosError
        );
    ...
    if (producoes && producoes.length > 0) {
      ...
      if (desvios.length > 0) {
        estatisticas.desvio_medio =
          desvios.reduce((a: number, b: number) => a + b, 0) / desvios.length;
        estatisticas.eficiencia_excelente = desvios.filter(
          (d: number) => d >= 95
        ).length;
        estatisticas.eficiencia_boa = desvios.filter(
          (d: number) => d >= 85 && d < 95
        ).length;
        estatisticas.eficiencia_regular = desvios.filter(
          (d: number) => d >= 75 && d < 85
        ).length;
        estatisticas.eficiencia_ruim = desvios.filter(
          (d: number) => d < 75
        ).length;
      }
    }
