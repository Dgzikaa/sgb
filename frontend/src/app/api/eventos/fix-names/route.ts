...
03/02	SÃ¡ÂBADO		ESPECIAL - "ALCIONE" - Karla Sangaletti (DJ)
04/02	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
05/02	SEGUNDA		FECHADO
06/02	TERÃ¡â€¡A		FECHADO
...
24/02	SÃ¡ÂBADO		ESPECIAL - "ZECA PAGODINHO" - Nenel Vida (DJ)
25/02	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
26/02	SEGUNDA		FECHADO
27/02	TERÃ¡â€¡A		FECHADO
...
03/03	SÃ¡ÂBADO		ESPECIAL - "ALCIONE" - Karla Sangaletti (DJ)
04/03	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
05/03	SEGUNDA		FECHADO
06/03	TERÃ¡â€¡A		FECHADO
...
17/03	SÃ¡ÂBADO		ESPECIAL - "ZECA PAGODINHO" - Nenel Vida (DJ)
18/03	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
19/03	SEGUNDA		FECHADO
20/03	TERÃ¡â€¡A		FECHADO
...
24/03	SÃ¡ÂBADO		ESPECIAL - "ALCIONE" - Karla Sangaletti (DJ)
25/03	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
26/03	SEGUNDA		FECHADO
27/03	TERÃ¡â€¡A		FECHADO
...
07/04	SÃ¡ÂBADO		ESPECIAL - "ZECA PAGODINHO" - Nenel Vida (DJ)
08/04	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
09/04	SEGUNDA		FECHADO
10/04	TERÃ¡â€¡A		FECHADO
...
14/04	SÃ¡ÂBADO		ESPECIAL - "ALCIONE" - Karla Sangaletti (DJ)
15/04	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
16/04	SEGUNDA		FECHADO
17/04	TERÃ¡â€¡A		FECHADO
...
21/04	SÃ¡ÂBADO		CARNAVAL - Bloco da Zuera (Carnaval)
22/04	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
23/04	SEGUNDA		FECHADO
24/04	TERÃ¡â€¡A		FECHADO
...
28/04	SÃ¡ÂBADO		DJ SET - MC's diversos (DJ)
29/04	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
30/04	SEGUNDA		FECHADO

01/05	TERÃ¡â€¡A		FERIADO - Trabalhador
...
05/05	SÃ¡ÂBADO		ESPECIAL - "ZECA PAGODINHO" - Nenel Vida (DJ)
06/05	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
07/05	SEGUNDA		FECHADO
08/05	TERÃ¡â€¡A		FECHADO
...
12/05	SÃ¡ÂBADO		DIA DAS MÃ¡Æ’ES - Especial MÃ¡Â£es que Sambam (Samba)
13/05	DOMINGO		Uma mesa e um pagode - Doze (Pagode)
14/05	SEGUNDA		FECHADO
15/05	TERÃ¡â€¡A		FECHADO
...
28/06	SÃ¡ÂBADO		Festival Junino - Sambadona (Samba)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Ã°Å¸â€Â§ Starting event name fix...');
    ...
    console.log(`Ã°Å¸â€œâ€¹ Parsed ${Object.keys(dateNameMap).length} event names`);
    ...
    console.log(`Ã°Å¸â€œÂ¥ Found ${eventos?.length || 0} events to update`);
    ...
    for (const evento of eventos || []) {
      ...
      console.log(`Ã°Å¸â€Â Event ${evento.data_evento}: current="${evento.nome_evento}", should be="${correctName}"`);
      
      if (correctName && correctName !== evento.nome_evento) {
        console.log(`Ã°Å¸â€œÂ Will update ${evento.data_evento}: "${evento.nome_evento}" â€ â€™ "${correctName}"`);
        updates.push({
          id: evento.id,
        ...
        console.log(`Å“â€¦ Event ${evento.data_evento} already has correct name: "${correctName}"`);
      } else {
        console.log(`Å¡Â Ã¯Â¸Â  No mapping found for ${evento.data_evento}`);
      }
    }
    
    console.log(`Ã°Å¸â€â€ž Will update ${updates.length} events`);
    ...
    for (const [date, correctName] of Object.entries(dateNameMap)) {
      ...
      if (updateError) {
        ...
        console.log(`Å“â€¦ Updated ${date} â€ â€™ "${correctName}"`);
        updatedCount++;
      }
    }
    ...
    for (const update of updates) {
      ...
      if (updateError) {
        ...
        console.log(`Å“â€¦ Updated ${update.data_evento}: "${update.old_name}" â€ â€™ "${update.new_name}"`);
        updatedCount++;
      }
    }
