...
export async function POST(request: NextRequest) {
  try {
    ...
    const diasFechados = [
      // Fechados gerais (segundas e terá§as)
    ...
    if (error) {
      console.error('Œ Erro ao remover dias fechados:', error);
    ...
    console.log(`œ… Removidos ${data?.length || 0} eventos de dias fechados`);
