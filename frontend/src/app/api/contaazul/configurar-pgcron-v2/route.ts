...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`ü§ñ Configurando pgcron V2 - Bar: ${barId}, A·ß·£o: ${action}`)
    ...
    console.error('ùå Erro na configura·ß·£o pgcron V2:', error)
...
async function configurarCronJob(supabase: any, barId: string) {
  try {
    ...
    console.log(`úÖ Cron job criado: ${jobName}`)
    ...
    return NextResponse.json({
      ...
      command: comando,
      nextRuns: [
        'Pr·≥xima execu·ß·£o: conforme cronograma',
...
async function verificarStatus(supabase: any, barId: string) {
  try {
    ...
    if (jobsError) {
      console.warn('ö†Ô∏è Erro ao buscar jobs:', jobsError)
    }
    ...
    if (runsError) {
      console.warn('ö†Ô∏è Erro ao buscar execu·ß·µes:', runsError)
    }
