...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('üîç Verificando credenciais ContaAzul...')
    ...
    if (expiraEm <= agora) {
      throw new Error('Token ContaAzul expirado. Renova·ß·£o necess·°ria.')
    }
    ...
    console.log('üí∞ Coletando receitas como dados brutos...')
      ...
      for (const categoria of categoriasReceita) {
        ...
        while (paginaReceita <= maxPaginas) {
          try {
            ...
            // üî• SALVAR DADOS BRUTOS - TRIGGER PROCESSAR·Å AUTOMATICAMENTE
            ...
            if (insertError) {
              console.error('ùå Erro ao salvar dados brutos receitas:', insertError)
              stats.erros++
            } else {
              stats.receitas_lotes_coletados++
              stats.total_registros_brutos += receitas.length
              console.log(`úÖ Receitas Cat: ${categoria.id}, P·°gina: ${paginaReceita} - ${receitas.length} registros salvos`)
            }
            ...
            console.error(`ùå Erro ao coletar receitas - Cat: ${categoria.id}, P·°gina: ${paginaReceita}:`, error)
            stats.erros++
            break
          }
        }
    ...
    console.log('üí∏ Coletando despesas como dados brutos...')
      ...
      for (const categoria of categoriasDespesa) {
        ...
        while (paginaDespesa <= maxPaginas) {
          try {
            ...
            if (insertError) {
              console.error('ùå Erro ao salvar dados brutos despesas:', insertError)
              stats.erros++
            } else {
              stats.despesas_lotes_coletados++
              stats.total_registros_brutos += despesas.length
              console.log(`úÖ Despesas Cat: ${categoria.id}, P·°gina: ${paginaDespesa} - ${despesas.length} registros salvos`)
            }
            ...
            console.error(`ùå Erro ao coletar despesas - Cat: ${categoria.id}, P·°gina: ${paginaDespesa}:`, error)
            stats.erros++
            break
          }
        }
    ...
    console.log(`   Ä¢ Categorias sincronizadas: ${stats.categorias_sincronizadas}`)
    console.log(`   Ä¢ Lotes de receitas coletados: ${stats.receitas_lotes_coletados}`)
    console.log(`   Ä¢ Lotes de despesas coletados: ${stats.despesas_lotes_coletados}`)
    console.log(`   Ä¢ Total de registros brutos: ${stats.total_registros_brutos}`)
    console.log(`   Ä¢ Erros: ${stats.erros}`)
    console.log(`   Ä¢ Dura·ß·£o: ${duracaoSegundos}s`)
    console.log(`   Ä¢ Processamento: Trigger autom·°tico`)
    ...
    console.error('ùå Erro na coleta de dados brutos:', error)
    return NextResponse.json({
      success: false,
