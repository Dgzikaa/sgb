  ...
  try {
    ...
    if (credError || !credentials) {
      return NextResponse.json({ 
        error: 'Credenciais ContaAzul não encontradas' 
      }, { status: 404 })
    }
    ...
    if (expiraEm <= agora) {
      return NextResponse.json({ 
        error: 'Token ContaAzul expirado. Renovação necessária.' 
      }, { status: 401 })
    }
    ...
    // 🆕 TESTE 3: Receitas v2 COM PARÂMETROS DE COMPETÊNCIA
    if (categoriasReceita && categoriasReceita.length > 0) {
    ...
    // 🆕 TESTE 4: Despesas v2 COM PARÂMETROS DE COMPETÊNCIA
    if (categoriasDespesa && categoriasDespesa.length > 0) {
    ...
    return NextResponse.json({
      sucesso: true,
      message: 'Teste endpoints v2 corretos com data_competencia concluído',
      testes,
      conclusoes: {
