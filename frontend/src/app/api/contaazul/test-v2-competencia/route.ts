...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Ã°Å¸Â§Âª TESTANDO ENDPOINTS V2 CORRETOS COM DATA_COMPETENCIA...')
    ...
    if (categoriasReceita && categoriasReceita.length > 0) {
      console.log('Ã°Å¸Â§Âª TESTE 1: Receitas v2 endpoint correto...')
      try {
    ...
    if (categoriasDespesa && categoriasDespesa.length > 0) {
      console.log('Ã°Å¸Â§Âª TESTE 2: Despesas v2 endpoint correto...')
      try {
    ...
    // Ã°Å¸â€ â€¢ TESTE 3: Receitas v2 COM PARÃ¡â€šMETROS DE COMPETÃ¡Å NCIA
    if (categoriasReceita && categoriasReceita.length > 0) {
      console.log('Ã°Å¸Â§Âª TESTE 3: Receitas v2 COM data_competencia_de e data_competencia_ate...')
      try {
        const categoria = categoriasReceita[0]
    ...
    // Ã°Å¸â€ â€¢ TESTE 4: Despesas v2 COM PARÃ¡â€šMETROS DE COMPETÃ¡Å NCIA
    if (categoriasDespesa && categoriasDespesa.length > 0) {
      console.log('Ã°Å¸Â§Âª TESTE 4: Despesas v2 COM data_competencia_de e data_competencia_ate...')
      try {
        const categoria = categoriasDespesa[0]
    ...
    // Ã°Å¸â€ â€¢ TESTE 5: Endpoint de PARCELAS
    if (eventosFinanceiros && eventosFinanceiros.length > 0) {
      console.log('Ã°Å¸Â§Âª TESTE 5: Endpoint de parcelas...')
    ...
    return NextResponse.json({
      sucesso: true,
      message: 'Teste endpoints v2 corretos com data_competencia concluÃ¡Â­do',
      testes,
      conclusoes: {
