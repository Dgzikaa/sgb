...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Ã°Å¸Â§Âª TESTANDO NOVO ENDPOINT COM DATA_COMPETENCIA...')
    ...
    if (!accessToken) {
      console.error('ÂÅ’ NÃ¡Â£o foi possÃ¡Â­vel obter token vÃ¡Â¡lido (renovaÃ¡Â§Ã¡Â£o automÃ¡Â¡tica falhou)')
      return NextResponse.json({ 
        error: 'Token ContaAzul indisponÃ¡Â­vel. Verifique as credenciais ou reautorize.' 
      }, { status: 401 })
    }

    console.log('Å“â€¦ Token vÃ¡Â¡lido obtido (com renovaÃ¡Â§Ã¡Â£o automÃ¡Â¡tica se necessÃ¡Â¡rio)')

    // TESTE 1: Contas a Receber com data_competencia
    console.log('Ã°Å¸â€Â TESTE 1: Endpoint contas-a-receber com data_competencia...')
    ...
    if (responseReceitas.ok) {
      ...
      console.log('Å“â€¦ TESTE 1 bem-sucedido! Total de itens:', dadosReceitas?.itens?.length || 0)
    } else {
      console.error('ÂÅ’ TESTE 1 falhou:', responseReceitas.status, responseReceitas.statusText)
      const errorText = await responseReceitas.text()
      console.error('Erro detalhado:', errorText)
    }

    // TESTE 2: Contas a Pagar com data_competencia
    console.log('Ã°Å¸â€Â TESTE 2: Endpoint contas-a-pagar com data_competencia...')
    ...
    if (responseDespesas.ok) {
      ...
      console.log('Å“â€¦ TESTE 2 bem-sucedido! Total de itens:', dadosDespesas?.itens?.length || 0)
    } else {
      console.error('ÂÅ’ TESTE 2 falhou:', responseDespesas.status, responseDespesas.statusText)
    ...
    return NextResponse.json({
      sucesso: true,
      message: 'Teste de endpoints com data_competencia concluÃ¡Â­do',
      analise,
      dados_completos: {
    ...
    console.error('ÂÅ’ Erro interno no teste:', error)
    return NextResponse.json({ 
