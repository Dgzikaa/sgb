...
// Fun·ß·£o para buscar TODOS os dados com pagina·ß·£o autom·°tica
async function buscarTodosRegistros(query: any, chunkSize = 1000) {
  ...
  while (hasMore) {
    ...
    if (error) {
      console.error('ùå Erro na pagina·ß·£o autom·°tica:', error)
      break
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Novos par·¢metros para ordena·ß·£o
    const sortField = searchParams.get('sort_field') || 'data_competencia'
    const sortDirection = searchParams.get('sort_direction') || 'desc'
    
    console.log(`üîç API Eventos: bar_id=${barId}, page=${page}, limit=${limit}, tipo=${tipo}, sort=${sortField}:${sortDirection}`)
    console.log(`üîç Filtros: ${JSON.stringify({dataInicial, dataFinal: any, mes, ano: any, categoriasFiltro})}`)
    ...
    // Fun·ß·£o para criar query base com filtros
    ...
    // üö® BUSCAR TODOS OS DADOS PARA C·ÅLCULO CORRETO DOS TOTAIS
    console.log('üî¢ Buscando TODOS os registros para calcular totais corretos (sem limite de 1000)...')
    
    const queryResumo = criarQueryComFiltros('tipo, valor')
    const resumoData = await buscarTodosRegistros(queryResumo)
    
    console.log(`üí∞ Total de registros encontrados para c·°lculo: ${resumoData.length}`)
    ...
    if (resumoData && resumoData.length > 0) {
      console.log('üí∞ Calculando totais a partir de', resumoData.length, 'registros...')
      ...
      console.log('üìä Tipos de registros encontrados:', tiposCount)
      ...
      console.log('üíö Primeiras 5 receitas:', receitas.map((r: any) => ({ tipo: r.tipo, valor: r.valor })))
      console.log('ù§Ô∏è Primeiras 5 despesas:', despesas.map((d: any) => ({ tipo: d.tipo, valor: d.valor })))
      ...
      resumo = resumoData.reduce((acc: any, evento: any) => {
        ...
        if (evento.tipo === 'receita') {
          ...
          console.warn(`ö†Ô∏è Tipo n·£o reconhecido: ${evento.tipo}`)
        }
      ...
      console.log('úÖ Totais calculados corretamente:', {
      ...
      console.log(`   Diferen·ßa receitas: ${Math.abs(resumo.total_receitas - receitasManual) < 0.01 ? 'úÖ OK' : 'ùå ERRO'}`)
      console.log(`   Diferen·ßa despesas: ${Math.abs(resumo.total_despesas - despesasManual) < 0.01 ? 'úÖ OK' : 'ùå ERRO'}`)
      
    } else {
      console.log('ö†Ô∏è Nenhum dado encontrado para c·°lculo dos totais')
    }

    // Buscar dados paginados com ordena·ß·£o correta na API
    ...
    // Se ordena·ß·£o ·© por categoria, precisamos de ordena·ß·£o adicional
    if (sortField === 'categoria') {
      query = query.order('data_competencia', { ascending: false }) // Ordena·ß·£o secund·°ria
    }
    ...
    console.log(`üìä Eventos paginados encontrados: ${eventos?.length || 0}`)
    ...
    console.log(`üìã Categorias encontradas: ${categorias?.length || 0}`)
    ...
    const lancamentos = eventos?.map((evento: any) => ({
      id: evento.evento_id,
      descricao: evento.descricao || 'Sem descri·ß·£o',
    ...
    // Total de registros para pagina·ß·£o (usar o total j·° calculado)
    const totalRegistros = resumo.total_lancamentos

    console.log(`úÖ Eventos carregados: ${lancamentos.length} de ${totalRegistros} total`)
    ...
    console.error('ùå Erro na API de eventos financeiros:', error)
    return NextResponse.json(
