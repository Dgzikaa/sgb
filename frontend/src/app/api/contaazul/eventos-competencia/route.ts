...
export async function GET(request: NextRequest) {
  try {
    ...
    if (resumoData && resumoData.length > 0) {
      ...
      console.log('üíö Primeiras 5 receitas:', receitas.map((r) => ({ tipo: r.tipo, valor: r.valor })))
      console.log('ù§Ô∏è Primeiras 5 despesas:', despesas.map((d) => ({ tipo: d.tipo, valor: d.valor })))
