...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!credentials.refresh_token) {
      return NextResponse.json({ 
        error: 'Refresh token nÃ¡Â£o disponÃ¡Â­vel' 
      }, { status: 400 })
    }

    console.log('Ã°Å¸â€â€ž ForÃ¡Â§ando renovaÃ¡Â§Ã¡Â£o do token...')

    // ForÃ¡Â§ar renovaÃ¡Â§Ã¡Â£o do token
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        error: 'Falha na renovaÃ¡Â§Ã¡Â£o do token',
        details: data.error || `HTTP ${response.status}: ${response.statusText}`,
        response_data: data
      }, { status: 400 })
    }

    // Calcular nova data de expiraÃ¡Â§Ã¡Â£o
    ...
    if (updateError) {
      console.error('ÂÅ’ Erro ao salvar novo token:', updateError)
      return NextResponse.json({
    ...
    console.error('ÂÅ’ Erro ao renovar token:', error)
    return NextResponse.json({ 
      error: 'Erro interno',
