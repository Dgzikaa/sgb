...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!credentials.refresh_token) {
      return NextResponse.json({ 
        error: 'Refresh token ná£o disponá­vel' 
      }, { status: 400 })
    }

    console.log('ðŸ”„ Forá§ando renovaá§á£o do token...')

    // Forá§ar renovaá§á£o do token
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({
        error: 'Falha na renovaá§á£o do token',
    ...
    if (updateError) {
      console.error('Œ Erro ao salvar novo token:', updateError)
      return NextResponse.json({
