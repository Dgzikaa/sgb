...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Ã°Å¸â€â€˜ TESTANDO TOKEN CONTAAZUL...')

    const accessToken = await getValidContaAzulToken(parseInt(barId))
    
    if (!accessToken) {
      return NextResponse.json({ 
        error: 'Token indisponÃ¡Â­vel',
        token_length: 0,
        token_valid: false
      }, { status: 401 })
    }

    console.log('Å“â€¦ Token obtido:', accessToken.substring(0, 20) + '...')
    ...
    console.error('ÂÅ’ Erro no teste de token:', error)
    return NextResponse.json({ 
      error: 'Erro interno',
