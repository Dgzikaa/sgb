    ...
    try {
      ...
      const { error: scoreError } = await supabase
        ...
        .update({
          ...
          categoria_score: (scoreResult as any).percentual > 80 ? 'excelente' : (scoreResult as any).percentual > 60 ? 'bom' : 'regular'
        })
        .eq('id', execucao.id)
      ...
      if (scoreError) {
        ...
        console.log(`✅ Score calculado: ${(scoreResult as any).score}/${(scoreResult as any).maxScore} (${(scoreResult as any).percentual.toFixed(1)}%)`)
      }
    } catch (scoreError) {
