...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .gte('timestamp_evento', new Date(Date.now() - parseInt(periodo) * 24 * 60 * 60 * 1000).toISOString())
    ...
    return NextResponse.json({
      success: false,
      error: 'Erro ao buscar eventos de usuÃ¡Â¡rio',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!bar_id || !sessao_id || !evento_tipo || !evento_nome) {
      return NextResponse.json({
        success: false,
        error: 'Campos obrigatÃ¡Â³rios: bar_id, sessao_id, evento_tipo, evento_nome'
      }, { status: 400 })
    }

    const supabase = await getAdminClient()

    // Extrair informaÃ¡Â§Ã¡Âµes da requisiÃ¡Â§Ã¡Â£o
    const forwarded = request.headers.get('x-forwarded-for')
    const ip = forwarded ? forwarded.split(',')[0] : request.headers.get('x-real-ip') || 'unknown'
    const referrer = request.headers.get('referer')
    const userAgent = user_agent || request.headers.get('user-agent') || 'unknown'

    // Detectar dispositivo se nÃ¡Â£o fornecido
    ...
    // Detectar navegador se nÃ¡Â£o fornecido
    ...
    const { data: evento, error } = await supabase
      ...
      .insert({
        ...
        elemento_alvo,
        dados_evento,
        tempo_gasto_segundos: tempo_gasto_segundos ? parseInt(tempo_gasto_segundos) : null,
    ...
    console.error('ÂÅ’ Erro ao registrar evento:', error)
    return NextResponse.json({
      success: false,
...
// Endpoint para registrar mÃ¡Âºltiplos eventos de uma vez (batch)
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!eventos || !Array.isArray(eventos) || eventos.length === 0) {
      return NextResponse.json({
        success: false,
        error: 'Campo obrigatÃ¡Â³rio: eventos (array com pelo menos 1 item)'
      }, { status: 400 })
    }
    ...
    const eventosProcessados = eventos.map((evento: unknown) => ({
      ...
      tempo_gasto_segundos: evento.tempo_gasto_segundos ? parseInt(evento.tempo_gasto_segundos) : null,
    ...
    console.error('ÂÅ’ Erro ao registrar eventos em lote:', error)
    return NextResponse.json({
      success: false,
