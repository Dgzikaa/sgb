...
export async function GET(request: NextRequest) {
  try {
    ...
    const resumo = {
      ...
      usuarios_unicos: new Set(eventos?.map((e: UsuarioEvento) => e.user_id).filter(Boolean)).size,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!bar_id || !sessao_id || !evento_tipo || !evento_nome) {
      return NextResponse.json({
        success: false,
        error: 'Campos obrigatórios: bar_id, sessao_id, evento_tipo, evento_nome'
      }, { status: 400 })
    }
    ...
    const { data: evento, error } = await supabase
      ...
      .insert({
        ...
        elemento_alvo,
        dados_evento,
        tempo_gasto_segundos: tempo_gasto_segundos ? parseInt(tempo_gasto_segundos as string) : null,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const eventosProcessados = eventos.map((evento: EventoInput) => ({
      ...
      tempo_gasto_segundos: evento.tempo_gasto_segundos ? parseInt(evento.tempo_gasto_segundos as string) : null,
