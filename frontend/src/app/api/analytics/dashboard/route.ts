...
export async function GET(request: NextRequest) {
  try {
    ...
    const metricasResumo = {
      usuarios_ativos_hoje: eventos?.filter((e: UsuarioEvento) => 
        e.timestamp_evento >= new Date().toISOString().split('T')[0] && e.user_id
      ).map((e: UsuarioEvento) => e.user_id).filter((v: string, i: number, a: string[]) => a.indexOf(v) === i).length || 0,
      
      sessoes_hoje: eventos?.filter((e: UsuarioEvento) => 
        e.timestamp_evento >= new Date().toISOString().split('T')[0]
      ).length || 0,
      
      tempo_medio_sessao: eventos?.length ? 
        Math.round(eventos.reduce((acc: number, e: UsuarioEvento) => acc + (e.tempo_gasto_segundos || 0), 0) / eventos.length) : 0,
        
      taxa_mobile: eventos?.length ? 
        Math.round((eventos.filter((e: UsuarioEvento) => e.dispositivo_tipo === 'mobile').length / eventos.length) * 100) : 0
    }

    // === KPIs RESUMIDOS ===
    const kpisResumo = {
      total_kpis: kpis?.length || 0,
      kpis_atingidos: kpis?.filter((k: KPI) => k.status_meta === 'atingido').length || 0,
      kpis_criticos: kpis?.filter((k: KPI) => k.status_meta === 'critico').length || 0,
      percentual_sucesso: kpis?.length ? 
        Math.round((kpis.filter((k: KPI) => k.status_meta === 'atingido').length / kpis.length) * 100) : 0
    }

    // === PERFORMANCE RESUMIDA ===
    const performanceResumo = {
      tempo_resposta_medio: performance?.length ? 
        Math.round(performance.reduce((acc: number, p: Performance) => acc + p.tempo_resposta_ms, 0) / performance.length) : 0,
      
      total_requests: performance?.length || 0,
      taxa_erro: performance?.length ? 
        Math.round((performance.filter((p: Performance) => p.status_code >= 400).length / performance.length) * 100) : 0,
      
      endpoints_mais_lentos: performance
        ?.sort((a: Performance, b: Performance) => b.tempo_resposta_ms - a.tempo_resposta_ms)
        .slice(0, 5)
        .map((p: Performance) => ({
    ...
    // === EVENTOS POR TIPO ===
    const eventosPorTipo = eventos?.reduce((acc: EventosAccumulator, evento: UsuarioEvento) => {
    ...
    const alertasResumo = {
      ...
      criticos: alertas?.filter((a: Alerta) => a.severidade === 'critical').length || 0,
      warnings: alertas?.filter((a: Alerta) => a.severidade === 'warning').length || 0,
      errors: alertas?.filter((a: Alerta) => a.severidade === 'error').length || 0
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        alertas_criticos: alertas?.filter((a: Alerta) => a.severidade === 'critical').slice(0, 5) || [],
        
        performance_trends: performance?.map((p: Performance) => ({
