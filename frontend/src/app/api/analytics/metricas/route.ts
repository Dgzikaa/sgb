...
export async function GET(request: NextRequest) {
  try {
    ...
    metricas?.forEach((metrica: unknown) => {
    ...
    console.error('ÂÅ’ Erro ao buscar mÃ¡Â©tricas:', error)
    return NextResponse.json({
      success: false,
      error: 'Erro ao buscar mÃ¡Â©tricas do sistema',
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!bar_id || !tipo_metrica || !categoria || !nome_metrica || valor === undefined) {
      return NextResponse.json({
        success: false,
        error: 'Campos obrigatÃ¡Â³rios: bar_id, tipo_metrica, categoria, nome_metrica, valor'
      }, { status: 400 })
    }

    const supabase = await getAdminClient()

    // Calcular variaÃ¡Â§Ã¡Â£o percentual se valor_anterior fornecido
    ...
    return NextResponse.json({
      success: true,
      data: metrica,
      message: 'MÃ¡Â©trica registrada com sucesso'
    })

  } catch (error) {
    console.error('ÂÅ’ Erro ao registrar mÃ¡Â©trica:', error)
    return NextResponse.json({
      success: false,
      error: 'Erro ao registrar mÃ¡Â©trica',
      details: error instanceof Error ? error.message : 'Erro desconhecido'
    }, { status: 500 })
  }
} 
