...
export default function ChecklistsFuncionario() {
  ...
  // Mock data - filtrado por usuário
  const checklistsMock: ChecklistFuncionario[] = [
    {
      ...
      setor: 'cozinha',
      tipo: 'abertura',
      descricao: 'Checklist de abertura da cozinha - procedimentos obrigatórios',
      tempo_estimado: 45,
      ...
      secoes: [
        {
          ...
          itens: [
            ...
            },
            {
              ...
              descricao: 'Testar todas as bocas do fogão',
              tipo: 'sim_nao',
              obrigatorio: true,
              status: 'pendente'
            },
        ...
        },
        {
          ...
          itens: [
            ...
            },
            {
              ...
              descricao: 'Como você avalia a limpeza geral?',
              tipo: 'avaliacao',
              obrigatorio: false,
              status: 'pendente'
            },
    ...
    },
    {
      ...
      secoes: [
        {
          ...
          itens: [
            ...
            },
            {
              ...
              descricao: 'Tirar foto do exaustor após limpeza',
              tipo: 'foto_camera',
              obrigatorio: false,
              status: 'pendente',
        ...
        },
        {
          ...
          itens: [
            {
              id: 'estoque_seco',
              titulo: 'Organização do estoque seco',
            ...
            },
            {
              ...
              titulo: 'Avaliação geral da limpeza',
              descricao: 'Como você avalia a limpeza semanal?',
              tipo: 'avaliacao',
              obrigatorio: false,
              status: 'pendente'
            },
            {
              ...
              descricao: 'Alguma observação sobre a limpeza semanal?',
              tipo: 'texto',
              obrigatorio: false,
              status: 'pendente',
              opcoes: { placeholder: 'Digite suas observações...' }
            },
            {
              id: 'assinatura_supervisor',
              titulo: 'Assinatura do supervisor',
              descricao: 'Confirme a inspeção da limpeza semanal',
              tipo: 'assinatura',
              obrigatorio: true,
              status: 'pendente'
            }
     ...
     {
      id: '3',
      nome: 'Segurança Semanal',
      setor: 'seguranca',
      tipo: 'seguranca',
      descricao: 'Verificações de segurança e prevenção de incêndios',
      tempo_estimado: 60,
      responsavel: 'João Silva',
      status: 'pendente',
      prioridade: 'alta',
      secoes: [
        {
          ...
          nome: 'Prevenção de Incêndios',
          cor: 'bg-red-500',
          itens: [
            {
              id: 'extintores',
              titulo: 'Verificação dos extintores',
              descricao: 'Conferir pressão e validade dos extintores',
              tipo: 'sim_nao',
              obrigatorio: true,
              status: 'pendente'
            },
            {
              ...
              descricao: 'Verificar se as saídas estão livres',
              tipo: 'sim_nao',
              obrigatorio: true,
              status: 'pendente'
            },
        ...
        {
          id: 'equipamentos_gas',
          nome: 'Equipamentos de Gás',
          cor: 'bg-yellow-500',
          itens: [
            {
              id: 'vazamentos_gas',
              titulo: 'Verificação de vazamentos',
              descricao: 'Usar detector ou água com sabão',
              tipo: 'sim_nao',
              obrigatorio: true,
              status: 'pendente'
            },
            {
              id: 'valvulas_gas',
              titulo: 'Funcionamento das válvulas',
              descricao: 'Testar abertura e fechamento das válvulas',
              tipo: 'sim_nao',
              obrigatorio: true,
              status: 'pendente'
            },
            {
              ...
              titulo: 'Assinatura do responsável',
              descricao: 'Confirme as verificações de segurança',
              tipo: 'assinatura',
              obrigatorio: true,
              status: 'pendente'
            }
  ...
  const enviarChecklist = async () => {
    ...
    try {
      ...
      // Recarregar checklists realizados também
      if (aba === 'realizados') {
        carregarChecklistsRealizados()
      }
  ...
  // Função para processar foto capturada
  ...
  // Função para abrir pad de assinatura
  const abrirAssinaturaPad = (secaoId: string, itemId: string) => {
    console.log('Abrindo pad de assinatura', { secaoId, itemId })
  ...
  const handleAssinaturaCapturada = (result: any) => {
    ...
    if (!itemAssinaturaAtual) {
      console.error('itemAssinaturaAtual é null!')
      return
    }
          ...
          {item.tipo === 'avaliacao' && (
            <div className="flex justify-center gap-3">
              {[1, 2, 3, 4, 5].map((rating) => (
                ...
                >
                  {rating === 1 ? 'Å¾' : rating === 2 ? 'Å¾' : rating === 3 ? '¢' : rating === 4 ? 'Å' : 'Å'}
                </button>
              ))}
            </div>
          )}
          ...
          {item.tipo === 'foto_camera' && (
            <div className="space-y-3">
              ...
              >
                ...
                {item.valor ? 'Foto Capturada Sim' : 'Tirar Foto'}
              </Button>
                  ...
                  {typeof item.valor === 'string' && (item.valor.startsWith('blob:') || item.valor.startsWith('http')) && (
                    <div className="space-y-2">
                      ...
                      {item.valor.startsWith('http') && (
                        <p className="text-xs text-green-600">Salvo na nuvem</p>
                      )}
                    </div>
                  )}
                </div>
          ...
          {item.tipo === 'assinatura' && (
            <div className="space-y-3">
              <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-all ${
                ...
                {item.valor ? (
                  <div>
                    ...
                    <p className="text-green-600 font-medium">Assinatura capturada Sim</p>
                  </div>
                ) : (
                  <div>
              ...
              {!item.valor && (
                <Button
                  onClick={() => {
                    console.log('Botão "Capturar Assinatura" clicado', { secaoId, itemId: item.id, tipo: item.tipo })
              ...
              {item.valor && (
                <div className="bg-green-50 border border-green-200 p-4 rounded-lg text-center space-y-2">
                  <p className="text-green-700 font-medium">Assinatura salva com sucesso</p>
          ...
          {aba === 'pendentes' ? (
            checklists.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">Você está em dia com suas verificações! Ã°Å¸Å½â€°</p>
              </div>
            ) : (
            ...
            loadingRealizados ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">Complete alguns checklists para vê-los aqui Ã°Å¸â€œÅ </p>
              </div>
            ) : (
