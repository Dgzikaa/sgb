...
export default function AgendamentoPage() {
  ...
  // 1. Coloque as funções useCallback antes dos useEffect
    ...
    try {
      ...
      if (savedData) {
        try {
          ...
          // Verificar se é versão antiga (sem version)
          if (!parsed.version) {
            console.log('🔄 Migrando dados antigos...');
            isMigration = true;
            // Tentar carregar backup se dados antigos estiverem corrompidos
            loadBackup();
            return;
          }
          ...
          if (parsed.pagamentos && Array.isArray(parsed.pagamentos)) {
            ...
            if (isFirstLoad) {
              toast({
                ...
                description: `${parsed.pagamentos.length} pagamento(s) carregado(s)`,
              });
          ...
          // Tentar carregar backup se dados estiverem corrompidos
          loadBackup();
        }
      }
    } catch (error) {
  ...
  }, [pagamentos, toast, createBackup]);

  // 2. Depois, os useEffect que usam essas funções
  useEffect(() => {
    ...
    try {
      ...
      if (backupData) {
        ...
        toast({
          ...
          description: `${parsed.pagamentos?.length || 0} pagamentos restaurados`,
        });
      } else {
        toast({
          title: 'Nenhum backup encontrado',
          description: 'Não há backup para restaurar',
          variant: 'destructive',
        });
      }
    } catch (error) {
  ...
  const createBackup = useCallback(() => {
    try {
      ...
      toast({
        title: 'Backup criado',
        description: `${pagamentos.length} pagamentos salvos`,
      });
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <Dialog open={modalPixKey} onOpenChange={setModalPixKey}>
          <DialogContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            ...
            <DialogFooter>
              <Button onClick={cancelarAtualizacaoPix} className="btn-outline">
                Cancelar
              </Button>
              <Button
        ...
        <Dialog open={modalEdicao} onOpenChange={setModalEdicao}>
          <DialogContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 max-w-2xl">
            ...
            <DialogFooter>
              <Button onClick={cancelarEdicao} className="btn-outline">
                Cancelar
              </Button>
