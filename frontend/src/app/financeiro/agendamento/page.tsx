...
export default function AgendamentoPage() {
  ...
  const [pagamentoEmEdicao, setPagamentoEmEdicao] = useState<PagamentoAgendamento | null>(null)
  ...
  const [isCadastrandoStakeholder, setIsCadastrandoStakeholder] = useState(false)
  
  // Modal de atualização de chave PIX
  const [modalPixKey, setModalPixKey] = useState(false)
  const [stakeholderParaPix, setStakeholderParaPix] = useState<Stakeholder | null>(null)
  ...
  const loadCategoriasECentrosCusto = async () => {
    ...
    try {
      ...
      const centrosCustoResponse = await fetch('/api/financeiro/nibo/centros-custo')
  ...
  const saveToLocalStorage = () => {
    try {
      ...
      localStorage.setItem('sgb_backup_count', (parseInt(backupCount) + 1).toString())
    ...
    try {
      ...
      if (savedData) {
        ...
        if (oldData) {
          ...
          localStorage.setItem(STORAGE_KEYS.PAGAMENTOS, JSON.stringify(migratedData))
  ...
  const adicionarPagamento = () => {
    if (!novoPagamento.cpf_cnpj || !novoPagamento.nome_beneficiario || !novoPagamento.valor || !novoPagamento.data_pagamento || !novoPagamento.categoria_id || !novoPagamento.centro_custo_id) {
      toast({
  ...
  const verificarStakeholder = async (pagamento: PagamentoAgendamento): Promise<Stakeholder> => {
    try {
      ...
      const cpfCnpj = pagamento.cpf_cnpj || pagamento.chave_pix || '00000000000'
      const response = await fetch(`/api/financeiro/nibo/stakeholders?q=${cpfCnpj}`)
  ...
  const agendarNoNibo = async (pagamento: PagamentoAgendamento, stakeholder: Stakeholder) => {
    try {
      // Formatar valor corretamente
      const valorNumerico = parseFloat(pagamento.valor.replace('R$', '').replace(',', '.').trim())
  ...
  const salvarEdicao = () => {
    ...
    if (!pagamentoEmEdicao.data_pagamento) camposObrigatorios.push('Data de Pagamento')
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'erro':
        return <Badge variant="secondary" className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">Erro</Badge>
      default:
  ...
  const buscarStakeholder = async (document: string) => {
    ...
    try {
      const response = await fetch(`/api/financeiro/nibo/stakeholders?q=${documentoLimpo}`)
      const data = await response.json()
  ...
  const cancelarCadastroStakeholder = () => {
    setModalStakeholder(false)
    setStakeholderEmCadastro({ document: '', name: '' })
    setNovoPagamento(prev => ({ ...prev, cpf_cnpj: '', nome_beneficiario: '' }))
  }

  // Funções para atualização de chave PIX
  const verificarChavePix = async (stakeholder: Stakeholder) => {
    try {
      const response = await fetch(`/api/financeiro/nibo/stakeholders/${stakeholder.id}`)
      const data = await response.json()
  ...
  const formatarPixKey = (valor: string, tipo: number): string => {
    ...
    switch (tipo) {
      ...
      case 4: // Chave Aleatória (UUID)
        ...
        if (apenasDigitos.length <= 12) return `${apenasDigitos.slice(0, 8)}-${apenasDigitos.slice(8)}`
        if (apenasDigitos.length <= 16) return `${apenasDigitos.slice(0, 8)}-${apenasDigitos.slice(8, 12)}-${apenasDigitos.slice(12)}`
        if (apenasDigitos.length <= 20) return `${apenasDigitos.slice(0, 8)}-${apenasDigitos.slice(8, 12)}-${apenasDigitos.slice(12, 16)}-${apenasDigitos.slice(16)}`
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
              ...
              <Card className="bg-white dark:bg-gray-800 border-0 shadow-lg sticky top-6">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                    ...
                    <span className="text-lg font-bold text-yellow-600 dark:text-yellow-400">{metricas.pendentes}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-blue-700 dark:text-blue-300">Agendados</span>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-green-700 dark:text-green-300">Aprovados</span>
                    </div>
                    <span className="text-lg font-bold text-green-600 dark:text-green-400">{metricas.aprovados}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-red-700 dark:text-red-300">Erros</span>
                    </div>
                    <span className="text-lg font-bold text-red-600 dark:text-red-400">{metricas.erros}</span>
                  </div>
              ...
              <Tabs defaultValue="manual" className="space-y-6">
                ...
                <TabsContent value="manual">
                  <Card className="bg-white dark:bg-gray-800 border-0 shadow-lg">
                    <CardHeader>
                      <CardTitle className="text-gray-900 dark:text-white">Adicionar Pagamento Manual</CardTitle>
                      <CardDescription className="text-gray-600 dark:text-gray-400">Preencha os dados do pagamento para agendamento</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <Label htmlFor="cpf_cnpj" className="text-gray-700 dark:text-gray-300">CPF/CNPJ *</Label>
                          <div className="flex gap-2 mt-1">
                            <Input
                              id="cpf_cnpj"
                              value={novoPagamento.cpf_cnpj}
                              onChange={(e) => setNovoPagamento(prev => ({ ...prev, cpf_cnpj: formatarDocumento(e.target.value) }))}
                            ...
                            />
                            <Button
                              onClick={() => buscarStakeholder(novoPagamento.cpf_cnpj)}
                              type="button"
                              size="sm"
                        ...
                        <div>
                          <Label htmlFor="nome" className="text-gray-700 dark:text-gray-300">Nome do Beneficiário *</Label>
                          <Input
                            id="nome"
                            value={novoPagamento.nome_beneficiario}
                            onChange={(e) => setNovoPagamento(prev => ({ ...prev, nome_beneficiario: e.target.value }))}
                        ...
                        <div>
                          <Label htmlFor="chave_pix" className="text-gray-700 dark:text-gray-300">Chave PIX</Label>
                          <Input
                            id="chave_pix"
                            value={novoPagamento.chave_pix}
                            onChange={(e) => setNovoPagamento(prev => ({ ...prev, chave_pix: e.target.value }))}
                        ...
                        <div>
                          <Label htmlFor="valor" className="text-gray-700 dark:text-gray-300">Valor *</Label>
                          <Input
                            id="valor"
                        ...
                        <div>
                          <Label htmlFor="data_pagamento" className="text-gray-700 dark:text-gray-300">Data de Pagamento *</Label>
                          <Input
                            id="data_pagamento"
                            type="date"
                            value={novoPagamento.data_pagamento}
                            onChange={(e) => setNovoPagamento(prev => ({ ...prev, data_pagamento: e.target.value }))}
                            className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white"
                          />
                        </div>
                        <div>
                          <Label htmlFor="data_competencia" className="text-gray-700 dark:text-gray-300">Data de Competência</Label>
                          <Input
                            id="data_competencia"
                            type="date"
                            value={novoPagamento.data_competencia}
                            onChange={(e) => setNovoPagamento(prev => ({ ...prev, data_competencia: e.target.value }))}
                            className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white"
                          />
                        </div>
                        <div>
                          ...
                          <SelectWithSearch
                            ...
                            onValueChange={(value) => setNovoPagamento(prev => ({ ...prev, categoria_id: value || '' }))}
                        ...
                        <div>
                          <Label htmlFor="centro_custo" className="text-gray-700 dark:text-gray-300">Centro de Custo *</Label>
                          <SelectWithSearch
                            ...
                            onValueChange={(value) => setNovoPagamento(prev => ({ ...prev, centro_custo_id: value || '' }))}
                    ...
                    <CardContent>
                      {pagamentos.length === 0 ? (
                        <div className="text-center py-12">
                          ...
                          <p className="text-gray-500 dark:text-gray-400">Nenhum pagamento na lista</p>
                          <p className="text-sm text-gray-400 dark:text-gray-500">Adicione pagamentos manualmente para começar</p>
                        </div>
                      ) : (
                            ...
                            >
                              <div className="flex-1">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600 dark:text-gray-400">
                                  <div>
                                    <span className="font-medium">CPF/CNPJ:</span> {pagamento.cpf_cnpj ? formatarDocumento(pagamento.cpf_cnpj) : 'Não informado'}
                                  </div>
                                  <div>
                                    <span className="font-medium">Valor:</span> {pagamento.valor}
                                  </div>
                                  <div>
                                    <span className="font-medium">Data:</span> {new Date(pagamento.data_pagamento).toLocaleDateString('pt-BR')}
                                  </div>
                                  <div>
                                    <span className="font-medium">Chave PIX:</span> {pagamento.chave_pix || 'Não informada'}
                                  </div>
                                </div>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600 dark:text-gray-400 mt-2">
                                  <div>
                                    <span className="font-medium">Categoria:</span> {categorias.find((c: any) => c.id === pagamento.categoria_id)?.nome || 'Não selecionada'}
                                  </div>
                                  <div>
                                    <span className="font-medium">Centro de Custo:</span> {centrosCusto.find((c: any) => c.id === pagamento.centro_custo_id)?.nome || 'Não selecionado'}
                                  </div>
                                </div>
                                {pagamento.descricao && (
                                ...
                                <div className="mt-2 text-xs text-gray-500 dark:text-gray-500">
                                  Criado: {new Date(pagamento.created_at).toLocaleString('pt-BR')} | 
                                  Atualizado: {new Date(pagamento.updated_at).toLocaleString('pt-BR')}
                                </div>
                              </div>
                              <div className="flex items-center gap-2 ml-4">
                                <Button
                                  ...
                                  size="sm"
                                  className="bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white h-8 w-8 p-0 rounded-lg shadow-sm"
                                ...
                                {pagamento.status !== 'pendente' && (
                                  <Button
                                    onClick={() => redefinirStatus(pagamento.id)}
                                    size="sm"
                                    className="bg-orange-600 hover:bg-orange-700 dark:bg-orange-500 dark:hover:bg-orange-600 text-white h-8 w-8 p-0 rounded-lg shadow-sm"
        ...
        <Dialog open={modalStakeholder} onOpenChange={setModalStakeholder}>
          <DialogContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-4 py-4">
              <div>
                <Label htmlFor="modal-document" className="text-gray-700 dark:text-gray-300">
                  CPF/CNPJ
                </Label>
                <Input
                  id="modal-document"
                  ...
                  disabled
                  className="mt-1 bg-gray-100 dark:bg-gray-600 border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400"
                />
              </div>
              <div>
                <Label htmlFor="modal-name" className="text-gray-700 dark:text-gray-300">
                  Nome *
                </Label>
                <Input
                  id="modal-name"
                  value={stakeholderEmCadastro.name}
                  onChange={(e) => setStakeholderEmCadastro(prev => ({ ...prev, name: e.target.value }))}
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={isCadastrandoStakeholder || !stakeholderEmCadastro.name.trim()}
                className="btn-primary"
              >
        ...
        <Dialog open={modalPixKey} onOpenChange={setModalPixKey}>
          <DialogContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-4 py-4">
              <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <p className="text-sm text-blue-600 dark:text-blue-400">
                  <strong>CPF/CNPJ:</strong> {stakeholderParaPix?.document ? formatarDocumento(stakeholderParaPix.document) : 'Não informado'}
                </p>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="same-as-document" className="text-gray-700 dark:text-gray-300">
                  Chave PIX é o mesmo CPF/CNPJ?
                </Label>
              </div>

              <div>
                <Label htmlFor="modal-pix-type" className="text-gray-700 dark:text-gray-300">
                  Tipo de Chave PIX *
                </Label>
                <select
                  id="modal-pix-type"
                  ...
                  className="mt-1 w-full bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-md"
                >
              ...
              <div>
                <Label htmlFor="modal-pix-key" className="text-gray-700 dark:text-gray-300">
                  Chave PIX *
                </Label>
                <Input
                  id="modal-pix-key"
                  ...
                  onChange={(e) => {
                    ...
                    const formattedValue = pixKeyData.pixKeyType === 2 ? e.target.value : formatarPixKey(rawValue, pixKeyData.pixKeyType)
                    setPixKeyData(prev => ({ ...prev, pixKey: formattedValue }))
                  }}
                  placeholder={
                    pixKeyData.pixKeyType === 1 ? "(61) 99999-9999" :
                    pixKeyData.pixKeyType === 2 ? "email@exemplo.com" :
                    pixKeyData.pixKeyType === 3 ? "000.000.000-00" :
            ...
            <DialogFooter>
              <Button
                onClick={cancelarAtualizacaoPix}
                className="btn-outline"
              >
                Cancelar
              </Button>
          ...
          <DialogContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 max-w-2xl">
            ...
            {pagamentoEmEdicao && (
              <div className="space-y-4 py-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="edit-cpf_cnpj" className="text-gray-700 dark:text-gray-300">CPF/CNPJ</Label>
                    <Input
                      id="edit-cpf_cnpj"
                      value={formatarDocumento(pagamentoEmEdicao.cpf_cnpj)}
                      onChange={(e) => setPagamentoEmEdicao(prev => prev ? { ...prev, cpf_cnpj: removerFormatacao(e.target.value) } : null)}
                      placeholder="000.000.000-00 ou 00.000.000/0000-00"
                      className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                    />
                  </div>
                  <div>
                    <Label htmlFor="edit-nome" className="text-gray-700 dark:text-gray-300">Nome do Beneficiário</Label>
                    <Input
                      id="edit-nome"
                      value={pagamentoEmEdicao.nome_beneficiario}
                      onChange={(e) => setPagamentoEmEdicao(prev => prev ? { ...prev, nome_beneficiario: e.target.value } : null)}
                      placeholder="Nome completo"
                      className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                    />
                  </div>
                  <div>
                    <Label htmlFor="edit-chave_pix" className="text-gray-700 dark:text-gray-300">Chave PIX</Label>
                    <Input
                      id="edit-chave_pix"
                      value={pagamentoEmEdicao.chave_pix}
                      onChange={(e) => setPagamentoEmEdicao(prev => prev ? { ...prev, chave_pix: e.target.value } : null)}
                      placeholder="CPF, CNPJ, email ou telefone"
                      className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                    />
                  </div>
                  <div>
                    <Label htmlFor="edit-valor" className="text-gray-700 dark:text-gray-300">Valor</Label>
                    <Input
                      id="edit-valor"
                      value={pagamentoEmEdicao.valor}
                      onChange={(e) => setPagamentoEmEdicao(prev => prev ? { ...prev, valor: e.target.value } : null)}
                      placeholder="R$ 0,00"
                      className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                    />
                  </div>
                  <div>
                    <Label htmlFor="edit-data_pagamento" className="text-gray-700 dark:text-gray-300">Data de Pagamento</Label>
                    <Input
                      id="edit-data_pagamento"
                      type="date"
                      value={pagamentoEmEdicao.data_pagamento}
                      onChange={(e) => setPagamentoEmEdicao(prev => prev ? { ...prev, data_pagamento: e.target.value } : null)}
                      className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white"
                    />
                  </div>
                  <div>
                    <Label htmlFor="edit-data_competencia" className="text-gray-700 dark:text-gray-300">Data de Competência</Label>
                    <Input
                      id="edit-data_competencia"
                      type="date"
                      value={pagamentoEmEdicao.data_competencia}
                      onChange={(e) => setPagamentoEmEdicao(prev => prev ? { ...prev, data_competencia: e.target.value } : null)}
                      className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white"
                    />
                  </div>
                                                    <div>
                                    <Label htmlFor="edit-categoria" className="text-gray-700 dark:text-gray-300">Categoria *</Label>
                                    <SelectWithSearch
                                      options={categorias.map((categoria: any) => ({
                                        value: categoria.id,
                                        label: categoria.nome
                                      ...
                                      onValueChange={(value) => setPagamentoEmEdicao(prev => prev ? { ...prev, categoria_id: value || '' } : null)}
                                      placeholder="Selecione uma categoria"
                                      searchPlaceholder="Buscar categoria..."
                                      className="mt-1"
                                    />
                                  ...
                                  <div>
                                    <Label htmlFor="edit-centro_custo" className="text-gray-700 dark:text-gray-300">Centro de Custo *</Label>
                                    <SelectWithSearch
                                      options={centrosCusto.map((centro: any) => ({
                                        value: centro.id,
                                        label: centro.nome
                                      ...
                                      onValueChange={(value) => setPagamentoEmEdicao(prev => prev ? { ...prev, centro_custo_id: value || '' } : null)}
                                      placeholder="Selecione um centro de custo"
                                      searchPlaceholder="Buscar centro de custo..."
                                      className="mt-1"
                                    />
                ...
                <div>
                  <Label htmlFor="edit-descricao" className="text-gray-700 dark:text-gray-300">Descrição</Label>
                  <Textarea
                    id="edit-descricao"
                    value={pagamentoEmEdicao.descricao}
                    onChange={(e) => setPagamentoEmEdicao(prev => prev ? { ...prev, descricao: e.target.value } : null)}
                    placeholder="Descrição do pagamento"
                    className="mt-1 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                  />
                </div>
