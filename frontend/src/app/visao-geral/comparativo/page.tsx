...
export default function ComparativoPage() {
  ...
  const [tipoComparacao, setTipoComparacao] =
    useState<TipoComparacao>('datas-especificas');
  ...
  const [recorrenciaClientes, setRecorrenciaClientes] =
    useState<RecorrenciaData | null>(null);
  ...
  const [dadosArtistas, setDadosArtistas] = useState<{
    artista1: ArtistaStats | null;
    artista2: ArtistaStats | null;
  }>({
    artista1: null,
    artista2: null,
  });
    ...
    try {
      ...
      if (!error && data) {
        const artistasUnicos = [
          ...new Set(
            data.map((item: { nome_artista: string }) => item.nome_artista)
          ),
        ] as string[];
  ...
  }, [buscarArtistas, tipoComparacao, selectedBar]);
  ...
  const buscarDadosData = async (data: string): Promise<DadosComparacao> => {
    ...
    try {
      ...
      console.log(
        `📦 Total de registros Yuzer encontrados: ${yuzerTotalData.length}`
      );

      const yuzerFaturamentoTotal =
        yuzerTotalData?.reduce(
          (sum: number, item: YuzerRegistro) =>
      ...
      const contahubFaturamentoTotal =
        fatHoraTotalData?.reduce(
          (sum: number, item: ContaHubRegistro) =>
      ...
      const isYuzerDay =
        yuzerRegistrosTotal > 0 &&
        yuzerFaturamentoTotal > contahubFaturamentoTotal;
      const isContaHubDay = contahubRegistrosTotal > 0 && !isYuzerDay;

      console.log(`📊 DETECÇÃO DE SISTEMA para ${data}:`);
      console.log(
        `   🍺 Yuzer: ${yuzerRegistrosTotal} registros, R$ ${yuzerFaturamentoTotal.toFixed(2)} (TOTAL COMPLETO)`
      );
      console.log(
        `   💻 ContaHub: ${contahubRegistrosTotal} registros, R$ ${contahubFaturamentoTotal.toFixed(2)} (TOTAL COMPLETO)`
      );
      console.log(
        `   🎯 Sistema detectado: ${isYuzerDay ? 'YUZER' : isContaHubDay ? 'CONTAHUB' : 'HÍBRIDO'}`
      );
      ...
      if (isYuzerDay) {
        ...
        console.log(
          `🍺 DIA YUZER DETECTADO - Usando faturamento completo: R$ ${yuzerFaturamentoTotal.toFixed(2)}`
        );
        ...
        if (contahubFaturamentoTotal > 0) {
          console.log(
            `💻 Adicionando dados complementares do ContaHub: R$ ${contahubFaturamentoTotal.toFixed(2)}`
          );
        ...
        console.log(
          `💻 Buscando faturamento do ContaHub/Pagamentos para ${data}...`
        );
        ...
        if (yuzerEstatisticas && yuzerEstatisticas.length > 0) {
          faturamentoYuzer = yuzerEstatisticas.reduce(
            (sum: number, item: YuzerEstatistica) =>
              sum + (parseFloat(item.total) || 0),
            0
          );
          console.log(
            `🍺 Faturamento Yuzer (estatísticas): R$ ${faturamentoYuzer.toFixed(2)}`
          );
        }
        ...
        while (hasMoreData) {
          ...
          const chunkSympla = fatData
            .filter((item: FaturamentoItem) =>
              item.origem?.toLowerCase().includes('sympla')
            )
            .reduce(
              (sum: number, item: FaturamentoItem) =>
          ...
          console.log(
            `💻 Chunk processado: ${fatData.length} registros, Sympla: R$ ${chunkSympla.toFixed(2)}, ContaHub: R$ ${chunkContaHub.toFixed(2)}`
          );
      ...
      console.log(
        `💰 Buscando faturamento adicional na tabela 'periodo' para ${data}...`
      );
      ...
      try {
        ...
        if (periodoData && periodoData.length > 0) {
          ...
          const dadosValidos = periodoData.filter(
            (item: PeriodoItem) => item.liquido_netto != null
          );
          if (dadosValidos.length > 0) {
            faturamentoPeriodo = dadosValidos.reduce(
              (sum: number, item: PeriodoItem) =>
                sum + (parseFloat(item.liquido_netto) || 0),
              0
            );
            console.log(
              `💰 Faturamento período encontrado: R$ ${faturamentoPeriodo.toFixed(2)} (${dadosValidos.length} registros)`
            );
            faturamento += faturamentoPeriodo;
          } else {
            console.log(
              `💰 Nenhum faturamento período válido encontrado para ${data}`
            );
          }
        } else {
      ...
      console.log(
        `🎫 Buscando faturamento de bilheteria Sympla para ${data}...`
      );
      ...
      try {
        ...
        if (bilheteriaData && bilheteriaData.length > 0) {
          faturamentoBilheteriaSympla = bilheteriaData.reduce(
            (sum: number, item: BilheteriaItem) =>
              sum + (parseFloat(item.total_liquido) || 0),
            0
          );
          console.log(
            `🎫 Faturamento bilheteria Sympla encontrado: R$ ${faturamentoBilheteriaSympla.toFixed(2)} (${bilheteriaData.length} registros)`
          );
          faturamento += faturamentoBilheteriaSympla;
          faturamentoSympla += faturamentoBilheteriaSympla; // Adicionar ao total Sympla
        } else {
          console.log(
            `🎫 Nenhum faturamento de bilheteria Sympla encontrado para ${data}`
          );
        }
      } catch (error) {
      ...
      console.log(
        `💰 FATURAMENTO TOTAL FINAL: R$ ${faturamento.toFixed(2)} (Yuzer: R$ ${faturamentoYuzer.toFixed(2)} + ContaHub: R$ ${faturamentoContaHub.toFixed(2)} + Sympla: R$ ${faturamentoSympla.toFixed(2)} + Período: R$ ${faturamentoPeriodo.toFixed(2)} + Bilheteria: R$ ${faturamentoBilheteriaSympla.toFixed(2)})`
      );

      if (faturamento === 0) {
        console.log(
          `⚠️ ATENÇÃO: Nenhum faturamento encontrado para ${data}. Pode ser uma data futura sem dados reais.`
        );
      }
      ...
      if (isYuzerDay) {
        ...
        try {
          ...
          if (symplaData && symplaData.length > 0) {
            ...
            console.log(
              `🎫 Check-ins Sympla encontrados para dia Yuzer: ${clientes}`
            );
          } else {
            console.log(
              `🎫 Nenhum check-in Sympla encontrado para dia Yuzer ${data}`
            );
            // Fallback para pessoas_diario_corrigido se não houver Sympla
            console.log(
              `👥 Fallback: Buscando clientes na tabela 'pessoas_diario_corrigido' para ${data}...`
            );
            try {
              ...
              console.log(
                `❌ Erro ao buscar fallback pessoas_diario_corrigido:`,
                error
              );
            }
          }
          ...
          console.log(
            `❌ Erro ao buscar check-ins Sympla para dia Yuzer:`,
            error
          );
        }
      } else {
        ...
        console.log(
          `👥 DIA CONTAHUB - Buscando clientes na tabela 'pessoas_diario_corrigido' para ${data}...`
        );
        try {
          ...
          if (pessoasData) {
            ...
            console.log(
              `⚠️ Nenhum dado de clientes encontrado na tabela pessoas_diario_corrigido para ${data}`
            );
          }
        } catch (error) {
          console.log(
            `❌ Erro ao buscar clientes pessoas_diario_corrigido:`,
            error
          );
        }
          ...
          if (symplaData && symplaData.length > 0) {
            ...
            if (clientes === 0) {
              ...
              if (clientesSympla > clientes) {
                console.log(
                  `📊 Sympla tem mais clientes (${clientesSympla}) que pessoas_diario_corrigido (${clientes}), usando Sympla`
                );
                clientes = clientesSympla;
                clientesSource = 'sympla_checkins_maior';
              } else {
                console.log(
                  `📊 Mantendo pessoas_diario_corrigido (${clientes}) maior que Sympla (${clientesSympla})`
                );
      ...
      console.log(
        `   ✅ Dados válidos? ${faturamento > 0 || clientes > 0 ? 'SIM' : 'NÃO'}`
      );
      ...
      if (data && data.length === 10) {
        try {
          ...
          const response = await fetch(
            `/api/dashboard/reservas-getin?data=${data}&tipo=dia`
          );
          if (response.ok) {
      ...
      if (tempoData && tempoData.length > 0) {
        console.log(
          `⏱️ Dados de tempo encontrados: ${tempoData.length} registros`
        );
        ...
        tempoMedioBar =
          temposBar.length > 0
            ? temposBar.reduce(
                (sum: number, item: TempoItem) => sum + parseFloat(item.t1_t2),
                0
              ) / temposBar.length
            : 0;
        ...
        tempoMedioCozinha =
          temposCozinha.length > 0
            ? temposCozinha.reduce(
                (sum: number, item: TempoItem) => sum + parseFloat(item.t1_t2),
                0
              ) / temposCozinha.length
            : 0;

        console.log(
          `⏱️ Tempo médio cozinha: ${tempoMedioCozinha.toFixed(1)}min (${temposCozinha.length} registros)`
        );
        console.log(
          `⏱️ Tempo médio bar: ${tempoMedioBar.toFixed(1)}min (${temposBar.length} registros)`
        );

        if (temposCozinha.length === 0 && temposBar.length === 0) {
          console.log(
            `⚠️ Nenhum tempo válido encontrado nos ${tempoData.length} registros para ${data}`
          );
          console.log(
            `📋 Grupos encontrados: ${[...new Set(tempoData.map((t: TempoItem) => t.grp_desc))].slice(0, 5).join(', ')}`
          );
        }
      } else {
      ...
      console.log(
        `⏰ Buscando faturamento e clientes por hora para ${data}...`
      );
      ...
      if (isYuzerDay) {
        ...
        console.log(
          `🍺 Buscando TODOS os dados por hora do Yuzer para ${data}...`
        );
        ...
        console.log(
          `📦 Total de registros por hora encontrados: ${yuzerHoraData.length}`
        );
        ...
        if (yuzerHoraData && yuzerHoraData.length > 0) {
          ...
          console.log(
            `🎫 Distribuindo ${clientes} check-ins Sympla proporcionalmente ao faturamento Yuzer por hora...`
          );

          // Agrupar faturamento por hora
          const dadosPorHora: {
            [hora: string]: { faturamento: number; pedidos: Set<string> };
          } = {};
          ...
          yuzerHoraData.forEach((item: YuzerHoraItem) => {
            if (item.data_hora_pedido) {
              ...
              const horaString =
                item.data_hora_pedido.split('T')[1] || '00:00:00';
          ...
          const faturamentoTotal = Object.values(dadosPorHora).reduce(
            (sum, dados) => sum + dados.faturamento,
            0
          );
          ...
          if (diferenca !== 0) {
            const horaComMaiorFaturamento = Object.keys(dadosPorHora).reduce(
              (max, hora) =>
                dadosPorHora[hora].faturamento > dadosPorHora[max].faturamento
                  ? hora
                  : max
            );
          ...
          console.log(
            `🍺 Yuzer - Dados por hora: ${faturamento_horas.length} horários`
          );
          console.log(
            `📊 Faturamento acumulado: R$ ${faturamentoAcumulado.toFixed(2)}`
          );
        ...
        console.log(
          `👥 Buscando clientes por hora da tabela analitico para ${data}...`
        );
        ...
        console.log(
          `�� Total de registros de clientes encontrados: ${clientesHoraData.length}`
        );
        ...
        if (clientesHoraData && clientesHoraData.length > 0) {
          ...
          console.log(
            `👥 Nenhum dado de clientes por hora encontrado na tabela analitico para ${data}`
          );
        }
        ...
        if (fatHoraData && fatHoraData.length > 0) {
          ...
          const dadosOrdenados = fatHoraData
            .map((f: { hora: string; valor: string; qtd?: string }) => {
              ...
              const vendas =
                clientesPorHora[hora] || parseInt(f.qtd || '0') || 0;
          ...
          faturamento_horas = dadosOrdenados.map(
            (item: { hora: string; faturamento: number; vendas: number }) => {
          ...
          console.log(
            `💻 ContaHub - Dados por hora: ${faturamento_horas.length} horários`
          );
          console.log(
            `📊 Faturamento acumulado: R$ ${faturamentoAcumulado.toFixed(2)}`
          );
        ...
        faturamento_horas
          ...
          .forEach(
            (h: {
              ...
              console.log(
                `   ${h.hora}: R$ ${h.faturamento.toFixed(2)} | ${h.vendas} clientes | Acum: R$ ${h.faturamento_acumulado?.toFixed(2)} | ${h.clientes_acumulados} clientes`
              );
            }
          );
      } else {
        console.log(
          `⏰ Nenhum dado de faturamento por hora encontrado para ${data}`
        );
      }
  ...
  const buscarStatsArtista = async (
    nomeArtista: string
  ): Promise<ArtistaStats | null> => {
    try {
      ...
      console.log(
        `🔍 Buscando TODOS os eventos para ${nomeArtista} no bar ${selectedBar?.id}...`
      );
      ...
      console.log(
        `📅 TOTAL de eventos encontrados para ${nomeArtista}: ${eventos.length}`
      );
      console.log(`📅 Eventos completos:`, eventos);
      const datasEventos = eventos.map(
        (e: { data_evento: string }) => e.data_evento
      );
      console.log(
        `📆 TODAS as datas dos eventos (${datasEventos.length}): ${datasEventos.join(', ')}`
      );
      ...
      const dadosValidos = dadosEventos.filter(
        (d: DadosComparacao) => d.faturamento > 0 || d.clientes > 0
      );
      console.log(
        `✅ Dados válidos (faturamento > 0 ou clientes > 0): ${dadosValidos.length}`
      );
      ...
      dadosEventos.forEach((dados: DadosComparacao, index: number) => {
        ...
        console.log(
          `📅 ${dados.data}: Faturamento R$ ${dados.faturamento.toFixed(2)}, Clientes: ${dados.clientes}, Válido: ${isValido ? '✅' : '❌'}`
        );
        if (!isValido) {
          console.log(
            `   ⚠️ Evento sem dados: pode ser data futura ou problema na busca`
          );
        }
      });

      if (dadosValidos.length < datasEventos.length) {
        const datasSemDados = dadosEventos
          .filter(
            (d: DadosComparacao) => d.faturamento === 0 && d.clientes === 0
          )
          .map((d: DadosComparacao) => d.data);
        console.log(
          `⚠️ ATENÇÃO: ${datasSemDados.length} datas sem dados válidos: ${datasSemDados.join(', ')}`
        );
        console.log(
          `💡 Provavelmente são datas futuras ainda sem movimentação real`
        );
      }
      ...
      const totalFaturamento = dadosValidos.reduce(
        (sum, d) => sum + d.faturamento,
        0
      );
      const totalFaturamentoYuzer = dadosValidos.reduce(
        (sum, d) => sum + d.faturamentoYuzer,
        0
      );
      const totalFaturamentoContaHub = dadosValidos.reduce(
        (sum, d) => sum + d.faturamentoContaHub,
        0
      );
      const totalFaturamentoSympla = dadosValidos.reduce(
        (sum, d) => sum + d.faturamentoSympla,
        0
      );
      const totalClientes = dadosValidos.reduce(
        (sum, d) => sum + d.clientes,
        0
      );
      const totalReservas = dadosValidos.reduce(
        (sum, d) => sum + d.reservas,
        0
      );
      ...
      console.log(
        `💰 Total faturamento: R$ ${totalFaturamento.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
      );
      console.log(
        `🍻 Total Yuzer: R$ ${totalFaturamentoYuzer.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
      );
      console.log(
        `🎫 Total ContaHub: R$ ${totalFaturamentoContaHub.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
      );
      console.log(
        `🎟️ Total Sympla: R$ ${totalFaturamentoSympla.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
      );
      console.log(
        `🔄 Faturamento médio será: R$ ${(totalFaturamento / dadosValidos.length).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
      );

      // Calcular tempos médios (apenas onde há dados)
      const dadosComTempoCozinha = dadosValidos.filter(
        (d: DadosComparacao) => d.tempoCozinha > 0
      );
      const dadosComTempoBar = dadosValidos.filter(
        (d: DadosComparacao) => d.tempoBar > 0
      );

      const tempoMedioCozinha =
        dadosComTempoCozinha.length > 0
          ? dadosComTempoCozinha.reduce((sum, d) => sum + d.tempoCozinha, 0) /
            dadosComTempoCozinha.length
          : 0;

      const tempoMedioBar =
        dadosComTempoBar.length > 0
          ? dadosComTempoBar.reduce((sum, d) => sum + d.tempoBar, 0) /
            dadosComTempoBar.length
          : 0;
      ...
      if (tempoMedioBar > 0) {
        ...
        eficienciaAtendimento = Math.max(
          0,
          Math.min(100, 100 - tempoMedioBar * 5)
        );
      }
      ...
      if (dadosValidos.length >= 2) {
        ...
        if (primeiroEvento.clientes > 0) {
          crescimentoClientes =
            ((ultimoEvento.clientes - primeiroEvento.clientes) /
              primeiroEvento.clientes) *
            100;
        }
      }

      // Fidelização = recorrência + crescimento (métrica combinada)
      const fidelizacaoClientes =
        recorrenciaMedia * 0.7 + Math.max(0, crescimentoClientes) * 0.3;
      ...
      const resultado: ArtistaStats = {
        ...
        faturamentoContaHubMedio:
          totalFaturamentoContaHub / dadosValidos.length,
      ...
      console.error(
        `❌ Erro ao buscar stats do artista ${nomeArtista}:`,
        error
      );
      return null;
    }
  ...
  const gerarInsightComparacao = () => {
    ...
    // Faturamento
    const difFaturamento =
      (dados2?.faturamento || 0) - (dados1?.faturamento || 0);
    const percFaturamento =
      (dados1?.faturamento || 0) > 0
        ? (difFaturamento / (dados1?.faturamento || 0)) * 100
        : 0;

    if (difFaturamento > 0) {
      insights.push(
        `📈 Faturamento cresceu R$ ${difFaturamento.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} (+${percFaturamento.toFixed(1)}%)`
      );
    } else if (difFaturamento < 0) {
      insights.push(
        `📉 Faturamento caiu R$ ${Math.abs(difFaturamento).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} (${percFaturamento.toFixed(1)}%)`
      );
    }
    ...
    if (Math.abs(difTicket) > 5) {
      insights.push(
        `🎯 Ticket médio ${difTicket > 0 ? 'aumentou' : 'diminuiu'} R$ ${Math.abs(difTicket).toFixed(2)}`
      );
    }

    // Tempos
    if ((dados2?.tempoCozinha || 0) > 0 && (dados1?.tempoCozinha || 0) > 0) {
      const difTempoCozinha =
        (dados2?.tempoCozinha || 0) - (dados1?.tempoCozinha || 0);
      if (Math.abs(difTempoCozinha) > 1) {
        insights.push(
          `👨‍🍳 Tempo de cozinha ${difTempoCozinha > 0 ? 'aumentou' : 'diminuiu'} ${Math.abs(difTempoCozinha).toFixed(1)} min`
        );
      }
    }

    // Artistas
    if ((dados2?.artista || '') !== (dados1?.artista || '')) {
      insights.push(
        `🎤 Mudança de artista: ${dados1?.artista || 'N/A'} → ${dados2?.artista || 'N/A'}`
      );
    }
  ...
  const gerarInsightArtistas = () => {
    ...
    // Faturamento médio
    const melhorFaturamento =
      artista1.faturamentoMedio > artista2.faturamentoMedio
        ? artista1
        : artista2;
    const difFaturamento = Math.abs(
      artista1.faturamentoMedio - artista2.faturamentoMedio
    );
    insights.push(
      `💰 ${melhorFaturamento.nome} gera R$ ${difFaturamento.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} a mais por evento`
    );

    // Clientes médios
    const melhorClientes =
      artista1.clientesMedio > artista2.clientesMedio ? artista1 : artista2;
    const difClientes = Math.abs(
      artista1.clientesMedio - artista2.clientesMedio
    );
    insights.push(
      `👥 ${melhorClientes.nome} atrai ${difClientes.toFixed(0)} clientes a mais por evento`
    );

    // Ticket médio
    const melhorTicket =
      artista1.ticketMedio > artista2.ticketMedio ? artista1 : artista2;
    const difTicket = Math.abs(artista1.ticketMedio - artista2.ticketMedio);
    insights.push(
      `🎯 ${melhorTicket.nome} tem ticket médio R$ ${difTicket.toFixed(2)} superior`
    );

    // Consistência
    const maisConsistente =
      artista1.totalEventos > artista2.totalEventos ? artista1 : artista2;
    insights.push(
      `🎪 ${maisConsistente.nome} tem mais eventos registrados (${maisConsistente.totalEventos} vs ${maisConsistente === artista1 ? artista2.totalEventos : artista1.totalEventos})`
    );

    // Performance geral
    const scoreArtista1 =
      artista1.faturamentoMedio * 0.4 +
      artista1.clientesMedio * 0.3 +
      artista1.ticketMedio * 0.3;
    const scoreArtista2 =
      artista2.faturamentoMedio * 0.4 +
      artista2.clientesMedio * 0.3 +
      artista2.ticketMedio * 0.3;
    const melhorGeral = scoreArtista1 > scoreArtista2 ? artista1 : artista2;
    insights.push(
      `🏆 ${melhorGeral.nome} tem melhor performance geral para a casa`
    );
  ...
  const gerarInsights = (
    dados1: DadosComparacao,
    dados2: DadosComparacao,
    stats1: ArtistaStats,
    stats2: ArtistaStats
  ): string[] => {
    const insights: string[] = [];
    ...
    const diferencaFat =
      (((dados1?.faturamento || 0) - (dados2?.faturamento || 0)) /
        (dados2?.faturamento || 0)) *
      100;
    if (Math.abs(diferencaFat) > 20) {
      const melhor =
        diferencaFat > 0 ? dados1?.artista || 'N/A' : dados2?.artista || 'N/A';
      insights.push(
        `💰 ${melhor} tem faturamento ${Math.abs(diferencaFat).toFixed(1)}% superior`
      );
    }

    // Análise de ticket médio
    const diferencaTicket =
      (((dados1?.ticketMedio || 0) - (dados2?.ticketMedio || 0)) /
        (dados2?.ticketMedio || 0)) *
      100;
    if (Math.abs(diferencaTicket) > 15) {
      const melhor =
        diferencaTicket > 0
          ...
          : dados2?.artista || 'N/A';
      insights.push(
        `🎯 ${melhor} gera ticket médio ${Math.abs(diferencaTicket).toFixed(1)}% maior`
      );
    }
    ...
    if ((dados1?.tempoBar || 0) > 0 && (dados2?.tempoBar || 0) > 0) {
      const melhorTempo =
        (dados1?.tempoBar || 0) < (dados2?.tempoBar || 0)
          ? dados1?.artista || 'N/A'
          : dados2?.artista || 'N/A';
    ...
    // Análise de clientes
    const diferencaClientes =
      (((dados1?.clientes || 0) - (dados2?.clientes || 0)) /
        (dados2?.clientes || 0)) *
      100;
    if (Math.abs(diferencaClientes) > 25) {
      const melhor =
        diferencaClientes > 0
          ...
          : dados2?.artista || 'N/A';
      insights.push(
        `👥 ${melhor} atrai ${Math.abs(diferencaClientes).toFixed(1)}% mais público`
      );
    }
  ...
  const buscarRecorrenciaPorArtista = async (nomeArtista: string) => {
    try {
      ...
      if (!eventosArtista || eventosArtista.length < 2) {
        console.log(
          `❌ Poucos eventos para ${nomeArtista}: ${eventosArtista?.length || 0}`
        );
        return 0;
      }
      ...
      for (const evento of eventosArtista) {
        try {
          ...
          console.log(
            `Erro ao buscar dados para ${evento.data_evento}:`,
            error
          );
        }
      }
      ...
      if (eventosComDados > 0) {
        ...
        const percentualEventosComDados =
          (eventosComDados / totalEventos) * 100;
        ...
        recorrenciaEstimada =
          totalEventos >= 10 ? 30 : totalEventos >= 5 ? 20 : 15;
      }
      ...
      console.log(
        `📊 Recorrência estimada para ${nomeArtista}: ${recorrenciaEstimada.toFixed(1)}% (${totalEventos} eventos, ${eventosComDados} com dados, média ${(totalPessoas / eventosComDados || 0).toFixed(0)} pessoas)`
      );
  ...
  const compararDatas = async () => {
    if (!artistaSelecionado1 || !artistaSelecionado2 || !selectedBar?.id)
      return;

    setLoading(true);
    console.log(
      `🎤 Comparando artistas: ${artistaSelecionado1} vs ${artistaSelecionado2}`
    );
    ...
    try {
      ...
      if (!stats1 || !stats2) {
        alert(
          'Erro ao buscar dados dos artistas. Verifique se existem eventos cadastrados.'
        );
        return;
      }
      ...
      // Gerar insights
      const novosInsights = gerarInsights(
        dadosData1,
        dadosData2,
        stats1,
        stats2
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-slate-600 mt-1">
            Compare performance entre datas específicas ou artistas diferentes
          </p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={e =>
                setTipoComparacao(e.target.value as TipoComparacao)
              }
              className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
              <option value="datas-especificas">
                🗓️ Comparar Datas Específicas
              </option>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              >
                ...
                {listaArtistas
                  .filter(a => a !== artistaSelecionado1)
                  .map(artista => (
                    <option key={artista} value={artista}>
                      {artista}
                    </option>
            ...
            <div className="flex items-end">
              <button
                ...
                disabled={
                  !artistaSelecionado1 || !artistaSelecionado2 || loading
                }
                className="w-full bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
      ...
      {((tipoComparacao === 'datas-especificas' &&
        ...
        (tipoComparacao === 'artistas' &&
          dadosArtistas.artista1 &&
          dadosArtistas.artista2)) && (
        <>
            ...
            dadosArtistas.artista2 && (
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 className="text-lg font-bold text-slate-800 mb-4">
                  🎯 Visão Geral dos Artistas
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <span className="font-medium">
                      🎤 {dadosArtistas.artista1.nome}:
                    </span>
                    ...
                    <div className="mt-1 text-xs text-gray-600 space-y-1">
                      <div>
                        💰 Faturamento médio: R${' '}
                        {dadosArtistas.artista1.faturamentoMedio.toLocaleString(
                          'pt-BR',
                      ...
                      <div>
                        📊 Faturamento total: R${' '}
                        {dadosArtistas.artista1.faturamentoTotal.toLocaleString(
                          'pt-BR',
                          { minimumFractionDigits: 2 }
                        )}
                      </div>
                      {dadosArtistas.artista1.faturamentoYuzerTotal > 0 && (
                        <div>
                          🍻 Yuzer total: R${' '}
                          {dadosArtistas.artista1.faturamentoYuzerTotal.toLocaleString(
                            'pt-BR',
                            { minimumFractionDigits: 2 }
                          )}
                        </div>
                      ...
                      {dadosArtistas.artista1.faturamentoContaHubTotal > 0 && (
                        <div>
                          🎫 ContaHub total: R${' '}
                          {dadosArtistas.artista1.faturamentoContaHubTotal.toLocaleString(
                            'pt-BR',
                            { minimumFractionDigits: 2 }
                          )}
                        </div>
                      ...
                      {dadosArtistas.artista1.faturamentoSymplaTotal > 0 && (
                        <div>
                          🎟️ Sympla total: R${' '}
                          {dadosArtistas.artista1.faturamentoSymplaTotal.toLocaleString(
                            'pt-BR',
                            { minimumFractionDigits: 2 }
                          )}
                        </div>
                      ...
                      <div>
                        👥 Clientes total:{' '}
                        {dadosArtistas.artista1.clientesTotal.toLocaleString(
                          'pt-BR'
                        )}
                      </div>
                      <div>
                        📊 {dadosArtistas.artista1.eventosComDados}/
                        {dadosArtistas.artista1.totalEventos} eventos com dados
                      </div>
                    </div>
                  </div>
                  <div>
                    <span className="font-medium">
                      🎤 {dadosArtistas.artista2.nome}:
                    </span>
                    <span className="ml-2 text-green-600">
                      ✅ Dados disponíveis
                    </span>
                    <div className="mt-1 text-xs text-gray-600 space-y-1">
                      <div>
                        💰 Faturamento médio: R${' '}
                        {dadosArtistas.artista2.faturamentoMedio.toLocaleString(
                          'pt-BR',
                          { minimumFractionDigits: 2 }
                        )}
                      </div>
                      <div>
                        📊 Faturamento total: R${' '}
                        {dadosArtistas.artista2.faturamentoTotal.toLocaleString(
                          'pt-BR',
                          { minimumFractionDigits: 2 }
                        )}
                      </div>
                      {dadosArtistas.artista2.faturamentoYuzerTotal > 0 && (
                        <div>
                          🍻 Yuzer total: R${' '}
                          {dadosArtistas.artista2.faturamentoYuzerTotal.toLocaleString(
                            'pt-BR',
                            { minimumFractionDigits: 2 }
                          )}
                        </div>
                      ...
                      {dadosArtistas.artista2.faturamentoContaHubTotal > 0 && (
                        <div>
                          🎫 ContaHub total: R${' '}
                          {dadosArtistas.artista2.faturamentoContaHubTotal.toLocaleString(
                            'pt-BR',
                            { minimumFractionDigits: 2 }
                          )}
                        </div>
                      ...
                      {dadosArtistas.artista2.faturamentoSymplaTotal > 0 && (
                        <div>
                          🎟️ Sympla total: R${' '}
                          {dadosArtistas.artista2.faturamentoSymplaTotal.toLocaleString(
                            'pt-BR',
                            { minimumFractionDigits: 2 }
                          )}
                        </div>
                      ...
                      <div>
                        👥 Clientes total:{' '}
                        {dadosArtistas.artista2.clientesTotal.toLocaleString(
                          'pt-BR'
                        )}
                      </div>
                      <div>
                        📊 {dadosArtistas.artista2.eventosComDados}/
                        {dadosArtistas.artista2.totalEventos} eventos com dados
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {tipoComparacao === 'datas-especificas' &&
            dadosComparativos.length === 2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {dadosComparativos.map(
                  (dados: DadosComparacao, index: number) => (
                    <div
                      key={index}
                    ...
                    >
                      <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
                        ...
                        <span className="ml-auto text-sm font-normal text-slate-600">
                          {parseDataSemTimezone(dados.data).toLocaleDateString(
                            'pt-BR'
                          )}
                        </span>
                      </h3>
                        ...
                        <div className="bg-white rounded-lg p-3">
                          ...
                          <div className="space-y-1">
                            {dados.pratos
                              ...
                              .map(
                                (
                                  prato: { nome: string; quantidade: number },
                                  i: number
                                ) => (
                                  <div
                                    key={i}
          ...
          {tipoComparacao === 'artistas' &&
            ...
            dadosArtistas.artista2 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[dadosArtistas.artista1, dadosArtistas.artista2].map(
                  (artista: ArtistaStats, index: number) => (
                    <div
                      key={index}
                      className={`bg-white rounded-xl p-6 shadow-sm border ${
                        index === 0
                    ...
                    >
                      <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
                        ...
                        <span className="ml-auto text-sm font-normal text-slate-600">
                          {artista.totalEventos} eventos (
                          {artista.eventosComDados} com dados)
                        </span>
                      </h3>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="card-valor">
                            ...
                            <div className="valor-monetario">
                              R${' '}
                              {artista.faturamentoTotal.toLocaleString(
                                'pt-BR',
                                { minimumFractionDigits: 2 }
                              )}
                            </div>
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              Média: R${' '}
                              {artista.faturamentoMedio.toLocaleString(
                                'pt-BR',
                                { minimumFractionDigits: 2 }
                              )}
                            </div>
                          ...
                          <div className="card-valor">
                            ...
                            <div className="valor-monetario">
                              R${' '}
                              {artista.ticketMedio.toLocaleString('pt-BR', {
                                minimumFractionDigits: 2,
                              })}
                            </div>
                            <div className="debug-valor">
                        ...
                        {artista.tempoMedioCozinha > 0 &&
                          artista.tempoMedioBar > 0 && (
                            <div className="grid grid-cols-2 gap-4">
                              <div className="card-valor">
                                <div className="text-2xl mb-1">👨‍🍳</div>
                                <div className="valor-monetario">
          ...
          {recorrenciaClientes &&
            tipoComparacao === 'datas-especificas' &&
            typeof recorrenciaClientes === 'object' && (
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 className="text-lg font-bold text-slate-800 mb-4">
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-yellow-700 mt-1">
                          {recorrenciaClientes.message}
                        </p>
                        <div className="grid grid-cols-2 gap-4 mt-3 text-sm">
                          <div>
                            <span className="font-medium">
                              {recorrenciaClientes.data?.data1}:
                            </span>
                            <span className="ml-2">
                              {recorrenciaClientes.data?.totalData1 || 0}{' '}
                              pessoas
                            </span>
                          </div>
                          <div>
                            <span className="font-medium">
                              {recorrenciaClientes.data?.data2}:
                            </span>
                            <span className="ml-2">
                              {recorrenciaClientes.data?.totalData2 || 0}{' '}
                              pessoas
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="text-center p-4 bg-blue-50 rounded-lg">
                        ...
                        <div className="text-xs text-blue-500 mt-1">
                          {recorrenciaClientes.percentualRecorrencia || 0}% do
                          total
                        </div>
                      </div>
                      ...
                      <div className="text-center p-4 bg-purple-50 rounded-lg">
                        ...
                        <div className="text-xs text-purple-500 mt-1">
                          Crescimento:{' '}
                          {recorrenciaClientes.crescimento?.toFixed(1) || 0}%
                        </div>
                      </div>
                    </div>
                  </>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-2">
                      {gerarInsightComparacao()?.map(
                        (insight: string, i: number) => (
                          <div
                            key={i}
                            className="text-sm text-slate-600 bg-gray-50 p-2 rounded"
                          >
                            {insight}
                          </div>
                        )
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {(() => {
                        ...
                        if (
                          insights.some(i => i.includes('Faturamento caiu'))
                        ) {
                          recomendacoes.push(
                            '🔄 Revisar estratégias de pricing e promoções'
                          );
                        }
                        if (
                          insights.some(i => i.includes('clientes a menos'))
                        ) {
                          recomendacoes.push(
                            '📢 Intensificar marketing e divulgação'
                          );
                        }
                        if (
                          insights.some(i =>
                            i.includes('Tempo de cozinha aumentou')
                          )
                        ) {
                          recomendacoes.push(
                            '⚡ Otimizar processos da cozinha'
                          );
                        }
                        if (
                          insights.some(i => i.includes('Mudança de artista'))
                        ) {
                          recomendacoes.push(
                            '🎵 Avaliar impacto do artista nos resultados'
                          );
                        }

                        if (recomendacoes.length === 0) {
                          recomendacoes.push(
                            '✅ Performance consistente, manter estratégias atuais'
                          );
                        }
                        ...
                        return recomendacoes.map((rec: string, i: number) => (
                          <div
                            key={i}
                            className="text-sm text-slate-600 bg-blue-50 p-2 rounded"
                          >
                            {rec}
                          </div>
                        ));
          ...
          {tipoComparacao === 'datas-especificas' &&
            dadosComparativos.length === 2 &&
            (dadosComparativos[0].faturamento_horas.length > 0 ||
              dadosComparativos[1].faturamento_horas.length > 0) && (
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <h3 className="text-lg font-bold text-slate-800 mb-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-blue-700">
                    <div>
                      ...
                      <div>
                        • <strong>Linha superior:</strong> Vendas naquela hora
                        específica
                      </div>
                      <div>
                        • <strong>Linha inferior:</strong> Total acumulado até
                        aquela hora
                      </div>
                    </div>
                    <div>
                      ...
                      <div>
                        • <strong>Linha superior:</strong> Clientes acumulados
                        da Data 1
                      </div>
                      <div>
                        • <strong>Linha inferior:</strong> Clientes acumulados
                        da Data 2
                      </div>
                    </div>
                  </div>
                  <div className="mt-2 text-xs text-blue-600">
                    <strong>Exemplo:</strong> Às 18h, se mostra &quot;R$ 500 |
                    R$ 2.000&quot;, significa que naquela hora vendeu R$ 500 e o
                    total do dia até às 18h era R$ 2.000
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  const todosHorarios = new Set([
                    ...dadosComparativos[0].faturamento_horas.map(
                      (h: { hora: string }) => h.hora
                    ),
                    ...dadosComparativos[1].faturamento_horas.map(
                      (h: { hora: string }) => h.hora
                    ),
                  ]);
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="max-h-96 overflow-y-auto space-y-1">
                        {horariosOrdenados.map(hora => {
                          const data1Hora =
                            dadosComparativos[0].faturamento_horas.find(
                              h => h.hora === hora
                            );
                          const data2Hora =
                            dadosComparativos[1].faturamento_horas.find(
                              h => h.hora === hora
                            );
                          ...
                          const acumulado1 =
                            data1Hora?.faturamento_acumulado || 0;
                          const acumulado2 =
                            data2Hora?.faturamento_acumulado || 0;
                          const clientes1 = data1Hora?.clientes_acumulados || 0;
                          const clientes2 = data2Hora?.clientes_acumulados || 0;

                          // Calcular diferenças percentuais
                          const diferencaHora =
                            ((valor2 - valor1) / (valor1 || 1)) * 100;
                          const diferencaAcumulado =
                            ((acumulado2 - acumulado1) / (acumulado1 || 1)) *
                            100;
                          const temDiferenca =
                            Math.abs(diferencaHora) > 5 ||
                            Math.abs(diferencaAcumulado) > 5;

                          return (
                            <div
                              key={hora}
                              className={`grid grid-cols-5 gap-2 text-xs py-2 px-2 rounded-lg ${
                                temDiferenca
                            ...
                            >
                              ...
                              >
                                <div className="font-semibold">
                                  {valor1 > 0
                                    ? `R$ ${valor1.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}`
                                    : '---'}
                                </div>
                                <div className="text-xs text-blue-600">
                                  {acumulado1 > 0
                                    ? `R$ ${acumulado1.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}`
                                    : '---'}
                                </div>
                              </div>
                              ...
                              >
                                <div className="font-semibold">
                                  {valor2 > 0
                                    ? `R$ ${valor2.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}`
                                    : '---'}
                                </div>
                                <div className="text-xs text-green-600">
                                  {acumulado2 > 0
                                    ? `R$ ${acumulado2.toLocaleString('pt-BR', { minimumFractionDigits: 0 })}`
                                    : '---'}
                                </div>
                              </div>
                                ...
                                {acumulado1 > 0 && acumulado2 > 0 && (
                                  <div
                                    className={`text-xs px-1 py-0.5 rounded ${
                                      diferencaAcumulado > 0
                                        ? 'bg-blue-100 text-blue-700'
                                        : 'bg-orange-100 text-orange-700'
                                    }`}
                                  >
                                    {diferencaAcumulado > 0 ? '+' : ''}
                                    {diferencaAcumulado.toFixed(0)}%
                                  </div>
                                )}
                              </div>
                            </div>
                      ...
                      <div className="mt-4 pt-4 border-t">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                          <div className="space-y-2">
                            ...
                            {(() => {
                              const horas1 =
                                dadosComparativos[0].faturamento_horas.filter(
                                  (h: { faturamento: number }) =>
                                    h.faturamento > 0
                                );
                              if (horas1.length === 0)
                                return (
                              ...
                              const melhorHora1 = horas1.reduce((max, h) =>
                                ...
                                <div className="space-y-1 text-xs text-slate-600">
                                  <div>
                                    ...
                                    <span className="font-semibold">
                                      {formatHour(melhorHora1.hora)}
                                    </span>{' '}
                                    (R${' '}
                                    {melhorHora1.faturamento.toLocaleString(
                                      'pt-BR',
                                  ...
                                  <div>
                                    ...
                                    <span className="font-semibold">
                                      R${' '}
                                      {(
                                        ultimaHora1?.faturamento_acumulado || 0
                                  ...
                                  <div>
                                    👥 Clientes finais:{' '}
                                    <span className="font-semibold">
                                      {ultimaHora1?.clientes_acumulados || 0}
                                    </span>
                                  </div>
                                </div>
                              );
                          ...
                          <div className="space-y-2">
                            ...
                            {(() => {
                              const horas2 =
                                dadosComparativos[1].faturamento_horas.filter(
                                  (h: { faturamento: number }) =>
                                    h.faturamento > 0
                                );
                              if (horas2.length === 0)
                              ...
                              const melhorHora2 = horas2.reduce((max, h) =>
                                h.faturamento > max.faturamento ? h : max
                              );
                              ...
                              return (
                                <div className="space-y-1 text-xs text-slate-600">
                                  <div>
                                    🕐 Melhor horário:{' '}
                                    <span className="font-semibold">
                                      {formatHour(melhorHora2.hora)}
                                    </span>{' '}
                                    (R${' '}
                                    {melhorHora2.faturamento.toLocaleString(
                                      'pt-BR',
                                      { minimumFractionDigits: 0 }
                                    )}
                                    )
                                  ...
                                  <div>
                                    ...
                                    <span className="font-semibold">
                                      R${' '}
                                      {(
                                        ultimaHora2?.faturamento_acumulado || 0
                                      ).toLocaleString('pt-BR', {
                                        minimumFractionDigits: 0,
                                      })}
                                    </span>
                                  ...
                                  <div>
                                    👥 Clientes finais:{' '}
                                    <span className="font-semibold">
                                      {ultimaHora2?.clientes_acumulados || 0}
                                    </span>
                                  </div>
                                </div>
                              );
              ...
              <div>
                ...
                <div className="space-y-2">
                  {(() => {
                    ...
                    recomendacoes.push(
                      `🎯 Priorizar agendamento de ${melhor.nome} para datas especiais`
                    );
                    recomendacoes.push(
                      `📈 ${melhor.nome} tem ROI superior para eventos premium`
                    );
                    recomendacoes.push(
                      `🔄 Analisar estratégias de ${pior.nome} para otimizar performance`
                    );
                    recomendacoes.push(
                      `💰 ${melhor.nome} justifica investimento maior em cachê`
                    );
