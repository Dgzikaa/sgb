...
export default function MetricasBarrasPage() {
  ...
  const buscarDadosMensais = async () => {
    ...
    try {
      console.log(
        '📊 Buscando dados mensais CORRETOS para',
        anoSelecionado,
        '(Sympla + Yuzer + Contahub)'
      );
      ...
      for (let mes = 1; mes <= 12; mes++) {
        ...
        console.log(
          `💰 Buscando faturamento CORRETO para ${meses[mes - 1]}: ${dataInicio} até ${dataFim}`
        );
        ...
        const faturamento = allFaturamentoData.reduce(
          (sum: number, item: PagamentoData) =>
            sum + parseFloat(String(item.liquido) || '0'),
          0
        );
        console.log(
          `💰 ${meses[mes - 1]}: R$ ${faturamento.toLocaleString('pt-BR')} (${allFaturamentoData.length} registros)`
        );
        ...
        if (!clientesError && clientesData) {
          clientes = clientesData.reduce(
            (sum: number, item: PessoasData) =>
              sum + parseInt(String(item.total_pessoas_bruto) || '0'),
            0
          );
          clientesPagantes = clientesData.reduce(
            (sum: number, item: PessoasData) =>
              sum + parseInt(String(item.pessoas_pagantes) || '0'),
            0
          );
          console.log(
            `👥 ${meses[mes - 1]}: ${clientes} pessoas totais, ${clientesPagantes} pagantes`
          );
        }

        // TICKET MÉDIO CORRETO: Faturamento / Pessoas Pagantes
        const ticketMedio =
          clientesPagantes > 0 ? faturamento / clientesPagantes : 0;
        ...
        try {
          const response = await fetch(
            `/api/dashboard/reservas-getin?data_inicio=${dataInicio}&data_fim=${dataFim}&tipo=periodo`
          );
          ...
          if (result.success && result.data?.estatisticas) {
            ...
            console.log(
              `📅 ${meses[mes - 1]}: ${reservas} pessoas reservadas (${result.data.estatisticas.total_reservas || 0} reservas)`
            );
          } else {
          ...
          console.error(
            `❌ Erro ao buscar reservas para ${meses[mes - 1]}:`,
            error
          );
        }
        ...
        console.log(
          `⏱️ ${meses[mes - 1]}: Cozinha ${tempoCozinha.toFixed(1)}min (${allTempoCozinhaData.length} registros), Bar ${tempoBar.toFixed(1)}min (${allTempoBarData.length} registros)`
        );
      ...
      dadosCompletos.forEach(mes => {
        console.log(
          `   ${mes.mes}: Fat R$ ${mes.faturamento.toLocaleString('pt-BR')}, ${mes.clientes} clientes, TM R$ ${mes.ticketMedio.toFixed(2)}, ${mes.reservas} reservas`
        );
      });
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <h1 className="text-2xl font-bold text-slate-800 mb-2">
            📊 Dashboard - Métricas Mensais Comparativas
          </h1>
          <p className="text-slate-600">
            Comparação lado a lado das métricas mensais do {selectedBar?.nome}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="flex items-center space-x-4">
            <div>
              ...
              >
                <option value={2025} className="text-gray-900 bg-white">
                  2025
                </option>
              </select>
            </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                {metricas.map(metrica => {
                  const total = dadosMensais.reduce(
                    (sum, mes) =>
                      sum + (mes[metrica.key as keyof DadosMensal] as number),
                    0
                  );
                  const media =
                    total /
                    dadosMensais.filter(
                      mes =>
                        (mes[metrica.key as keyof DadosMensal] as number) > 0
                    ).length;
                  const metaAnual =
                    (metas[metrica.metaKey as keyof MetasConfig] as number) *
                    (metrica.key === 'faturamento' ||
                    metrica.key === 'clientes' ||
                    metrica.key === 'reservas'
                      ? 12
                      : 1);
                  const percentual =
                    metaAnual > 0 ? (total / metaAnual) * 100 : 0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-lg font-bold text-slate-800">
                        {metrica.unit === 'R$' ? 'R$ ' : ''}
                        {(metrica.key === 'tempoCozinha' ||
                        metrica.key === 'tempoBar' ||
                        metrica.key === 'ticketMedio'
                          ? media
                          : total
                      ...
                      <p
                        className={`text-xs font-medium ${percentual >= 100 ? 'text-green-600' : 'text-red-600'}`}
                      >
            ...
            {metricas.map(metrica => {
              const maxValor = Math.max(
                ...dadosMensais.map(
                  mes => mes[metrica.key as keyof DadosMensal] as number
                ),
                metas[metrica.metaKey as keyof MetasConfig] as number
              );
                  ...
                  <div className="space-y-4">
                    {dadosMensais.map(
                      (
                        ...
                        const meta = metas[
                          metrica.metaKey as keyof MetasConfig
                        ] as number;
                        const largura =
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              ...
                              onMouseEnter={() =>
                                setHoveredItem({ metrica: metrica.key, index })
                              }
                              ...
                              <div className="w-full bg-gray-200 rounded-full h-8 relative">
                                <div
                                  className={`h-8 rounded-full transition-all duration-500 flex items-center justify-end pr-2 ${
                                    hoveredItem?.metrica === metrica.key &&
                            ...
                            <div className="w-32 text-right">
                              ...
                              <div
                                className={`text-xs ${acimaMeta ? 'text-green-600' : 'text-red-600'}`}
                              >
                                {meta > 0
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-xs text-slate-500">
                        {(() => {
                          const mesesComDados = dadosMensais.filter(
                            mes =>
                              (mes[
                                metrica.key as keyof DadosMensal
                              ] as number) > 0
                          );
                          const mesesAcimaMeta = mesesComDados.filter(
                            mes =>
                              (mes[
                                metrica.key as keyof DadosMensal
                              ] as number) >=
                ...
                <div className="space-y-4">
                  ...
                  {metricas.slice(0, 3).map(metrica => {
                    const melhorMes = dadosMensais.reduce((max, mes) =>
                      (mes[metrica.key as keyof DadosMensal] as number) >
                      (max[metrica.key as keyof DadosMensal] as number)
                        ? mes
                        : max
                    );
                    return (
                    ...
                    return (
                      <div className="space-y-2">
                        <p className="text-sm text-slate-600">
                          ...
                          metas anuais atingidas
                        </p>
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            className="bg-green-500 h-2 rounded-full transition-all duration-500"
                            style={{
                              width: `${(metasAtingidas / totalMetas) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
          ...
          <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 text-center">
            ...
            <p className="text-slate-500">
              Escolha o ano que deseja analisar e clique em &quot;Analisar&quot;
              para visualizar a comparação mensal.
            </p>
          </div>
        )}
      </div>
