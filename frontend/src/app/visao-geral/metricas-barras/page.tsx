...
export default function MetricasBarrasPage() {
  ...
  const meses = [
    'Janeiro', 'Fevereiro', 'MarÃ¡Â§o', 'Abril', 'Maio', 'Junho',
    'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'
  ]

  const metricas = [
    { key: 'faturamento', label: 'Faturamento', icon: 'Ã°Å¸â€™Â°', unit: 'R$', color: 'blue', metaKey: 'metaMensalFaturamento' },
    { key: 'clientes', label: 'Clientes', icon: 'Ã°Å¸â€˜Â¥', unit: '', color: 'green', metaKey: 'metaMensalClientes' },
    { key: 'ticketMedio', label: 'Ticket MÃ¡Â©dio', icon: 'Ã°Å¸Å½Â¯', unit: 'R$', color: 'purple', metaKey: 'ticketMedioTarget' },
    { key: 'reservas', label: 'Reservas', icon: 'Ã°Å¸â€œâ€¦', unit: 'pessoas', color: 'orange', metaKey: 'reservasMensais' },
    { key: 'tempoCozinha', label: 'Tempo Cozinha', icon: 'Ã°Å¸â€˜Â¨â‚¬ÂÃ°Å¸ÂÂ³', unit: 'min', color: 'red', metaKey: 'tempoSaidaCozinha' },
    { key: 'tempoBar', label: 'Tempo Bar', icon: 'Ã°Å¸ÂÂ¹', unit: 'min', color: 'cyan', metaKey: 'tempoSaidaBar' }
  ]
  ...
  const buscarDadosMensais = async () => {
    ...
    try {
      console.log('Ã°Å¸â€œÅ  Buscando dados mensais CORRETOS para', anoSelecionado, '(Sympla + Yuzer + Contahub)')
      
      const dadosCompletos: DadosMensal[] = []
      
      for (let mes = 1; mes <= 12; mes++) {
        console.log(`Ã°Å¸â€œâ€¦ Processando ${meses[mes - 1]} (${mes}/12)...`)
        ...
        console.log(`Ã°Å¸â€™Â° ${meses[mes - 1]}: R$ ${faturamento.toLocaleString('pt-BR')} (${allFaturamentoData.length} registros)`)

        // BUSCAR CLIENTES CORRETOS: Usar dados agregados (Sympla + Yuzer)
        console.log(`Ã°Å¸â€˜Â¥ Buscando clientes CORRETOS para ${meses[mes - 1]}`)
        ...
        // TICKET MÃ¡â€°DIO CORRETO: Faturamento / Pessoas Pagantes
        const ticketMedio = clientesPagantes > 0 ? faturamento / clientesPagantes : 0

        // BUSCAR RESERVAS CORRETAS: GetIn API com dados agregados por perÃ¡Â­odo
        console.log(`Ã°Å¸â€œâ€¦ Buscando reservas GetIn para ${meses[mes - 1]}`)
        ...
        try {
          ...
          if (result.success && result.data?.estatisticas) {
            ...
            console.log(`Ã°Å¸â€œâ€¦ ${meses[mes - 1]}: ${reservas} pessoas reservadas (${result.data.estatisticas.total_reservas || 0} reservas)`)
          } else {
            console.log(`Ã°Å¸â€œâ€¦ ${meses[mes - 1]}: Nenhuma reserva encontrada`)
          }
        } catch (error) {
          console.error(`ÂÅ’ Erro ao buscar reservas para ${meses[mes - 1]}:`, error)
        }

        // BUSCAR TEMPOS CORRETOS: Usar chunking + filtros inteligentes
        console.log(`ÂÂ±Ã¯Â¸Â Buscando tempos CORRETOS para ${meses[mes - 1]}`)
        ...
        while (hasMore) {
          const { data: chunk, error } = await supabase
            ...
            .lte('dia', dataFimInt)
            .not('t0_t2', 'is', null)
            .gt('t0_t2', 60)      // MÃ¡Â­nimo 1 minuto
            .lt('t0_t2', 2700)    // MÃ¡Â¡ximo 45 minutos
            .order('dia')
            .range(offset, offset + CHUNK_SIZE - 1)
        ...
        while (hasMore) {
          const { data: chunk, error } = await supabase
            ...
            .lte('dia', dataFimInt)
            .not('t0_t3', 'is', null)
            .gt('t0_t3', 30)      // MÃ¡Â­nimo 0.5 minutos
            .lt('t0_t3', 1200)    // MÃ¡Â¡ximo 20 minutos
            .order('dia')
            .range(offset, offset + CHUNK_SIZE - 1)
        ...
        // Processar tempos com filtros adicionais e remoÃ¡Â§Ã¡Â£o de outliers
        let tempoCozinha = 0
        ...
        console.log(`ÂÂ±Ã¯Â¸Â ${meses[mes - 1]}: Cozinha ${tempoCozinha.toFixed(1)}min (${allTempoCozinhaData.length} registros), Bar ${tempoBar.toFixed(1)}min (${allTempoBarData.length} registros)`)
      ...
      console.log('Å“â€¦ DADOS MENSAIS CORRETOS processados com sucesso!')
      console.log('Ã°Å¸â€œÅ  Resumo do processamento:')
      ...
      console.error('ÂÅ’ Erro ao buscar dados mensais:', error)
      setDadosMensais([])
    } finally {
      setLoading(false)
    }
    ...
    return cores[info?.color || 'blue']
      ...
      <ProtectedRoute requiredModule="dashboard_metricas_barras">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          <h1 className="text-2xl font-bold text-slate-800 mb-2">Ã°Å¸â€œÅ  Dashboard - MÃ¡Â©tricas Mensais Comparativas</h1>
          <p className="text-slate-600">ComparaÃ¡Â§Ã¡Â£o lado a lado das mÃ¡Â©tricas mensais do {selectedBar?.nome}</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            {/* Resumo Anual */}
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Ã°Å¸â€œË† Resumo Anual {anoSelecionado}</h3>
              <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                {metricas.map((metrica) => {
                  ...
                  const metaAnual = (metas[metrica.metaKey as keyof MetasConfig] ) * (metrica.key === 'faturamento' || metrica.key === 'clientes' || metrica.key === 'reservas' ? 12 : 1)
            ...
            {/* GrÃ¡Â¡ficos de Barras por MÃ¡Â©trica */}
            {metricas.map((metrica) => {
              const maxValor = Math.max(...dadosMensais.map((mes) => mes[metrica.key as keyof DadosMensal] as number), metas[metrica.metaKey as keyof MetasConfig] )
              ...
              return (
                <div key={metrica.key} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                  <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center">
                    ...
                    {metrica.label} - ComparaÃ¡Â§Ã¡Â£o Mensal {anoSelecionado}
                  </h3>
                  
                  <div className="space-y-4">
                    {dadosMensais.map((dados, index) => {
                      const valor = dados[metrica.key as keyof DadosMensal] as number
                      const meta = metas[metrica.metaKey as keyof MetasConfig] 
                      ...
                      return (
                        <div key={index} className="flex items-center space-x-4">
                                  ...
                                  <div className="mt-1 pt-1 border-t border-gray-600">
                                    <span className={`font-semibold ${acimaMeta ? 'text-green-300' : 'text-red-300'}`}>
                                      {(metrica.key === 'tempoCozinha' || metrica.key === 'tempoBar') 
                                        ? (acimaMeta ? 'Å“â€¦ Dentro da meta' : 'ÂÅ’ Fora da meta')
                                        : (acimaMeta ? 'Å“â€¦ Acima da meta' : 'ÂÅ’ Abaixo da meta')
                                      }
                                      {meta > 0 && (
                                        <span className="ml-1">
                          ...
                          <div className="w-8">
                            {valor > 0 ? (acimaMeta ? 'Å“â€¦' : 'Å¡Â Ã¯Â¸Â') : 'ÂÅ’'}
                          </div>
                        </div>
                      )
                    })}
                  ...
                  {/* Legenda e estatÃ¡Â­sticas */}
                  <div className="mt-6 pt-4 border-t border-gray-200">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className="flex items-center space-x-2">
                          <div className="w-3 h-0.5 bg-red-600"></div>
                          <span className="text-xs text-slate-600">Meta: {metrica.unit === 'R$' ? 'R$ ' : ''}{(metas[metrica.metaKey as keyof MetasConfig] ).toLocaleString('pt-BR')} {metrica.unit !== 'R$' ? metrica.unit : ''}</span>
                        </div>
                      </div>
                      <div className="text-xs text-slate-500">
                        {(() => {
                          const mesesComDados = dadosMensais.filter((mes) => (mes[metrica.key as keyof DadosMensal] as number) > 0)
                          const mesesAcimaMeta = mesesComDados.filter((mes) => (mes[metrica.key as keyof DadosMensal] as number) >= (metas[metrica.metaKey as keyof MetasConfig] ))
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Ã°Å¸â€Â Insights Comparativos</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <h4 className="font-semibold text-slate-700">Ã°Å¸Ââ€  Melhores Meses</h4>
                ...
                <div className="space-y-4">
                  <h4 className="font-semibold text-slate-700">Ã°Å¸â€œÅ  Performance Geral</h4>
                  {(() => {
          ...
          <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-800 mb-2">Selecione um ano para anÃ¡Â¡lise</h3>
            <p className="text-slate-500">
              Escolha o ano que deseja analisar e clique em "Analisar" para visualizar a comparaÃ¡Â§Ã¡Â£o mensal.
            </p>
          </div>
        )}
      </div>
