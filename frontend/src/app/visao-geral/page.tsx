...
export default function VisaoGeralPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'coming_soon':
        return (
          <span className="text-xs bg-gray-100 text-gray-800 dark:bg-gray-900/30 dark:text-gray-400 px-2 py-1 rounded-full">
            Em Breve
          </span>
        );
      default:
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {visaoGeralItems.map(item => {
              ...
              return (
                <div
                  key={item.id}
                  className="card-dark hover:shadow-lg transition-shadow duration-300 group"
                >
                  <div className="p-6 pb-3">
                    <div className="flex items-start justify-between mb-3">
                      <div
                        className={`w-10 h-10 bg-gradient-to-r ${getColorClasses(item.color)} rounded-xl flex items-center justify-center shadow-md`}
                      >
          ...
          <div className="card-dark p-6">
            <div className="text-center">
              ...
              <p className="card-description-dark max-w-2xl mx-auto">
                ...
                às suas integrações ativas (Windsor.ai, NIBO, WhatsApp). Use os
                filtros de período para análises customizadas.
              </p>
            </div>
          </div>
        </div>
