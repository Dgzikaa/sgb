...
export default function MetricaEvolucaoPage() {
  ...
  const buscarDadosMetrica = async () => {
    ...
    try {
      ...
      if (metricaSelecionada === 'faturamento') {
        // NOVA ESTRATÉGIA: Buscar faturamento de TODAS as fontes (ContaHub + Sympla + Yuzer)
        console.log('🔍 Buscando faturamento COMPLETO (ContaHub + Sympla + Yuzer) de', dataInicioCorrigida, 'até', periodoFim)
        ...
        try {
          ...
          console.log('💰 Amostra valores:', dados.slice(0, 5).map((d: DadosMetrica) => ({ 
            data: d.data, 
        ...
        try {
          ...
          try {
            ...
            if (allFaturamentoData.length > 0 && clientesData && clientesData.length > 0) {
              ...
              const faturamentoPorDia = allFaturamentoData.reduce((acc: FaturamentoPorDia, item) => {
              ...
              const clientesPorDia = clientesData.reduce((acc: ClientesPorDia, item: PeriodoItem) => {
                const data = item.dt_gerencial
        ...
        try {
          ...
          try {
            ...
            if (allTempoData.length > 0) {
              ...
              const temposPorDia = allTempoData.reduce((acc: {[key: string]: number[]}, item) => {
                ...
                const tempoMinutos = parseInt((item as TempoDataItem)[campoTempo]) / 60
          ...
          try {
            ...
            if (!verError && verificacao && verificacao.length > 0) {
              console.log('🔍 Verificação: Dados disponíveis na tabela tempo:', verificacao.map((d: VerificacaoItem) => d.dia))
  ...
  const metricaInfo = metricas.find(m => m.value === metricaSelecionada)
  const valorTotal = dadosEvolucao.reduce((sum: number, item: DadosMetrica) => sum + item.valor, 0)
  ...
  const maxValor = Math.max(...dadosEvolucao.map((d: DadosMetrica) => d.valor), metaMedia || 0)
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                {(() => {
                  const diasAcimaMeta = dadosEvolucao.filter((item: DadosMetrica) => 
                    item.meta && (
                      (metricaSelecionada === 'tempo_cozinha' || metricaSelecionada === 'tempo_bar') 
                ...
                {dadosEvolucao.map((item: DadosMetrica, index: number) => {
                  ...
                  if (index === 0) {
                    console.log('🎨 RENDERIZANDO GRÁFICO:', {
                      ...
                      todasAsDatas: dadosEvolucao.map((d: DadosMetrica) => d.data),
                      dadosCompletos: dadosEvolucao.map((d: DadosMetrica) => ({ data: d.data, valor: d.valor }))
                    })
                  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  {(() => {
                    ...
                    const mediaPrimeiros = primeirosValores.reduce((sum: number, item: DadosMetrica) => sum + item.valor, 0) / primeirosValores.length
                    const mediaUltimos = ultimosValores.reduce((sum: number, item: DadosMetrica) => sum + item.valor, 0) / ultimosValores.length
