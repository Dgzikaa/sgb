...
export default function ReceitasPage() {
  ...
  const carregarInsumos = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        console.log(`úÖ ${insumosFormatados.length} insumos carregados - Cozinha: ${cozinhaCount}, Bar: ${barCount}`)
      } else {
  ...
  // Fun·ß·£o para sincronizar receitas/insumos do Google Sheets
  const syncReceitasFromSheets = async () => {
    ...
    try {
      ...
      if (data.success) {
        setSyncLog('Sincroniza·ß·£o conclu·≠da!')
        toast.success('Receitas e insumos atualizados do Google Sheets!')
        await recarregarDados()
      } else {
        setSyncLog(data.error || 'Erro desconhecido na sincroniza·ß·£o')
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">ö†Ô∏è Selecione um Bar</h1>
            ...
            <div className="mb-6">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 sm:flex sm:gap-2">
                  ...
                  >
                    üë®Äçüç≥ Cozinha ({insumos.filter((i: any) => i.tipo_local === 'cozinha').length})
                  </Button>
                  <Button
                    ...
                    className="flex items-center justify-center gap-1 sm:gap-2 h-11 text-sm sm:text-base touch-manipulation"
                  >
                    üç∫ Bar ({insumos.filter((i: any) => i.tipo_local === 'bar').length})
                  </Button>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 sm:flex sm:gap-2">
                  <Button
                    ...
                    className="flex items-center justify-center gap-1 sm:gap-2 h-11 text-sm sm:text-base touch-manipulation"
                  >
                    üë®Äçüç≥ Cozinha ({receitas.filter((r: any) => r.tipo_local === 'cozinha').length})
                  </Button>
                  <Button
                    ...
                    className="flex items-center justify-center gap-1 sm:gap-2 h-11 text-sm sm:text-base touch-manipulation"
                  >
                    üç∫ Bar ({receitas.filter((r: any) => r.tipo_local === 'bar').length})
                  </Button>
                </div>
              </div>
            </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
                    ...
                    Receitas ({tipoLocalReceitas === 'bar' ? 'üç∫ Bar' : 'üë®Äçüç≥ Cozinha'})
                  </CardTitle>
                  <Button
              ...
              <CardContent>
                {receitasFiltradas.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      {buscaReceitas ? `Busca por: "${buscaReceitas}"` : 'Clique em "Nova Receita" para come·ßar'}
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        <Dialog open={modalCriarInsumo} onOpenChange={setModalCriarInsumo}>
          <DialogContent className="max-w-2xl w-[95vw] max-h-[90vh] bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 flex flex-col">
            ...
            <div className="space-y-4 flex-1 overflow-y-auto">
              ...
              <div>
                <Label className="text-gray-700 dark:text-gray-300">Observa·ß·µes</Label>
                <Textarea
