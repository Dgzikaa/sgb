...
export default function ContaAzulAutoLogin() {
  ...
  const copyAutoFillScript = async () => {
    const script = `// Ã°Å¸Â¤â€“ Script AutomÃ¡Â¡tico ContaAzul - Cole e pressione Enter
// Email: ${autoLoginData?.email}
// Senha: ${autoLoginData?.password}
// CÃ¡Â³digo 2FA: ${autoLoginData?.totpCode}

(function() {
  console.log('Ã°Å¸Å¡â‚¬ Iniciando preenchimento automÃ¡Â¡tico...');
  ...
  console.log('Å“â€¦ Campos preenchidos! Clicando em login...');
  
  // Clicar no botÃ¡Â£o de login apÃ¡Â³s 500ms
  setTimeout(() => {
    const submitButtons = document.querySelectorAll('button[type="submit"], button.btn-primary, input[type="submit"]');
    if (submitButtons.length > 0) {
      submitButtons[0].click();
      console.log('Å“â€¦ BotÃ¡Â£o de login clicado!');
    }
  }, 500);
  
  // Monitorar campo 2FA
  console.log('Ã°Å¸â€˜â‚¬ Aguardando campo 2FA...');
  const checkFor2FA = setInterval(() => {
    const totpFields = document.querySelectorAll('input[name="totp"], input[name="code"], input[placeholder*="cÃ¡Â³digo" i], input[placeholder*="2fa" i], input[type="text"][maxlength="6"]');
    ...
    if (totpFields.length > 0) {
      totpFields.forEach(field => {
        if (!field.value || field.value.length < 6) {
          ...
          field.dispatchEvent(new Event('input', { bubbles: true }));
          field.dispatchEvent(new Event('change', { bubbles: true }));
          console.log('Å“â€¦ CÃ¡Â³digo 2FA preenchido!');
          
          // Submeter apÃ¡Â³s preencher 2FA
          setTimeout(() => {
            const submit2FA = document.querySelectorAll('button[type="submit"], button.btn-primary');
            if (submit2FA.length > 0) {
              submit2FA[0].click();
              console.log('Å“â€¦ CÃ¡Â³digo 2FA enviado!');
            }
          }, 500);
        }
      });
  ...
  // Parar de verificar apÃ¡Â³s 30 segundos
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          <CardTitle className="text-center">
            Ã°Å¸Â¤â€“ Login AutomÃ¡Â¡tico ContaAzul
          </CardTitle>
        </CardHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="font-mono text-sm">{'â‚¬Â¢'.repeat(8)}</p>
                </div>
                <Button
                  size="sm"
                  variant="outline"
              ...
              <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">VÃ¡Â¡lido por 30 segundos</p>
                </div>
                <Button
                  size="sm"
                  variant="outline"
