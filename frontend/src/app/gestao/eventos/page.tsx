...
function EventForm({
  ...
  const [formData, setFormData] = useState<Partial<Evento>>({
    nome_evento: evento?.nome_evento || evento?.nome || '',
    descricao: '',
    data_evento: '',
    hora_inicio: '',
    hora_fim: '',
  ...
  useEffect(() => {
    if (evento) {
      setFormData({
        ...evento,
        data_evento: evento.data_evento?.split('T')[0] || evento.data?.split('T')[0] || '',
              ...
              {dayEvents.slice(0, 2).map(evento => {
                ...
                return (
                  <div
                    ...
                    title={evento.nome_evento || evento.nome}
                  >
                    <div className="flex items-center gap-1">
                      <span>{categoria.icon}</span>
                      <span className="truncate">{evento.nome_evento || evento.nome}</span>
                    </div>
                  </div>
                );
              })}
