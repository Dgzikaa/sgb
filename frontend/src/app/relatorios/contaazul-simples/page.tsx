...
export default function ContaAzulSimplesPage() {
  ...
  useEffect(() => {
    setPageTitle('ğŸ’° ContaAzul - Dados Financeiros');
    return () => setPageTitle('');
  }, [setPageTitle]);
  ...
  const conectarContaAzul = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setErro(result.error || 'Falha na conexá£o');
      }
    } catch (error) {
      setErro('Erro de conexá£o');
    } finally {
      setLoading(false);
    }
  };
      ...
      {!conectado ? (
        <div className="text-center">
          <div className="bg-blue-50 p-6 rounded-lg mb-6">
            ...
            >
              {loading ? 'ğŸ”„ Conectando...' : 'ğŸš€ Conectar ContaAzul'}
            </button>
          </div>

          {erro && (
            <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded">
              <p className="text-red-700">Œ {erro}</p>
            </div>
          )}
        </div>
      ) : (
        <div className="text-center">
          ...
          {dados && (
            <div className="bg-white p-6 rounded-lg border">
              <h3 className="text-lg font-semibold mb-4">ğŸ“Š Seus Dados:</h3>
