  ...
  const handleDebugDiscrepancias = async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log('✔️ Debug data set:', result.debug_analysis);
        
        // Definir como resultado também para mostrar na interface
        setResult({
          success: true,
          message: '✔️ Análise de discrepâncias concluída',
          logs: [
      ...
      {statusLoading ? (
        ...
        <div className="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded-lg">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-1">ContaHub em Modo Manutenção</h3>
              ...
              <div className="text-xs text-yellow-600 dark:text-yellow-300 space-y-1">
                <div>Email configurado: {contahubStatus?.contahub_status?.detalhes?.email_configurado ? '✔️' : '❌'}</div>
                <div>Senha configurada: {contahubStatus?.contahub_status?.detalhes?.senha_configurada ? '✔️' : '❌'}</div>
              </div>
      ...
      <PageCard>
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          <div className="space-y-3">
            <h3 className="text-sm font-semibold card-title-dark uppercase tracking-wide">Testes Básicos</h3>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold card-title-dark uppercase tracking-wide">Verificações</h3>
      ...
      {result && (
        <div className="mt-6 p-4 card-dark rounded-lg">
            ...
            {result.logs && result.logs.length > 0 && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Logs do Servidor:</h3>
            ...
            {result.estatisticas && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Estatísticas:</h3>
            ...
            {result.resultados && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Resultados da Limpeza:</h3>
                {/* Sistema Raw */}
                ...
                {result.resultados.tabelas_contahub && (
                  <div className="space-y-2">
                    <h4 className="font-medium card-title-dark">❌ Detalhes por Tabela:</h4>
                    {Object.entries(result.resultados.tabelas_contahub).map(([tabela, info]: [string, any]) => (
                      <div key={tabela} className="flex justify-between items-center p-2 bg-gray-100 dark:bg-gray-800 rounded text-sm">
                        ...
                        <span className={info.removidos > 0 ? 'text-green-600 dark:text-green-300' : 'text-gray-500 dark:text-gray-400'}>
                          ...
                          {info.erro && <span className="text-red-500 dark:text-red-400 ml-2">❌ {info.erro}</span>}
                        </span>
                      </div>
                    ))}
                  </div>
            ...
            {result.totais && (
              <div className="mb-4 p-4 bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded">
                <h3 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-2">❌ Resultados da Limpeza Completa</h3>
            ...
            {result.amostra_estrutura && (
              <div className="mb-4 card-dark">
                <h3 className="font-semibold card-title-dark mb-2">❌ Debug de Logs e Status:</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  ...
                  <div className="bg-yellow-50 dark:bg-yellow-900 p-3 rounded">
                    <div className="text-sm text-yellow-600 dark:text-yellow-200">Não Processados</div>
                    <div className="font-bold text-yellow-800 dark:text-yellow-100">{result.estatisticas?.nao_processados || 0}</div>
                  </div>
                  <div className="bg-red-50 dark:bg-red-900 p-3 rounded">
                    <div className="text-sm text-red-600 dark:text-red-300">Problemáticos</div>
                ...
                {result.estrutura_tabela && (
                  <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-800 rounded">
                    <div className="font-medium card-title-dark mb-2">❌ Estrutura da Tabela sistema_raw:</div>
                ...
                {result.estatisticas?.contagem_por_tipo && (
                  <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-800 rounded">
                    <div className="font-medium card-title-dark mb-2">❌ Contagem por Tipo:</div>
                ...
                {result.ultimos_registros && result.ultimos_registros.length > 0 && (
                  <div className="mb-4">
                    <div className="font-medium card-title-dark mb-2">❌ Últimos Registros:</div>
                    <div className="space-y-2">
                ...
                {result.estrutura_tabela?.exemplo_registro && (
                  <div className="mb-4">
                    <div className="font-medium card-title-dark mb-2">❌ Exemplo de Registro:</div>
            ...
            {result.analise && (
              <div className="mb-4 card-dark">
                <h3 className="font-semibold card-title-dark mb-2">❌ Análise da Estrutura dos Dados:</h3>
                <div className="mb-3 p-3 bg-blue-50 dark:bg-blue-900 rounded">
                ...
                {result.analise.amostras_por_tipo && Object.entries(result.analise.amostras_por_tipo).map(([tipo, amostra]: [string, any], index: number) => (
                  <div key={index} className="mb-4 p-4 bg-white dark:bg-gray-900 border dark:border-gray-700 rounded">
                    ...
                    <div className="space-y-3">
                      {/* Estrutura principal */}
                      <div>
                        <div className="font-medium card-title-dark mb-1">❌ Estrutura Principal:</div>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div className={`p-2 rounded text-sm ${amostra.estrutura_json.tem_metadados ? 'bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-300'}`}>
                          ...
                          <div>{amostra.estrutura_json.tem_metadados ? '✔️ Sim' : '❌ Não'}</div>
                        </div>
                        <div className={`p-2 rounded text-sm ${amostra.estrutura_json.tem_list ? 'bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900 text-red-700 dark:text-red-300'}`}>
                          ...
                          <div>{amostra.estrutura_json.tem_list ? '✔️ Sim' : '❌ Não'}</div>
                        </div>
                        <div className={`p-2 rounded text-sm ${amostra.estrutura_json.list_e_array ? 'bg-green-50 dark:bg-green-900 text-green-700 dark:text-green-300' : 'bg-yellow-50 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-200'}`}>
                          <div className="font-medium card-title-dark">✔️ Array</div>
                          <div>{amostra.estrutura_json.list_e_array ? '✔️ Sim' : '❌ ' + amostra.estrutura_json.tipo_list}</div>
                        </div>
                      ...
                      {amostra.estrutura_json.metadados_info && (
                        <div className="p-3 bg-blue-50 dark:bg-blue-900 rounded">
                          <div className="font-medium text-blue-800 dark:text-blue-200 mb-2">❌ Informações dos Metadados:</div>
                      ...
                      {amostra.estrutura_json.list_info && (
                        <div className="p-3 bg-green-50 dark:bg-green-900 rounded">
                          <div className="font-medium text-green-800 dark:text-green-200 mb-2">❌ Informações da List:</div>
...
{JSON.stringify(amostra.estrutura_json.list_info.primeiro_item_sample, null, 2)}
            ...
            {result.resultados && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Resultado:</h3>
            ...
            {result.logs && result.logs.length > 0 && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Logs da Execução:</h3>
            ...
            {result.logs && result.logs.length > 0 && (
              <div className="mb-4">
                <h3 className="font-semibold text-red-600 dark:text-red-400 mb-2">❌ Erros da Execução:</h3>
            ...
            {result.status && (
              <div>
                <h3 className="font-semibold card-title-dark mb-2">❌ Status das Tabelas:</h3>
            ...
            {result.resultados && Object.keys(result.resultados).length > 0 && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Teste de Campos das Tabelas:</h3>
            ...
            {debugData && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Análise de Discrepâncias dos Dados</h3>
                
                {/* Análise Clientes CPF */}
                {debugData.clientes_cpf && (
                  <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg mb-4">
                    <h4 className="font-semibold text-lg mb-3 card-title-dark">❌ Clientes CPF:</h4>
                    ...
                    {debugData.clientes_cpf.analise?.cpfsExtras?.length > 0 && (
                      <div className="mb-4 p-3 bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 rounded">
                        <h5 className="font-semibold text-red-800 dark:text-red-200 mb-2">❌ CPFs Extras (não deveriam estar no banco):</h5>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    ...
                    {debugData.clientes_cpf.analise?.cpfsFaltando?.length > 0 && (
                      <div className="mb-4 p-3 bg-orange-50 dark:bg-orange-900 border border-orange-200 dark:border-orange-700 rounded">
                        <h5 className="font-semibold text-orange-800 dark:text-orange-200 mb-2">❌ CPFs Faltando (deveriam estar no banco):</h5>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    ...
                    {debugData.clientes_cpf.duplicatas?.cpfs_duplicados?.length > 0 && (
                      <div className="mb-4 p-3 bg-yellow-50 dark:bg-yellow-900 border border-yellow-200 dark:border-yellow-700 rounded">
                        <h5 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-2">❌ CPFs Duplicados:</h5>
                    ...
                    {debugData.clientes_cpf.duplicatas?.raw_ids_duplicados?.length > 0 && (
                      <div className="mb-4 p-3 bg-purple-50 dark:bg-purple-900 border border-purple-200 dark:border-purple-700 rounded">
                        <h5 className="font-semibold text-purple-800 dark:text-purple-200 mb-2">❌ Raw IDs com Múltiplos Registros:</h5>
                        <div className="space-y-1">
                    ...
                    <details className="mt-4">
                      ...
                      <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div>
                          <h6 className="font-semibold text-green-800 dark:text-green-200 mb-2">✔️ Dados Esperados (ContaHub):</h6>
                        ...
                        <div>
                          <h6 className="font-semibold text-blue-800 dark:text-blue-200 mb-2">❌ Dados Reais (Banco):</h6>
                ...
                {debugData.periodo && (
                  <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg mb-4">
                    <h4 className="font-semibold text-lg mb-3 card-title-dark">❌ Período:</h4>
                    ...
                    {debugData.periodo.analise?.vdsExtras?.length > 0 && (
                      <div className="mb-4 p-3 bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 rounded">
                        <h5 className="font-semibold text-red-800 dark:text-red-200 mb-2">❌ VDs Extras (não esperados):</h5>
                    ...
                    {debugData.periodo.analise?.vdsFaltando?.length > 0 && (
                      <div className="mb-4 p-3 bg-orange-50 dark:bg-orange-900 border border-orange-200 dark:border-orange-700 rounded">
                        <h5 className="font-semibold text-orange-800 dark:text-orange-200 mb-2">❌ VDs Faltando:</h5>
                        <div className="grid grid-cols-4 md:grid-cols-6 gap-2">
                    ...
                    <details className="mt-4">
                      ...
                      <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div>
                          <h6 className="font-semibold text-blue-800 dark:text-blue-200 mb-2">✔️ Primeiros 10 Esperados:</h6>
                          <div className="bg-blue-50 dark:bg-blue-900 p-3 rounded max-h-60 overflow-y-auto">
                            <pre className="text-xs text-blue-800 dark:text-blue-200">
                        ...
                        <div>
                          <h6 className="font-semibold text-green-800 dark:text-green-200 mb-2">❌ Primeiros 10 Reais:</h6>
                          <div className="bg-green-50 dark:bg-green-900 p-3 rounded max-h-60 overflow-y-auto">
                            <pre className="text-xs text-green-800 dark:text-green-200">
                ...
                {debugData.tempo && (
                  <div className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                    <h4 className="font-semibold text-lg mb-3 card-title-dark">❌ Tempo (Apenas Dados Reais):</h4>
            ...
            {result.resultados && Object.entries(result.resultados).some(([tabela, info]: [string, any]) => info.campos_existentes) && (
              <div className="mb-4">
                <h3 className="font-semibold card-title-dark mb-2">❌ Descoberta de Campos Reais:</h3>
                        ...
                        <div className="flex justify-between items-center mb-3">
                          ...
                          <div className="flex space-x-2">
                            <span className="px-2 py-1 bg-green-100 dark:bg-green-800 text-green-800 dark:text-green-200 rounded text-sm font-medium">
                              ✔️ {info.total_existentes} existem
                            </span>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          ...
                          <div>
                            <h5 className="text-sm font-semibold text-green-700 dark:text-green-300 mb-2">
                              ✔️ Campos que EXISTEM ({info.total_existentes}):
                            </h5>
            ...
            {result.analises && result.analises.length > 0 && (
              <div className="mb-4">
                ...
                {result.resumo && (
                  <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900 rounded">
                    <h4 className="font-semibold text-blue-800 dark:text-blue-200 mb-2">❌ Resumo Geral:</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        <div className="font-medium text-blue-700 dark:text-blue-300">Datas de referência:</div>
                  ...
                  {result.analises.map((analise, index: number) => (
                    <div key={analise.id} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                        <div>
                          <h5 className="text-sm font-semibold card-description-dark mb-2">❌ Estrutura do Registro:</h5>
                          <div className="bg-gray-50 dark:bg-gray-800 p-2 rounded text-xs">
                            ...
                            <div><strong>Tem dados:</strong> {analise.estrutura_registro?.tem_dados ? '✔️' : '❌'}</div>
                            <div><strong>Tipo de dados:</strong> {analise.estrutura_registro?.tipo_dados_campo}</div>
                            <div><strong>✔️ objeto:</strong> {analise.estrutura_registro?.dados_e_objeto ? '✔️' : '❌'}</div>
                          </div>
                        </div>

                        <div>
                          <h5 className="text-sm font-semibold card-description-dark mb-2">❌ Campos de Identificação:</h5>
                          <div className="bg-gray-50 dark:bg-gray-800 p-2 rounded text-xs">
                            {typeof analise.campos_identificacao === 'object' && analise.campos_identificacao !== null ? (
                              <>
                                <div><strong>vd:</strong> {analise.campos_identificacao.tem_vd ? '✔️' : '❌'}</div>
                                <div><strong>trn:</strong> {analise.campos_identificacao.tem_trn ? '✔️' : '❌'}</div>
                                <div><strong>dt_gerencial:</strong> {analise.campos_identificacao.tem_dt_gerencial ? '✔️' : '❌'}</div>
                                <div><strong>pag:</strong> {analise.campos_identificacao.tem_pag ? '✔️' : '❌'}</div>
                                <div><strong>itm/prd:</strong> {analise.campos_identificacao.tem_itm ? '✔️' : '❌'} / {analise.campos_identificacao.tem_prd ? '✔️' : '❌'}</div>
                                <div><strong>hora/dds:</strong> {analise.campos_identificacao.tem_hora ? '✔️' : '❌'} / {analise.campos_identificacao.tem_dds ? '✔️' : '❌'}</div>
                      ...
                      {/* Wrapper format */}
                      <div className="mb-4">
                        <h5 className="text-sm font-semibold card-description-dark mb-2">❌ Wrapper Format:</h5>
                        <div className="bg-gray-50 dark:bg-gray-800 p-2 rounded text-xs">
                          <div><strong>Tem wrapper:</strong> {analise.tem_wrapper ? '✔️ SIM' : '❌ NÃO'}</div>
                        </div>
                      </div>
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            <PageText variant="body">
              Período: 31/01/2025 até 04/02/2025 (5 dias)
            </PageText>
            <PageText variant="body">
            ...
            <PageText variant="body">
              Processar Dados: Processa dados da sistema_raw para tabelas específicas
            </PageText>
            <PageText variant="body">
              Verificar Status: Mostra status das tabelas ContaHub
            </PageText>
            <PageText variant="body">
              Debug Dados: Analisa a estrutura dos dados salvos na sistema_raw para entender campos disponíveis
            </PageText>
            <PageText variant="body">
