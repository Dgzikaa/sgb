...
interface ItemChecklist {
  ...
  validacao?: Record<string, string | boolean>
}
...
function detectarMudancas(original: ChecklistData, atual: ChecklistData): MudancasDetectadas {
  ...
  if (atual.frequencia !== original.frequencia) {
    mudancas.push(`Frequência: "${original.frequencia}" → "${atual.frequencia}"`)
  }
  ...
  if (secoesOriginais.length !== secoesAtuais.length) {
    mudancas.push(`Número de seções: ${secoesOriginais.length} → ${secoesAtuais.length}`)
    ...
    itensAtuais.forEach((itemAtual, itemIndex) => {
      ...
      if (!itemOriginal) {
        ...
        if (itemAtual.obrigatorio !== itemOriginal.obrigatorio) {
          mudancas.push(`"${itemAtual.titulo}": ${itemOriginal.obrigatorio ? 'Obrigatório → Opcional' : 'Opcional → Obrigatório'}`)
        }
      }
    })
  })
