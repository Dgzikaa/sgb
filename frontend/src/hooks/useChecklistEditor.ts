...
export function useChecklistEditor(
  ...
  const carregarChecklist = useCallback(async (): Promise<void> => {
    try {
      setLoading(true);
      setError(null);
  ...
  const carregarVersoes = useCallback(async (): Promise<void> => {
    try {
  ...
  }, [checklistId, carregarChecklist, carregarVersoes]);
