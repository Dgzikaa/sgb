...
export function useChecklistExecution(): UseChecklistExecutionResult {
  ...
  const salvarRespostas = useCallback(
    async (autoSave: boolean = false): Promise<boolean> => {
  ...
  // Auto-save quando execução muda
  useEffect(() => {
    if (autoSaveEnabled && execucao && temAlteracoesPendentes) {
  ...
  const cancelarExecucao = async (motivo?: string): Promise<boolean> => {
    ...
    try {
      const response = await api.delete(
        `/api/execucoes/${execucao.id}?motivo=${encodeURIComponent(motivo || 'Cancelado pelo usuário')}`
      );
