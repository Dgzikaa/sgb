...
interface UseChecklistExecutionResult {
  ...
  finalizarExecucao: (observacoesFinais?: string, assinatura?: Record<string, unknown>) => Promise<boolean>
  cancelarExecucao: (motivo?: string) => Promise<boolean>
  
  // Edição de respostas
  atualizarResposta: (secaoIndex: number, itemIndex: number, valor: unknown, anexos?: Record<string, unknown>[]) => void
  adicionarAnexo: (secaoIndex: number, itemIndex: number, anexo: Record<string, unknown>) => void
...
export function useChecklistExecution(): UseChecklistExecutionResult {
  ...
  const finalizarExecucao = async (observacoesFinais?: string, assinatura?: Record<string, unknown>): Promise<boolean> => {
    ...
    try {
      ...
      const response = await api.post(`/api/checklists/execucoes/${execucao.id}/finalizar`, {
  ...
  const atualizarResposta = (secaoIndex: number, itemIndex: number, valor: unknown, anexos?: Record<string, unknown>[]): void => {
    if (!execucao) return
  ...
  const adicionarAnexo = (secaoIndex: number, itemIndex: number, anexo: Record<string, unknown>): void => {
    if (!execucao) return
...
export function useChecklistExecutions() {
  ...
  const carregarExecucoes = async (filtros?: unknown) => {
    try {
      setLoading(true)
      setError(null)
