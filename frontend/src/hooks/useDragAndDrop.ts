...
export function useDragAndDrop({
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent | React.TouchEvent, item: DraggableItem) => {
      if (disabled) return;
  ...
  const handleDragLeave = useCallback(
    (e: React.DragEvent | React.TouchEvent) => {
      ...
      const relatedTarget =
        'relatedTarget' in e ? (e.relatedTarget as HTMLElement) : null;
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent | React.TouchEvent, targetItem: DraggableItem) => {
      if (disabled) return;
      ...
      if (
        onReorder &&
        typeof draggedData.index === 'number' &&
        typeof targetItem.index === 'number'
      ) {
        onReorder(draggedData.index, targetItem.index, [
          draggedData,
          targetItem,
        ]);
      }
