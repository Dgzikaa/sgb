...
export function useTemplates(
  ...
  const deletarTemplate = async (id: string): Promise<boolean> => {
    try {
      const response = await api.delete(`/api/configuracoes/templates/${id}`);
...
export function useTemplate(id?: string): UseTemplateResult {
  ...
  const salvarTemplate = async (data: Partial<Template>): Promise<boolean> => {
    ...
    try {
      const response = await api.put(`/api/configuracoes/templates/${id}`, data);
  ...
  const criarTemplate = async (
    ...
    try {
      const response = await api.post('/api/configuracoes/templates', data);
...
export const templateUtils = {
  ...
  criarTemplateDeChecklist: (
    checklist: Record<string, unknown>
  ): Partial<Template> => {
    return {
      nome: `Template - ${checklist.nome as string}`,
      descricao: `Template criado a partir do checklist: ${checklist.nome as string}`,
      categoria: 'geral',
      setor: (checklist.setor as string) || 'geral',
      tipo: 'qualidade',
      frequencia: 'conforme_necessario',
      tempo_estimado: 30,
      publico: false,
      tags: [],
      estrutura: (checklist.estrutura as {
        secoes: Array<{
          nome: string;
          descricao?: string;
          cor: string;
