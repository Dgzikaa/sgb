...
export function useAnalyticsTracker(): UseAnalyticsTrackerReturn {
  ...
  // Detectar informaÃ¡Â§Ã¡Âµes do dispositivo
  ...
  // FunÃ¡Â§Ã¡Â£o para enviar eventos em lote
  ...
  // FunÃ¡Â§Ã¡Â£o principal para tracking
  ...
  // Tracking automÃ¡Â¡tico de page view
  const trackPageView = useCallback((pagina?: string) => {
    // Calcular tempo na pÃ¡Â¡gina anterior
    ...
    // Reset timer para nova pÃ¡Â¡gina
    pageStartTime.current = Date.now()
  }, [pathname, trackEvent])

  // Tracking de cliques
  const trackClick = useCallback((elemento: string, dados?: Record<string, unknown>) => {
    trackEvent({
      evento_tipo: 'click',
  ...
  // Tracking de aÃ¡Â§Ã¡Âµes
  const trackAction = useCallback((acao: string, dados?: Record<string, unknown>) => {
    trackEvent({
      evento_tipo: 'action',
      evento_nome: acao,
      dados_evento: dados
  ...
  // Tracking de erros
  const trackError = useCallback((erro: string, contexto?: Record<string, unknown>) => {
    trackEvent({
      evento_tipo: 'error',
  ...
  useEffect(() => {
    trackPageView()
  }, [pathname]) // Removido trackPageView da dependÃ¡Âªncia para evitar loop
...
// Hook simplificado para uso rÃ¡Â¡pido
