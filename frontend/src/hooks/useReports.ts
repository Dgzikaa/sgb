...
export function useReports(): UseReportsResult {
    ...
    async (filtros: FiltrosTemplates = {}) => {
      try {
        ...
        const response = await api.get(
          `/api/reports/templates?${params.toString()}`
        );
  ...
  const carregarExecucoes = useCallback(
    async (filtros: FiltrosExecucoes = {}) => {
      try {
        ...
        const response = await api.get(
          `/api/reports/execute?${params.toString()}`
        );
  ...
  const cancelarExecucao = useCallback(
    async (id: string): Promise<boolean> => {
      try {
        ...
        const response = await api.put(`/api/reports/execute/${id}`, {
          action: 'cancel',
        });
  ...
  const formatarDadosParaExportacao = useCallback(
    (dados: unknown[], template: Template): unknown[] => {
          ...
          switch (config.tipo) {
            ...
            case 'numero':
              itemFormatado[config.label || campo] = config.decimais
                ? parseFloat(valor).toFixed(config.decimais)
                : parseInt(valor);
              break;
            case 'data_hora':
              itemFormatado[config.label || campo] = valor
