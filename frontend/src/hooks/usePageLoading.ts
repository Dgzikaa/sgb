...
export function usePageLoading(
  ...
  const startLoading = useCallback(
    (type: SkeletonType = initialType, message?: string) => {
      setState(prev => ({
        ...prev,
        isLoading: true,
        skeletonType: type,
...
export function useMultipleLoading() {
  const [loadingStates, setLoadingStates] = useState<
    Record<string, PageLoadingState>
  >({});

  const startLoading = useCallback(
    (key: string, type: SkeletonType = 'fullscreen', message?: string) => {
