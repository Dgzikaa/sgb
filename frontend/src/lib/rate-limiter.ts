...
const RATE_LIMITS: Record<string, { requests: number; window: number }> = {
  ...
  'default': { requests: 100, window: 60000 }         // 100 requests por minuto padr·£o
};
...
// Fun·ß·£o para notificar Discord sobre rate limiting
...
export function createRateLimiter(endpoint: string) {
  return async (request: NextRequest): Promise<NextResponse | null> => {
    try {
      ...
      // Pegar configura·ß·£o do endpoint
      ...
      if (!rateLimitResult.success) {
        ...
        console.warn(`üö´ Rate limit excedido - IP: ${ip}, Endpoint: ${endpoint}`);
...
export async function getRateLimitStatus(): Promise<{
  ...
  try {
    ...
    console.error('ùå Erro ao verificar status rate limit:', error);
    return {
