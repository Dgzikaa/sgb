...
export const ExemplosCondicionais = {
  ...
  seTemperaturaForaObrigarFoto: (
    itemTemperatura: string,
    itemFoto: string,
    minTemp: number,
    maxTemp: number
  ) =>
    criarRegraCondicionalMultipla(
      ...
      [
        {
          itemDependencia: itemTemperatura,
          operador: 'menor_que',
          valor: minTemp,
          acao: 'obrigar',
        },
  ...
  seAvaliacaoBaixaObrigarJustificativa: (
    itemAvaliacao: string,
    itemJustificativa: string
  ) =>
    criarRegraCondicional(
      itemJustificativa,
      itemAvaliacao,
  ...
  seEquipamentoNaoFuncionandoMostrarManutencao: (
    itemEquipamento: string,
    itensManutencao: string[]
  ) =>
  ...
  seTipoProblemaShowCampos: (
    itemTipoProblema: string,
    valor: string,
...
export function useConditionalLogic(
  ...
  const obterItensObrigatorios = (
    itens: ItemCondicional[]
  ): ItemCondicional[] => {
    return itens.filter(
      item => item.visivel && (item.obrigatorio || item.obrigatorioCondicional)
    );
  };

  const validarItensObrigatorios = (
    itens: ItemCondicional[]
  ): {
    valido: boolean;
    itensVazios: string[];
