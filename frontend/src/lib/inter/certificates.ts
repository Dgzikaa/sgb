...
export function getInterCertificates(): { cert: Buffer; key: Buffer } {
  ...
  if (process.env.INTER_CERT_BASE64 && process.env.INTER_KEY_BASE64) {
    ...
    console.log('🔑 Chave privada Base64 (env) carregada:', key.length, 'bytes')
  ...
  const certPath = path.join(process.cwd(), 'src', 'lib', 'inter', 'fullchain.pem')
  ...
  const certBase64Path = path.join(process.cwd(), 'public', 'inter', 'cert_base64.txt')
  const keyBase64Path = path.join(process.cwd(), 'public', 'inter', 'key_base64.txt')
  ...
  throw new Error('Certificados mTLS não encontrados. Configure INTER_CERT_BASE64 e INTER_KEY_BASE64 no Vercel.')
} 