...
export async function getProdutoMaisVendido(periodo: 'hoje' | 'semana' | 'mes' = 'hoje'): Promise<ProdutoMaisVendido | null> {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      console.log('⚠️ Nenhum dado encontrado na tabela analitico para o período')
      return null
    }
    
    console.log(`📊 Registros encontrados: ${data.length}`)
    console.log(`📅 Período dos dados: ${data[0]?.vd_dtgerencial} a ${data[data.length-1]?.vd_dtgerencial}`)
    
    // Agrupar produtos e somar quantidades/valores
    const produtosAgrupados = data.reduce((acc: Record<string, ProdutoAgrupado>, item: AnaliticoItem) => {
...
export async function getVendasData(): Promise<VendasData> {
  ...
  try {
    ...
    // Calcular totais
    const totalHoje = vendasHoje?.reduce((sum: number, venda: VendaItem) => sum + parseFloat(String(venda.valorfinal)), 0) || 0
    const totalSemana = vendasSemana?.reduce((sum: number, venda: VendaItem) => sum + parseFloat(String(venda.valorfinal)), 0) || 0
...
export async function getClientesData(): Promise<ClientesData> {
  ...
  try {
    ...
    const clientesAntigos = clientesSemana?.filter((cliente: ClienteItem) => 
    ...
    console.log('👥 Clientes calculados baseado em mesas únicas:', {
      ...
      mesasUnicas: clientesHoje?.map((item: ClienteItem) => item.id).slice(0, 5) // mostrar primeiras 5 mesas
    })
  ...
  try {
    ...
    const dadosPromises = diasSemana.map(async (data, index) => {
      ...
      try {
        ...
        const faturamentoPagamentos = pagamentosData.data?.reduce((sum: number, item: { liquido?: string }) => sum + parseFloat(item.liquido || '0'), 0) || 0
        const faturamentoSympla = symplaData.data?.reduce((sum: number, item: { total_liquido?: string }) => sum + parseFloat(item.total_liquido || '0'), 0) || 0
        const faturamentoTotal = faturamentoPagamentos + faturamentoSympla

        const pessoasPeriodo = periodoData.data?.reduce((sum: number, item: { pessoas?: string }) => sum + parseInt(item.pessoas || '0'), 0) || 0
        const pessoasSympla = symplaData.data?.reduce((sum: number, item: { qtd_checkins_realizados?: string }) => sum + parseInt(item.qtd_checkins_realizados || '0'), 0) || 0
...
export async function getHistoricoDiaSemana(diaSemana: string, ultimasSemanas = 8): Promise<HistoricoDia[]> {
  ...
  try {
    console.log(`📈 Buscando histórico de ${diaSemana} das últimas ${ultimasSemanas} semanas`)
    ...
    const dadosFiltratos: Record<string, { data: string; faturamento: number; clientes: number }> = {}
    
    pagamentosData.data?.forEach((item: { dt_gerencial: string; liquido: string }) => {
    ...
    periodoData.data?.forEach((item: { dt_gerencial: string; pessoas: string }) => {
      const data = new Date(item.dt_gerencial)
      if (data.getDay() === diaSemanaIndex) {
        const dataStr = item.dt_gerencial
        if (!dadosFiltratos[dataStr]) {
    ...
    console.log(`📈 Histórico de ${diaSemana}:`, resultados.length, 'registros encontrados')
    return resultados
...
export async function getComparacaoPeriodos(periodo1: [string, string], periodo2: [string, string]): Promise<ComparacaoPeriodos> {
  ...
  try {
    ...
    const calcularPeriodo = async ([inicio, fim]: [string, string]) => {
      ...
      const faturamento = data?.reduce((sum: number, item: VendaItem) => sum + parseFloat(String(item.valorfinal)), 0) || 0
      const clientes = new Set(data?.map((item: VendaItem) => item.vd) || []).size
      const diasAtivos = new Set(data?.map((item: VendaItem) => item.vd_dtgerencial) || []).size
