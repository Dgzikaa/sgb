...
export class DiscordMarketingService {
  ...
  private formatRateLimitInfo(rateLimitInfo?: {
    ...
    const businessStatus = businessUsage < 30 ? '🟢' : businessUsage < 70 ? '⚠️' : '🔴';
    const platformStatus = platformUsage < 30 ? '🟢' : platformUsage < 70 ? '⚠️' : '🔴';
...
// ========================================
// Tipo para resultado de coleta concluída
interface ColetaConcluidaResultado {
  facebook_metricas: boolean;
  instagram_metricas: boolean;
  facebook_posts: number;
  instagram_posts: number;
...
type NotifyMarketingUpdateData =
  | { iniciando: boolean; tipo: 'manual' | 'automatica'; resultado?: ColetaConcluidaResultado }
  | { tipo: string; mensagem: string; detalhes?: string; acao_sugerida?: string }
  | { tipo: 'seguidores' | 'engajamento' | 'alcance'; plataforma: 'facebook' | 'instagram'; valor_atual: number; marco_atingido: number; crescimento: number }
  | { metrics: MarketingMetrics };

export async function notifyMarketingUpdate(tipo: 'coleta' | 'erro' | 'marco' | 'relatorio', dados: NotifyMarketingUpdateData): Promise<boolean> {
  try {
    switch (tipo) {
      case 'coleta':
        if ('iniciando' in dados && dados.iniciando) {
          return await sgbMarketingBot.notificarColetaIniciada(dados.tipo );
        } else if ('resultado' in dados && dados.resultado) {
      ...
      case 'marco':
        if (
          'tipo' in dados &&
          (dados.tipo === 'seguidores' || dados.tipo === 'engajamento' || dados.tipo === 'alcance') &&
          'plataforma' in dados &&
          'valor_atual' in dados &&
          'marco_atingido' in dados &&
          'crescimento' in dados
        ) {
