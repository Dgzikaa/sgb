...
export function calcularScoreSecao(secao: SecaoResposta): {
  score: number;
  maxScore: number;
  percentual: number;
} {
...
export function calcularScoreItem(item: ItemResposta): {
  score: number;
  maxScore: number;
  percentual: number;
} {
...
export function calcularScoreTotal(execucao: ExecucaoData): {
  score: number;
  maxScore: number;
  percentual: number;
} {
...
export function calcularProgresso(execucao: ExecucaoData): ProgressoExecucao {
  ...
  execucao.respostas.secoes.forEach((secao: SecaoResposta) => {
    ...
    const respondidosSecao = secao.itens.filter(
      (item: ItemResposta) => item.respondido
    ).length;
  ...
  const percentualConcluido =
    totalItens > 0 ? (itensRespondidos / totalItens) * 100 : 0;

  // Calcular tempo estimado restante
  const tempoDecorrido = Date.now() - new Date(execucao.iniciado_em).getTime();
  const tempoEstimadoRestante =
...
export function validarRespostasObrigatorias(execucao: ExecucaoData): {
  valido: boolean;
  erros: string[];
} {
  const erros: string[] = [];
...
export function gerarRelatorioExecucao(
  ...
  const relatorioSecoes = execucao.respostas.secoes.map(
    (secao: SecaoResposta) => {
