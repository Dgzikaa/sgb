...
class GoogleReviewsClient {
  ...
  async getBusinessReviews(params: {
    ...
    try {
      console.log('üåü Buscando reviews do Google:', params)
      ...
      if (!response.ok) {
        if (response.status === 503) {
          console.warn('ö†Ô∏è Google Places API n·£o configurada')
          return {
            success: false,
      ...
      console.log('úÖ Reviews obtidos:', data.data)
      return data
  ...
  /**
   * Busca reviews para m·∫ltiplos estabelecimentos (para compara·ß·£o)
   */
  async getMultipleBusinessReviews(businesses: Array<{
    ...
    for (const business of businesses) {
      try {
        ...
        console.error(`ùå Erro ao buscar reviews para ${business.name}:`, error)
        results.push({
          success: false,
