...
export class DiscordChecklistService {
  ...
  static async sendCompletion(execution: ChecklistExecution): Promise<boolean> {
    ...
    const timeEmoji = execution.tempo_execucao <= 30 ? "⚡️" : execution.tempo_execucao <= 60 ? "🏆" : "🎉"
  ...
  static async sendDailyReport(stats: ChecklistStats): Promise<boolean> {
    ...
    const performance_emoji = taxa_conclusao >= 90 ? "🏆" : taxa_conclusao >= 70 ? "👍" : "⚡️"
  ...
  static async sendWeeklyReport(weeklyStats: Record<string, unknown>): Promise<boolean> {
    const embed = {
      ...
      fields: [
        {
          name: "👍 Tendência",
          value: (weeklyStats.tendencia as string) || "Estável",
          inline: true
        },
        {
          name: "🏆 Melhor Dia",
        ...
        {
          name: "🎯 Meta Semanal",
          value: (weeklyStats.meta_atingida as boolean) ? "✅ Atingida" : "❌ Não atingida",
          inline: true
        }
      ],
      footer: {
        text: `SGB Analytics • Semana ${(weeklyStats.semana as string) || 'Atual'}`
      },
      timestamp: new Date().toISOString()
    }
    ...
    const embed = {
      ...
      fields: [
        ...
        },
        {
          ...
          value: Array.isArray(anomalia.acoes_sugeridas) ? (anomalia.acoes_sugeridas as string[]).join(', ') : "Monitorar situação",
          inline: false
        }
      ],
      footer: {
  ...
  private static async sendWebhook(payload: Record<string, unknown>): Promise<boolean> {
    try {
      ...
      if (response.ok) {
        ...
        console.error('❌ Erro ao enviar Discord webhook:', response.status, await response.text())
        return false
      }
    } catch (error) {
