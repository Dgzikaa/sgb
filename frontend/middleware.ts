...
function getStoredUser(request: NextRequest): User | null {
  try {
    ...
    const userData = JSON.parse(decodeURIComponent(userDataCookie)) as unknown as User
    return userData
  } catch (error) {
...
function hasMarketingPermission(user: User): boolean {
  ...
  console.log(`🔍 Tem módulo 18? ${hasModule18 ? 'Sim' : 'Não'}`)
  console.log(`🔍 Tem módulo marketing_360? ${hasModuleMarketing ? 'Sim' : 'Não'}`)
  
  const hasPermission = hasModule18 || hasModuleMarketing
  console.log(`🔍 RESULTADO FINAL: ${hasPermission ? 'Sim' : 'Não'}`)
