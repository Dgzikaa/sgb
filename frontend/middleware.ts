...
function hasMarketingPermission(user: User): boolean {
  ...
  console.log(`ðŸ” MÃ“DULOS: ${JSON.stringify(user.modulos_permitidos)}`)
  ...
  console.log(`ðŸ” Tem mÃ³dulo 18? ${hasModule18}`)
  console.log(`ðŸ” Tem mÃ³dulo marketing_360? ${hasModuleMarketing}`)
...
export function middleware(request: NextRequest) {
  ...
  if (pathname === '/visao-geral/marketing-360') {
    ...
    if (!user) {
      console.log('ðŸš« MIDDLEWARE: UsuÃ¡rio nÃ£o autenticado')
      return NextResponse.redirect(new URL('/login', request.url))
    }
    
    console.log(`ðŸ‘¤ USUÃRIO ENCONTRADO: ${JSON.stringify(user)}`)
    
    if (!hasMarketingPermission(user)) {
      console.log(`ðŸš« MIDDLEWARE: UsuÃ¡rio ${user.nome} (${user.role}) sem permissÃ£o para marketing`)
