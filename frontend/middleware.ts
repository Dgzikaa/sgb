...
function hasMarketingPermission(user: User): boolean {
  ...
  console.log(`ğŸ” Má“DULOS: ${JSON.stringify(user.modulos_permitidos)}`)
  ...
  console.log(`ğŸ” Tem má³dulo 18? ${hasModule18}`)
  console.log(`ğŸ” Tem má³dulo marketing_360? ${hasModuleMarketing}`)
...
export function middleware(request: NextRequest) {
  ...
  if (pathname === '/visao-geral/marketing-360') {
    ...
    if (!user) {
      console.log('ğŸš« MIDDLEWARE: Usuá¡rio ná£o autenticado')
      return NextResponse.redirect(new URL('/login', request.url))
    }
    
    console.log(`ğŸ‘¤ USUáRIO ENCONTRADO: ${JSON.stringify(user)}`)
    
    if (!hasMarketingPermission(user)) {
      console.log(`ğŸš« MIDDLEWARE: Usuá¡rio ${user.nome} (${user.role}) sem permissá£o para marketing`)
