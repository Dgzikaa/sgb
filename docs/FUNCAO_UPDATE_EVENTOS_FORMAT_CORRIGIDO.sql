...
BEGIN
    ...
    -- Receita total (contahub_pagamentos: dt_gerencial + liquido)
    SELECT COALESCE(SUM(liquido), 0) 
    INTO v_receita_contahub
    FROM contahub_pagamentos 
    WHERE bar_id = p_bar_id AND dt_gerencial = p_data_evento;
    
    -- Total couvert (contahub_periodo: dt_gerencial + vr_couvert)
    SELECT COALESCE(SUM(vr_couvert), 0)
    INTO v_total_couvert
    FROM contahub_periodo 
    WHERE bar_id = p_bar_id AND dt_gerencial = p_data_evento;
    
    -- Clientes reais (contahub_periodo: dt_gerencial + pessoas)
    SELECT COALESCE(SUM(pessoas), 0)
    ...
    -- Atualizar tabela eventos - APENAS COLUNAS QUE EXISTEM
    UPDATE eventos
    SET
        ...
        -- ✅ ATUALIZAR COLUNAS PRINCIPAIS QUE EXISTEM
        real_r = v_receita_contahub,
        cl_real = v_cl_real_contahub,
        te_real = v_te_real_calculado,  -- ✅ EXISTE: Ticket Entrada
        
        ultima_sync_contahub = NOW()
    WHERE data_evento = p_data_evento AND bar_id = p_bar_id;
    
    -- Return simples sem FORMAT problemático
    RETURN 'Evento atualizado AUTOMATICAMENTE - Data: ' || p_data_evento || 
           ', Bar: ' || p_bar_id || 
           ' | Receita: R$ ' || v_receita_contahub || 
           ' | Clientes: ' || v_cl_real_contahub || 
           ' | TE.Real: R$ ' || v_te_real_calculado || 
           ' | TB.Real: R$ ' || v_tb_real_calculado;
END;
$$;
