-- FUNÇÃO CORRIGIDA: update_eventos_after_contahub_processing()
...
BEGIN
    ...
    -- 1. RECEITA TOTAL (contahub_pagamentos)
    SELECT COALESCE(SUM(valorfinal), 0) 
    ...
    -- 7. CALCULAR TICKETS CORRETOS
    IF v_cl_real_contahub > 0 THEN
        -- TE.Real = Ticket de Entrada (couvert médio por cliente)
        v_te_real_calculado := v_total_couvert / v_cl_real_contahub;
        
        -- TB.Real = Ticket de Bar (receita - couvert) / clientes  
        v_tb_real_calculado := (v_receita_contahub - v_total_couvert) / v_cl_real_contahub;
    END IF;
    
    -- 8. ATUALIZAR TABELA EVENTOS
    UPDATE eventos
    SET
        -- Dados específicos ContaHub
        receita_contahub = v_receita_contahub,
        cl_real_contahub = v_cl_real_contahub,
        te_real_contahub = v_te_real_calculado,  -- Agora é dinheiro (couvert médio)
        tb_real_contahub = v_tb_real_calculado,  -- Agora é dinheiro (receita bar médio)
        ...
        -- Atualizar colunas principais (para dias normais)
        real_r = v_receita_contahub,
        cl_real = v_cl_real_contahub,
        te_real = v_te_real_calculado,  -- Ticket Entrada
        tb_real = v_tb_real_calculado,  -- Ticket Bar
        
        ultima_sync_contahub = NOW()
    WHERE data_evento = p_data_evento AND bar_id = p_bar_id;
    
    RETURN FORMAT(
        'Evento atualizado - Data: %s, Bar: %s | Receita: R$ %.2f | Clientes: %s | TE.Real: R$ %.2f | TB.Real: R$ %.2f | T.Coz: %.1f min | T.Bar: %.1f min',
