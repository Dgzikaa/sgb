// MEGA COLETA MAIO - Velocidade máxima
const fs = require('fs');
const script = fs.readFileSync('coleta_historica_gradual.js', 'utf8');
eval(script);

async function megaColetaMaio() {
  console.log('🚀 MEGA COLETA MAIO: 31 dias em alta velocidade!');
  
  const datas = [];
  for (let dia = 1; dia <= 31; dia++) {
    datas.push('2025-05-' + dia.toString().padStart(2, '0'));
  }
  ...
  console.log(`\n🎉 MAIO CONCLUÍDO! ${sucessos} dias, ${totalRegistros} registros`);
}
