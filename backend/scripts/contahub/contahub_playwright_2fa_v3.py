#!/usr/bin/env python3
"""
Coletor ContaAzul Playwright V3 - VERSÃO FINAL ROBUSTA
- Corrigido problema Unicode (emojis) para Windows
- Corrigido problema DateTime JSON serialization  
- Retry automático em falhas
- Capturas de tela em erros
- Logs otimizados para Windows
...
class WindowsLoggingFormatter(logging.Formatter):
    """Formatter que remove emojis para compatibilidade com Windows"""
    
    def format(self, record):
        # Mapear emojis para texto
        emoji_map = {
            '🚀': '[START]',
            '📁': '[DIR]', 
            '🌐': '[WEB]',
...
console_handler.setFormatter(WindowsLoggingFormatter(
    '%(asctime)s - %(levelname)s - [%(filename)s:%(lineno)d] - %(message)s'
))

# Handler para arquivo
file_handler = logging.FileHandler(
    f'contaazul_logs_{datetime.now().strftime("%Y%m%d")}.log',
    encoding='utf-8'
)
file_handler.setFormatter(WindowsLoggingFormatter(
    '%(asctime)s - %(levelname)s - [%(filename)s:%(lineno)d] - %(message)s'
))
...
class ContaAzulCollector:
    ...
    async def process_excel_file(self, filepath, output_dir):
        ...
        try:
            ...
            # Converter estatísticas para string
            stats_converted = datetime_to_string(self.stats)
            ...
            resultado = {
                ...
                "dados": {
                    ...
                    "estatisticas": stats_converted,
                    "metadados": {
                        "timestamp": datetime.now().isoformat(),
                        "metodo": "playwright_excel_download_v3",
                        "login_com_2fa": True,
                        "versao_robusta": True,
                        "versao_final": True,
                        "correcoes": [
                            "Unicode emojis compatível com Windows",
                            "DateTime JSON serialization corrigido",
                            "Logging otimizado"
                        ],
                        "screenshots_salvos": stats_converted['screenshots_salvos']
                    }
                }
            }
            
            # Salvar resultado
            result_filename = f"resultado_contaazul_v3_{timestamp}.json"
    ...
    async def collect_data(self, output_dir=None):
        ...
        try:
            ...
            logger.info("🚀 Iniciando coleta de dados ContaAzul V3 (Versão Final Robusta)")
            ...
            # Output final para integração (sem datetime objects)
            ...
            stats_safe = datetime_to_string(self.stats)
            
            erro_resultado = {
                "success": False,
                "error": str(e),
                "estatisticas": stats_safe,
                "screenshots_erro": stats_safe['screenshots_salvos']
            }
...
async def main():
    parser = argparse.ArgumentParser(description='Coletor ContaAzul Playwright V3 - Versão Final Robusta')
