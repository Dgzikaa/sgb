...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log(`🤖 PGCRON Manager - Ação: ${action}, Bar: ${barId}`)
    ...
    console.error('❌ Erro no PGCRON Manager:', errorMessage)
    return new Response(
...
async function configurarCronJob(supabaseClient: any, barId: string) {
  try {
    ...
    // 1. Verificar se já existe um job para este bar
    const { data: existingJobs } = await supabaseClient.rpc('verificar_cron_jobs_contaazul', {
      target_bar_id: barId
    })

    if (existingJobs && existingJobs.length > 0) {
      console.log(`⚠️ Job já existe para bar ${barId}, removendo primeiro...`)
    ...
    console.log(`✅ Cron job criado com sucesso: ${jobName}`)
    ...
    return new Response(
      JSON.stringify({ 
        ...
        jobName,
        schedule: cronExpression,
        nextRun: 'Próxima execução conforme cronograma'
      }),
      { 
        status: 200, 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    ...
    const errorMessage = error instanceof Error ? error.message : String(error)
    console.error('❌ Erro ao configurar cron job:', errorMessage)
    throw error
  }
}
...
async function verificarStatus(supabaseClient: any, barId: string) {
  try {
    console.log(`🔍 Verificando status do cron job para bar ${barId}`)
...
async function removerCronJob(supabaseClient: any, barId: string) {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error)
    console.error('❌ Erro ao remover cron job:', errorMessage)
    throw error
  }
}
