...
// Processa o JSON bruto salvo em meta_raw e popula as tabelas diárias
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 1. NÃO limpar tabelas de destino (precisamos manter dados históricos)
    ...
    if (rawError || !metaRaw) {
      ...
      return new Response(JSON.stringify({ success: false, error: 'meta_raw não encontrado', details: rawError }), { status: 404 });
    }
    ...
    return new Response(JSON.stringify({ success: true, message: 'Processamento concluído e tabelas populadas.', meta_raw_id: metaRaw.id }), { status: 200 });
  } catch (e) {
