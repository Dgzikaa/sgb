/**
 * 🗓️ DATE HELPERS - PROTEÇÃO CONTRA ANO INCORRETO (BACKEND)
 * 
...
/**
 * 🗓️ Formata data para ContaHub (DD.MM.YYYY)
 */
export function formatarDataContaHub(data: Date | string): string {
  const d = typeof data === 'string' ? new Date(data) : data;
  const dia = String(d.getDate()).padStart(2, '0');
  const mes = String(d.getMonth() + 1).padStart(2, '0');
  const ano = d.getFullYear();
...
/**
 * 🗓️ Converte data ContaHub (DD.MM.YYYY) para SQL (YYYY-MM-DD)
 */
export function contaHubParaSQL(dataContaHub: string): string {
  const [dia, mes, ano] = dataContaHub.split('.');
  return `${ano}-${mes}-${dia}`;
}
...
export function getDebugInfo(): {
  ...
  return {
    ...
    timezone: 'UTC', // Edge Functions rodam em UTC
    timestampUnix: agora.getTime()
  };
}

/**
 * 🗓️ Converte timestamp UTC para timezone do Brasil (America/Sao_Paulo)
 * Edge Functions rodam em UTC, mas dados devem ser salvos em horário local
 */
export function utcParaBrasil(dataUTC: Date | string): Date {
  const d = typeof dataUTC === 'string' ? new Date(dataUTC) : dataUTC;
  // Brasil: UTC-3 (horário padrão) ou UTC-2 (horário de verão)
  // Simplificado: sempre UTC-3
  const offsetBrasil = -3 * 60; // minutos
  const offsetAtual = d.getTimezoneOffset();
  const diffMinutos = offsetBrasil - offsetAtual;
  
  return new Date(d.getTime() + diffMinutos * 60 * 1000);
}
