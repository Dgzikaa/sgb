...
// Coleta dados do Instagram, Facebook (curtidas) e Ads, salva em meta_raw e chama a função de processamento
...
serve(async (req)=>{
  try {
    ...
    if (credError || !credenciais || !credenciais.configuracoes) {
      console.error('[meta-collect] Credenciais do Meta não encontradas', credError);
      return new Response(JSON.stringify({
        success: false,
        message: 'Credenciais do Meta não encontradas',
        credError
      }), { status: 400 });
    }
    const metaConfig = {
    ...
    // Chamar a API de processamento em produção (Vercel)
    try {
